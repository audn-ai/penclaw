...
import { S as isReasoningConstraintErrorMessage } from "./errors-BwoE4095.js";
...
import { t as sanitizeContentBlocksImages } from "./tool-images-Czo0NZbj.js";
...
function buildAgentsPolicyDigest(content, budget) {
  ...
  for (const candidate of candidates)
    if (highPriorityPattern.test(candidate.line)) trySelect(candidate);
...
function normalizeOpenAIResponsesIdPart(params) {
  ...
  const tail =
    `${sanitizeOpenAIResponsesIdTail(rawTail).slice(0, Math.max(1, maxTailLength - hashSuffix.length)) || "id"}${hashSuffix}`.slice(
      0,
      maxTailLength,
    );
  ...
  for (const msg of messages) {
    ...
    const finalContent = droppedReplayableReasoning
      ? nextContent.map((contentBlock) => {
          ...
          if (contentBlock.type !== "text" || contentBlock.textSignature === void 0)
            return contentBlock;
...
function validateAnthropicTurns(messages) {
  return validateTurnsWithConsecutiveMerge({
    messages: stripDanglingAnthropicToolUses(
      validateTurnsWithConsecutiveMerge({
        messages,
        role: "assistant",
        merge: mergeConsecutiveAssistantTurns,
      }),
...
export {
  ...
  normalizeOpenAIResponsesToolCallIds as u,
};
