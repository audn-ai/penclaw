...
import { t as resolveDoctorPrimaryModelRef } from "./primary-model-ref-DXiTtQTB.js";
import {
  i as resolveProviderToolPolicyEntry,
  n as normalizeToolProviderPolicyKey,
  r as resolveProviderToolPolicy,
} from "./provider-tool-policy-KyAGXjuf.js";
...
import {
  n as isToolAllowedByPolicyName,
  t as isToolAllowedByPolicies,
} from "./tool-policy-match-Bq6mIgOa.js";
...
const channelDoctorModuleLoader = createLazyImportLoader(
  () => import("./channel-doctor-BINiK6s5.js"),
);
...
function hasConfiguredSafeBins(cfg) {
  ...
  if (isRecord(globalExec) && Array.isArray(globalExec.safeBins) && globalExec.safeBins.length > 0)
    return true;
  return listAgentRecords(cfg).some((agent) => {
    const agentExec = isRecord(agent) && isRecord(agent.tools) ? agent.tools.exec : void 0;
    return (
      isRecord(agentExec) && Array.isArray(agentExec.safeBins) && agentExec.safeBins.length > 0
    );
  });
}
...
function collectMessageToolUnavailableTargets(cfg, options = {}) {
  ...
  if (agents.length === 0)
    return (
      options.sourceReplyRuntimeGrant
...
function collectVisibleReplyToolPolicyWarnings(cfg) {
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(targets)}; OpenClaw falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  if (
...
function collectChannelBoundMessageToolPolicyWarnings(cfg) {
  return collectChannelRouteTargets(cfg).flatMap((target) => {
    ...
    if (
      sourceReplyRuntimeMayAllowMessageTool(cfg)
        ? resolveSourceReplyMessageToolAvailability({
            cfg,
      ...
      return [];
    return [
      `- Agent "${target.agentId}" is routed from channel ${formatChannelList(target.channels)}, but the message tool is unavailable for that agent; explicit channel actions such as sendAttachment, upload-file, thread-reply, or reply can fail. Add "message" to the agent tool allowlist, add "group:messaging", or switch the agent to a profile that includes messaging tools.`,
    ];
  });
}
...
function collectConfiguredToolSectionGrantEntries(params) {
  ...
  for (const section of PROFILE_CONFIGURED_TOOL_SECTIONS)
    if (isRecord(params.tools?.[section.key]))
      entries.push({
...
function collectProfileConfiguredToolSectionScopeWarnings(params) {
  ...
  const profile =
    (typeof tools?.profile === "string" ? tools.profile : void 0) ?? params.inheritedProfile;
  if (!profile) return [];
  ...
  const alsoAllow = Array.isArray(tools?.alsoAllow)
    ? tools.alsoAllow.filter((entry) => typeof entry === "string")
  ...
  return [
    `- ${params.pathLabel}.profile is "${profile}" and ${uncoveredEntries.map((entry) => entry.label).join(" / ")} is configured, but configured sections no longer widen the active profile. ${advice}`,
  ];
}
function collectByProviderConfiguredToolSectionWarnings(params) {
  ...
  const inheritedByProvider = isRecord(params.inheritedTools?.byProvider)
  ...
  return Object.entries(byProvider).flatMap(([providerKey, policyValue]) => {
    ...
    return [
      `- ${providerPath}.profile is "${profile}" and ${uncoveredEntries.map((entry) => entry.label).join(" / ")} is configured, but configured sections no longer widen the provider profile. ${advice}`,
    ];
  });
}
...
function collectInheritedByProviderConfiguredToolSectionWarnings(params) {
  ...
  const overridingByProvider = isRecord(params.overridingTools?.byProvider)
  ...
  return Object.entries(inheritedByProvider).flatMap(([providerKey, policyValue]) => {
    ...
    return [
      `- ${inheritedPath}.profile is "${profile}" and ${uncoveredEntries.map((entry) => entry.label).join(" / ")} is configured, but configured sections no longer widen the inherited provider profile. ${advice}`,
    ];
  });
}
...
function collectProfileConfiguredToolSectionWarnings(cfg) {
  ...
  const globalAlsoAllow = Array.isArray(globalTools?.alsoAllow)
    ? globalTools.alsoAllow.filter((entry) => typeof entry === "string")
    : void 0;
  ...
  listAgentRecords(cfg).forEach((agent, index) => {
    ...
    const modelRef = resolveDoctorPrimaryModelRef(
      cfg,
      (agentId ? resolveAgentConfig(cfg, agentId) : void 0)?.model,
    );
    const agentPath = `agents.list[${index}].tools`;
    const includeInheritedSections =
      agentTools !== void 0 && typeof agentTools.profile !== "string";
...
async function resolveDoctorChannelPreviewConfig(params) {
  const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] =
    await Promise.all([
      import("./command-secret-gateway-3bXti0zo.js"),
      import("./command-secret-targets-CPcHE9CX.js"),
    ]);
...
async function collectDoctorPreviewNotes(params) {
  ...
  const channelPluginRuntime = await import("./channel-plugin-blockers-CAcz0cNV.js");
  ...
  if (hasChannelConfig) {
    ...
    const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } =
      await import("./open-policy-allowfrom-D9soBvg6.js");
  ...
  if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
    ...
    } = await import("./stale-plugin-config-bkNjtAsO.js");
  ...
  if (hasPluginConfig) {
    const { collectCodexRouteWarnings } = await import("./codex-route-warnings-BY9naVj7.js");
    warnings.push(
    ...
    const { collectContextEngineHostCompatibilityWarnings } =
      await import("./context-engine-host-compat-BK0OGYgm.js");
    warnings.push(
  ...
  if (hasSubagentAllowlistConfig(params.cfg)) {
    const { collectStaleSubagentAllowlistWarnings, scanStaleSubagentAllowlistReferences } =
      await import("./stale-subagent-allowlist-DgJXKQOq.js");
  ...
  const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-Bqv5EUlx.js");
  infoNotes.push(
    ...(await collectCodexNativeAssetInfoNotes({
      cfg: params.cfg,
      env,
    })),
  );
  if (hasPluginLoadPaths(params.cfg)) {
    const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } =
      await import("./bundled-plugin-load-paths-DNNRrJtE.js");
  ...
  if (hasChannelConfig) {
    ...
    const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-B7FAfK7s.js");
  ...
  if (hasToolsBySenderKey(params.cfg)) {
    const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } =
      await import("./legacy-tools-by-sender-D049pWDX.js");
  ...
  if (hasConfiguredSafeBins(params.cfg)) {
    ...
    } = await import("./exec-safe-bins-BiC6ytoP.js");
  ...
  const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } =
    await import("./stale-oauth-profile-shadows-LNtjNaT0.js");
  ...
  const { collectStaleConfiguredAuthOrderWarnings } =
    await import("./stale-auth-order-BoLmsbMw.js");
  warnings.push(
...
//#endregion
export {
  resolveDoctorChannelPreviewConfig as a,
  collectVisibleReplyToolPolicyWarnings as i,
  collectDoctorPreviewNotes as n,
  collectProfileConfiguredToolSectionWarnings as r,
  collectChannelBoundMessageToolPolicyWarnings as t,
};
