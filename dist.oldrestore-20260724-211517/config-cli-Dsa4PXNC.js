...
import { n as info, o as success, s as warn, t as danger } from "./globals-CNUzHTcU.js";
import {
  $ as AUTO_MANAGED_CONFIG_META_PATHS,
  C as collectUnsupportedSecretRefPolicyIssues,
  E as validateConfigObjectRawWithPlugins,
  u as readConfigFileSnapshot,
} from "./io-B3ne6NxF.js";
import {
  i as normalizeConfigIssues,
  n as formatConfigIssueLines,
} from "./issue-format-IJKFr2SL.js";
import { n as setCommandJsonMode } from "./json-mode-BqA1BKwh.js";
...
import {
  ...
  l as secretRefKey,
  o as isValidSecretProviderAlias,
  r as formatExecSecretRefIdValidationMessage,
  u as validateExecSecretRefId,
} from "./ref-contract-DVV0zYUk.js";
import {
  a as isPluginIntegrationSecretProviderConfig,
  r as resolveSecretRefValue,
  s as resolveSecretProviderIntegrationConfig,
} from "./resolve-Dvpn9c2h.js";
import { i as writeRuntimeJson, r as defaultRuntime } from "./runtime-Bz6o617W.js";
import { n as readBestEffortRuntimeConfigSchema } from "./runtime-schema-CeGDnHvL.js";
import {
...
} from "./target-registry-query-CH3KJb3I.js";
...
import { D as SecretProviderSchema } from "./zod-schema.core-CWTCY1ZZ.js";
          ...
          agents: {
            ...
            ...(defaults
              ? {
                  defaults: {
                    ...defaults,
                    ...(defaults.model !== void 0
                      ? { model: normalizeAgentDefaultModelValueForConfigMutation(defaults.model) }
                      : void 0),
                    ...(defaults.models !== void 0
...
function schemaTypes(schema) {
  ...
  if (Array.isArray(schema.type))
    return new Set(schema.type.filter((entry) => typeof entry === "string"));
...
function propertySchema(schema, segment) {
  ...
  for (const alternative of schemaAlternatives(schema)) {
    ...
    const explicit = (isSchemaRecord(alternative.properties) ? alternative.properties : void 0)?.[
      segment
    ];
...
function mergeConfigValue(existing, patch, path) {
  ...
  if (isRecord(existing) && isRecord(patch)) {
    ...
    for (const [key, value] of Object.entries(patch))
      next[key] =
        hasOwnPathKey(next, key) && isRecord(next[key]) && isRecord(value)
...
function buildApplyValueOperation(params) {
  if (isRecord(params.value) ? coerceSecretRef(params.value) : null)
    return {
      ...buildRefAssignmentOperation({
...
function operationClobbersAncestorChild(operation, managedPath, options) {
  ...
  if (
    ...
    (Boolean(options.merge) && operation.mutation !== "replace")
  )
...
async function runConfigOperations(params) {
  ...
  if (options.dryRun) {
    ...
    if (requiresFullSchemaValidation)
      errors.push(...collectDryRunSchemaErrors({ config: nextConfig }));
...
async function runConfigUnset(opts) {
  ...
  try {
    ...
    const hint = configApplyHintForOperations(
      ...
      normalizeConfigMutationModelRefs(structuredClone(next)),
    );
...
function registerConfigCli(program) {
  const cmd = program
    ...
    .action(async (opts) => {
      const { configureCommandFromSectionsArg } = await import("./configure-CqcBAjae.js");
