...
async function listSpawnedSessionKeys(params) {
  ...
  try {
    ...
    const keys = normalizeTrimmedStringList(
      (Array.isArray(list?.sessions) ? list.sessions : []).map((entry) => entry?.key),
    );
    return new Set(keys);
  } catch {
...
function createAgentToAgentPolicy(cfg) {
  ...
  const allowPatterns = (Array.isArray(routingA2A?.allow) ? routingA2A.allow : []).map((pattern) =>
    compileAgentAllowPattern(pattern),
  );
