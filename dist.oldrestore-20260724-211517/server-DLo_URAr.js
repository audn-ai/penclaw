...
import { n as normalizeUrlPath, r as resolveFileWithinRoot } from "./a2ui-RdgwszOx.js";
import {
  a as isA2uiPath,
  i as injectCanvasRuntime,
...
import { s as ensureDir } from "./utils-DtcDeqWS.js";
...
async function resolveDocumentCspSandbox(rootReal, realPath) {
  const segments = path.relative(rootReal, realPath).split(path.sep);
  if (segments[0] !== "documents" || segments.length < 3) return;
  try {
    const manifestRaw = await fs.readFile(
      path.join(rootReal, segments[0], segments[1] ?? "", "manifest.json"),
      "utf8",
    );
    return JSON.parse(manifestRaw).cspSandbox === "scripts" ? "scripts" : void 0;
  } catch {
    return;
  }
}
  ...
  const handleHttpRequest = async (req, res) => {
    ...
    try {
      ...
      if (mime === "text/html") {
        ...
        res.setHeader("Content-Type", "text/html; charset=utf-8");
        if (await resolveDocumentCspSandbox(rootReal, realPath)) {
  ...
  const server = http.createServer((req, res) => {
    ...
    (async () => {
      if (req.url && isA2uiPath(new URL(req.url, "http://localhost").pathname)) {
        const { handleA2uiHttpRequest } = await import("./a2ui-DHQYhnnm.js");
