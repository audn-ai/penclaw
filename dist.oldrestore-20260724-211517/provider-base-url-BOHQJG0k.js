    ...
    const batchResults = await Promise.all(
      batch.map(async (model) => {
        const showInfo = await queryOllamaModelShowInfoCached(
          apiBase,
          model,
          opts?.apiKey ? { apiKey: opts.apiKey } : void 0,
        );
...
function buildOllamaModelDefinition(modelId, contextWindow, capabilities) {
  ...
  return {
    ...
    contextWindow:
      contextWindow ?? (modelId.trim().toLowerCase() === "glm-5.2:cloud" ? 1e6 : 128e3),
  ...
  try {
    ...
    try {
      ...
      return {
        reachable: true,
        models: (
          (await readProviderJsonResponse(response, "ollama-provider-models.tags")).models ?? []
        ).filter((m) => m.name),
      };
    } finally {
      await release();
