...
import { r as fetchDiscord, t as DISCORD_DIRECTORY_LOOKUP_TIMEOUT_MS } from "./api-BRW47agJ.js";
...
async function listDiscordGuilds(token) {
  return (
    await fetchDiscord("/users/@me/guilds", token, fetch, {
...
async function listDiscordDirectoryGroupsLive(params) {
  ...
  for (const guild of guilds) {
    const channels = await fetchDiscord(`/guilds/${guild.id}/channels`, token, fetch, {
      timeoutMs: DISCORD_DIRECTORY_LOOKUP_TIMEOUT_MS,
    });
