...
e(() => {
  t = {
    ...
    mcpApp: { title: `MCP 앱`, unavailable: `MCP 앱을 사용할 수 없음: {error}` },
    githubPreview: {
    ...
    channels: {
      lastError: `마지막 오류`,
      refreshingStaleSnapshot: `백그라운드에서 채널 상태를 새로 고치는 중입니다. 마지막으로 성공한 스냅샷을 표시합니다.`,
      hub: {
        connectedTitle: `내 채널`,
        addTitle: `채널 추가`,
        ...
        runSetup: `설정 실행`,
        saveBeforeSetup: `저장되지 않은 채널 구성 변경 사항이 있습니다. 안내 설정을 실행하기 전에 저장하거나 다시 불러오세요.`,
      },
      setup: {
        ...
        doneBody: `구성이 저장되었습니다. Gateway가 채널을 자동으로 다시 로드합니다. 실시간 상태는 해당 카드에서 확인하세요.`,
        ...
        whatsappScanHelp: `휴대전화에서 WhatsApp → 설정 → 연결된 기기 → 기기 연결을 연 다음, 이 코드를 스캔하세요.`,
      ...
      generic: { subtitle: `채널 상태 및 구성.` },
      discord: { title: `Discord`, subtitle: `봇 상태 및 채널 구성.` },
      googleChat: { title: `Google Chat`, subtitle: `Chat API 웹훅 상태 및 채널 구성.` },
      imessage: { title: `iMessage`, subtitle: `macOS 브리지 상태 및 채널 구성.` },
      signal: { title: `Signal`, subtitle: `signal-cli 상태 및 채널 구성.` },
      slack: { title: `Slack`, subtitle: `소켓 모드 상태 및 채널 구성.` },
      telegram: { title: `Telegram`, subtitle: `봇 상태 및 채널 구성.` },
      whatsapp: {
        title: `WhatsApp`,
    ...
    lazyView: {
      ...
      staleSubtitle: `OpenClaw가 백그라운드에서 업데이트되었습니다. 최신 패널을 사용하려면 새로고침하세요.`,
      retry: `다시 시도`,
    },
    nodes: {
      pairing: {
        ...
        transportLimitedHint: `이 Gateway URL은 암호화되지 않은 ws://를 사용합니다. wss:// 또는 Tailscale Serve를 사용한 다음, 전체 액세스를 위한 새 코드를 생성하세요.`,
      ...
      inventory: {
        ...
        autoPaired: `자동 페어링됨`,
        versionDrift: `버전 불일치`,
        versionDriftTitle: `노드 {nodeVersion}, Gateway {gatewayVersion}. 오래된 구성 요소를 업데이트하여 플릿을 맞추세요.`,
        manualWake: `수동 깨우기 필요`,
        manualWakeTitle: `Gateway는 오프라인 상태인 Windows 노드를 깨울 수 없습니다. 컴퓨터를 시작하거나 네트워크 연결을 복원하세요.`,
        ...
        removeStalePromptBody: `영향을 받는 클라이언트는 다음 연결 시 별도의 알림 없이 다시 페어링됩니다.`,
        gateway: `gateway`,
        unpaired: `페어링 안 됨`,
    ...
    worktrees: {
      ...
      cleanNow: `지금 정리`,
      cleanupTitle: `정리`,
      cleanupSubtitle: `OpenClaw는 디스크 공간을 확보하기 위해 오래된 worktree를 주기적으로 제거합니다. 정리 수준을 조정하세요.`,
      cleanupMaxCount: `최대 worktree 수`,
      cleanupMaxCountHelp: `모든 저장소에서 유지할 관리 대상 worktree의 최대 개수입니다. 오래된 worktree부터 스냅샷을 생성한 후 제거합니다. 개수 제한을 비활성화하려면 0으로 설정하세요.`,
      cleanupMaxSize: `최대 총크기(GB)`,
      cleanupMaxSizeHelp: `관리되는 모든 worktree의 최대 총크기(GB)입니다. 크기 제한을 비활성화하려면 0으로 설정하세요.`,
...
스냅샷 없이 삭제할까요?`,
    ...
    agentScope: { label: `에이전트 범위`, allAgents: `모든 에이전트` },
    newSession: {
      title: `새 세션`,
      ...
      createOutcomeUnknown: `이 세션을 시작하는 동안 Gateway가 변경되었습니다. 이 작업을 다시 시작하기 전에 최근 세션을 확인하세요.`,
      catalogUnavailable: `이 세션 대상을 사용할 수 없습니다.`,
    },
    sessionsView: {
      deletePreservedWorktrees: `커밋되지 않았거나 푸시되지 않은 작업이 있는 {count}개의 세션 worktree가 유지되었습니다({branches}). Settings -> Worktrees에서 관리하세요.`,
      deletePreservedWorktreeConfirm: `세션의 worktree에 커밋되지 않았거나 푸시되지 않은 작업이 있어 유지되었습니다({branch}). 그래도 체크아웃을 삭제하시겠습니까?`,
      title: `세션`,
      ...
      transcriptSearchDescription: `기본 에이전트의 세션에서 사용자 및 어시스턴트 메시지의 정확한 단어나 구문을 찾습니다.`,
      ...
      transcriptSearchIndexing: `대화 기록 색인이 아직 업데이트 중입니다. 최근 메시지를 포함하려면 다시 시도하세요.`,
    ...
    agents: {
      ...
      toolCatalog: {
        ...
        profiles: { minimal: `최소`, coding: `코딩`, messaging: `메시징`, full: `전체` },
      },
      skillsPanel: {
        title: `Skills`,
        ...
        allEnabled: `모든 스킬이 활성화되어 있습니다. 스킬을 비활성화하면 에이전트별 허용 목록이 생성됩니다.`,
    ...
    configForm: {
      ...
      sections: {
        env: { label: `환경 변수`, description: `게이트웨이 프로세스에 전달되는 환경 변수` },
        update: { label: `업데이트`, description: `자동 업데이트 설정 및 릴리스 채널` },
        agents: { label: `에이전트`, description: `에이전트 구성, 모델 및 ID` },
        auth: { label: `인증`, description: `API 키 및 인증 프로필` },
        channels: { label: `채널`, description: `메시징 채널 (Telegram, Discord, Slack 등)` },
        messages: { label: `메시지`, description: `메시지 처리 및 라우팅 설정` },
        commands: { label: `명령`, description: `사용자 지정 슬래시 명령` },
        hooks: { label: `후크`, description: `웹훅 및 이벤트 후크` },
        skills: { label: `Skills`, description: `Skill 팩 및 기능` },
        tools: { label: `도구`, description: `도구 구성 (브라우저, 검색 등)` },
        gateway: { label: `Gateway`, description: `Gateway 서버 설정 (포트, 인증, 바인딩)` },
        wizard: { label: `설정 마법사`, description: `설정 마법사 상태 및 기록` },
        meta: { label: `메타데이터`, description: `Gateway 메타데이터 및 버전 정보` },
        logging: { label: `로깅`, description: `로그 수준 및 출력 구성` },
        browser: { label: `브라우저`, description: `브라우저 자동화 설정` },
        ui: { label: `UI`, description: `사용자 인터페이스 기본 설정` },
        models: { label: `모델`, description: `AI 모델 구성 및 공급자` },
        bindings: { label: `바인딩`, description: `키 바인딩 및 단축키` },
        broadcast: { label: `브로드캐스트`, description: `브로드캐스트 및 알림 설정` },
        audio: { label: `오디오`, description: `오디오 입력/출력 설정` },
        session: { label: `세션`, description: `세션 관리 및 지속성` },
        cron: { label: `Cron`, description: `예약된 작업 및 자동화` },
        web: { label: `Web`, description: `웹 서버 및 API 설정` },
        discovery: { label: `Discovery`, description: `서비스 검색 및 네트워킹` },
        canvasHost: { label: `Canvas Host`, description: `캔버스 렌더링 및 표시` },
        talk: { label: `Talk`, description: `음성 및 발화 설정` },
        plugins: { label: `플러그인`, description: `플러그인 관리 및 확장` },
        diagnostics: { label: `진단`, description: `계측, OpenTelemetry 및 캐시 추적 설정` },
        cli: { label: `CLI`, description: `CLI 배너 및 시작 동작` },
        secrets: { label: `Secrets`, description: `시크릿 공급자 구성` },
        acp: { label: `ACP`, description: `Agent Communication Protocol 런타임 및 스트리밍 설정` },
        mcp: { label: `MCP`, description: `Model Context Protocol 서버 정의` },
      },
    },
    quickSettings: {
      language: `언어`,
      model: {
        ...
        thinking: `사고`,
        fastMode: `빠른 모드`,
        thinkingLevels: { off: `끔`, low: `낮음`, medium: `보통`, high: `높음` },
        fastModes: { auto: `자동`, fast: `빠름`, standard: `표준` },
      },
      channels: {
        title: `채널`,
    ...
    configView: {
      ...
      themes: {
        claw: { label: `Claw`, description: `Chroma 계열` },
        knot: { label: `Knot`, description: `검정 & 빨강` },
        dash: { label: `Dash`, description: `초콜릿 블루프린트` },
      },
      textSizes: { small: `작게`, default: `기본값`, large: `크게`, xl: `XL`, xxl: `XXL` },
      notifications: {
        title: `푸시 알림`,
        ...
        subscribing: `구독 중...`,
        enable: `알림 사용`,
        blockedHint: `알림이 차단되었습니다. 알림을 허용하려면 브라우저 사이트 권한을 업데이트하세요.`,
      },
      appearance: {
        theme: `테마`,
        ...
        tweakcnInstructions: `tweakcn.com을 열고 테마를 선택하거나 만든 다음 Share를 클릭하고, 복사된 테마 링크를 여기에 붙여넣으세요. 공유 링크, 편집기 URL, 레지스트리 URL, 테마 ID 및 amethyst-haze와 같은 기본 테마 이름을 사용할 수 있습니다.`,
        ...
        loaded: `로드됨`,
        inlineHintBefore: `클릭`,
        inlineHintAfter: `하여 브라우저 로컬 tweakcn 테마를 하나 추가하세요. tweakcn에서 Share를 사용하고 복사된 링크를 여기에 붙여넣으세요.`,
        textSize: `텍스트 크기`,
      },
      chatPrefs: { title: `채팅`, hint: `브라우저 로컬 채팅 환경설정입니다.` },
      connection: { title: `연결`, gateway: `Gateway`, status: `상태`, assistant: `어시스턴트` },
      root: `<root>`,
      itemCount: `[{count}개 항목]`,
      ...
      formUnsafe: `구성에 폼 편집기가 안전하게 표현할 수 없는 필드가 포함되어 있습니다. 해당 항목을 편집하려면 Raw 모드를 사용하세요.`,
      ...
      sensitiveHidden: `민감한 값 {count}개가 숨겨져 있습니다. Raw 구성을 편집하려면 위의 표시 버튼을 사용하세요.`,
      sensitiveHiddenPlural: `민감한 값 {count}개가 숨겨져 있습니다. Raw 구성을 편집하려면 위의 표시 버튼을 사용하세요.`,
    },
    execApproval: {
    ...
    agentTools: {
      ...
      explicitAllowlist: `이 에이전트는 구성에서 명시적 허용 목록을 사용하고 있습니다. 도구 재정의는 Config 탭에서 관리됩니다.`,
      globalAllowlist: `전역 tools.allow가 설정되어 있습니다. 에이전트 재정의는 전역적으로 차단된 도구를 사용 설정할 수 없습니다.`,
      loadingCatalog: `런타임 도구 카탈로그를 로드하는 중…`,
      catalogFallback: `런타임 도구 카탈로그를 로드할 수 없습니다. 대신 기본 제공 대체 목록을 표시합니다.`,
    ...
    skillsPage: {
      ...
      tabs: { all: `전체`, ready: `준비됨`, needsSetup: `설정 필요`, disabled: `비활성화됨` },
    ...
    browser: {
      ...
      annotatePrompt: {
        introTitled: `{url}의 페이지에 주석을 달았습니다(페이지에서 보고한 제목: "{title}") — 첨부된 스크린샷에 제 마크업이 표시되어 있습니다.`,
        introUntitled: `{url}의 페이지에 주석을 달았습니다 — 첨부된 스크린샷에 제 마크업이 표시되어 있습니다.`,
        region: `표시된 영역 {index}: 가로 {x}% / 세로 {y}% 지점을 중심으로, 보기의 약 {width}% × {height}% 범위입니다.`,
        moreRegions: `…추가로 표시된 영역 {count}개가 더 있으며, 모두 스크린샷에 표시됩니다.`,
        elementDetail: `표시된 요소(페이지에서 보고됨): {descriptor} — ({x}, {y}) 위치의 {width}×{height}px.`,
    ...
    mcpPage: {
      ...
      runtimeHint: `런타임 변경은 저장 및 게시 후에 적용됩니다. 활성 에이전트는 다음 사용 시 MCP 런타임을 다시 빌드합니다.`,
      toolFilter: `도구 필터`,
      parallel: `병렬`,
    ...
    pluginsPage: {
      ...
      connectorAdded: `추가됨`,
      connectorAddedOauth: `{name}을(를) 추가했습니다. “{command}”로 인증한 다음 gateway를 다시 시작하세요.`,
      connectorAddedEndpoint: `{name}을(를) 추가했습니다. 사용하기 전에 MCP 설정에서 엔드포인트와 자격 증명을 업데이트하세요.`,
      connectorAddedReady: `{name}이(가) 추가되었습니다. 새 에이전트 세션에서 바로 사용할 수 있습니다.`,
      connectorDescriptions: {
        ...
        spotify: `기분에 맞춘 플레이리스트로 검색하고, 대기열에 추가하고, 하루를 음악으로 채워보세요.`,
      ...
      mcpHint: `에이전트에 추가 도구를 제공하려면 Model Context Protocol 서버를 연결하세요. 변경 사항은 새 에이전트 세션에 적용됩니다.`,
      mcpAdd: `서버 추가`,
      mcpAdding: `추가 중…`,
      mcpEmpty: `아직 구성된 MCP 서버가 없습니다. 여기에서 추가하거나 Discover에서 커넥터를 선택하세요.`,
      mcpNameLabel: `이름`,
      ...
      cancel: `취소`,
      removedRestart: `{name}이(가) 제거되었습니다. 변경 사항을 적용하려면 Gateway를 다시 시작해야 합니다.`,
      ...
      installedRestart: `{name}을(를) 설치했습니다. 변경 사항을 적용하려면 Gateway를 다시 시작해야 합니다.`,
      enabledSuccess: `{name}을(를) 활성화했습니다.`,
      enabledRestart: `{name}을(를) 활성화했습니다. 변경 사항을 적용하려면 Gateway를 다시 시작해야 합니다.`,
      disabledSuccess: `{name}을(를) 비활성화했습니다.`,
      disabledRestart: `{name}을(를) 비활성화했습니다. 변경 사항을 적용하려면 Gateway를 다시 시작해야 합니다.`,
    },
    aboutPage: {
    ...
    profilePage: {
      ...
      emptyBody: `아직 토큰 활동이 없습니다. 에이전트에게 메시지를 보내고 산호초가 빛나는 모습을 확인해 보세요.`,
    ...
    tasksPage: {
      ...
      runtime: { subagent: `하위 에이전트`, cron: `Cron`, acp: `ACP`, cli: `CLI`, unknown: `작업` },
    },
    skillWorkshop: {
      ...
      header: {
        ...
        selfLearningTooltip: `수정 사항과 완료된 주요 작업을 포착하고 검토하여 보류 중인 Skill 제안으로 만듭니다. 추가 백그라운드 토큰을 사용하며, 초안은 이 보드에 보류 중인 제안으로 등록됩니다.`,
        view: `워크숍 보기`,
        board: `보드`,
        today: `오늘`,
      },
      ...
      recency: { today: `오늘`, yesterday: `어제`, earlier: `이번 주 초` },
      ...
      revision: {
        title: `{verb} 제안`,
        description: `무엇을 변경할지 에이전트에게 알려주세요. 제안은 대기 상태로 유지되며 워크숍이 수정된 버전을 만듭니다.`,
        placeholder: `예: 읽지 않음 검색 대신 Gmail 라벨을 사용하고, 더 안전한 드라이런 단계를 추가하세요.`,
      ...
      selfLearning: {
        pitchTitle: `자기 학습 켜기`,
        pitchBody: `OpenClaw는 수정 사항과 완료된 주요 실행을 검토한 후 이 보드에 게시할 Skill 제안 초안을 작성합니다. 추가 백그라운드 토큰을 사용하며, 초안은 보류 중인 제안으로 등록됩니다.`,
        enable: `자기 학습 활성화`,
        enabling: `활성화 중…`,
      ...
      today: {
        ...
        emptyBody: `에이전트가 새로운 항목을 작성하지 않았습니다. Board로 전환하여 기록을 살펴보세요.`,
        new: `신규`,
        waiting: `대기 중`,
        reviewed: `검토됨`,
        agent: `에이전트`,
    ...
    activity: {
      ...
      status: { running: `실행 중`, done: `완료`, error: `오류` },
...
테스트:`,
    ...
    palette: {
      ...
      noResults: `결과 없음`,
      categories: { search: `검색`, navigation: `탐색`, skills: `Skills` },
      items: {
        sessions: `세션`,
        scheduled: `예약됨`,
        skills: `Skills`,
      ...
      footer: { navigate: `이동`, select: `선택`, close: `닫기` },
    },
    dreaming: {
      tabs: { scene: `장면`, diary: `일지`, advanced: `고급` },
      header: {
        refresh: `새로고침`,
        refreshing: `새로 고치는 중…`,
        on: `드리밍 켜짐`,
      ...
      phase: { light: `얕음`, deep: `깊음`, rem: `렘`, off: `꺼짐` },
      advanced: {
        eyebrow: `검토`,
        title: `일일 로그 검토`,
      ...
      wiki: {
        ...
        palaceTab: `메모리 팰리스`,
        dreamsExplainer: `이것은 시스템이 메모리를 재생하고 통합하는 동안 작성하는 원본 꿈 일기입니다. 메모리 시스템이 무엇을 인식하고 있는지, 그리고 어디가 아직 노이즈가 많거나 빈약해 보이는지 살펴보는 데 사용하세요.`,
        insightsExplainer: `이것은 외부 기록에서 클러스터링된 가져온 인사이트입니다. 어떤 것이든 지속적인 메모리로 승격되기 전에 가져오기가 무엇을 드러냈는지 검토하는 데 사용하세요.`,
        palaceExplainer: `이것은 시스템이 검색하고 추론할 수 있는 컴파일된 메모리 위키 표면입니다. 원본으로 가져온 소스 채팅이 아니라 실제 메모리 페이지, 주장, 미해결 질문, 모순을 살펴보는 데 사용하세요.`,
        copyArchivePath: `아카이브 경로 복사`,
        loadingInsights: `가져온 인사이트 로드 중…`,
        noInsights: `아직 가져온 인사이트가 없습니다`,
        noInsightsHint: `적용과 함께 ChatGPT 가져오기를 실행하면 클러스터링된 인사이트가 여기에 표시됩니다.`,
        ...
        emptyPalaceHint: `현재 위키에는 대부분 원시 소스 가져오기와 운영 보고서만 있습니다. 이 탭은 종합, 엔터티 또는 개념이 작성되기 시작하면 유용해집니다.`,
        claims: `주장`,
    ...
    modelProviders: {
      ...
      readOnly: {
        ...
        adminRequired: `탐색만 가능합니다. 모델을 변경하려면 operator.admin 액세스 권한이 필요합니다.`,
      },
      notes: {
    ...
    usage: {
      common: { emptyValue: `—`, unknown: `알 수 없음` },
      loading: { title: `사용량 개요`, badge: `로딩 중` },
      metrics: { tokens: `토큰`, cost: `비용`, session: `세션`, sessions: `세션` },
      providerUsage: {
        title: `제공업체 요금제 및 결제`,
      ...
      export: { label: `내보내기`, sessionsCsv: `세션 CSV`, dailyCsv: `일별 CSV`, json: `JSON` },
      cacheStatus: {
        warning: `사용량 캐시가 백그라운드에서 다시 빌드되고 있습니다. 표시된 합계가 최신이 아닐 수 있습니다.`,
        title: `{status}: 대기 중 {pending}, 오래됨 {stale}, 캐시됨 {cached}`,
        status: { refreshing: `새로 고치는 중`, stale: `오래됨`, partial: `부분적` },
      },
      empty: {
        title: `날짜 범위부터 시작하세요`,
    ...
    login: {
      ...
      failure: {
        ...
        authRequired: {
          title: `인증 필요`,
          summary: `Gateway에 연결할 수 있지만 이 브라우저가 연결되기 전에 일치하는 토큰 또는 비밀번호가 필요합니다.`,
          stepPaste: `openclaw dashboard --no-open의 토큰을 붙여넣거나 구성된 비밀번호를 입력하세요.`,
          stepGenerate: `토큰이 구성되어 있지 않으면 Gateway 호스트에서 openclaw doctor --generate-gateway-token을 실행하세요.`,
          stepConnect: `자격 증명을 업데이트한 뒤 Connect를 다시 클릭하세요.`,
        },
        authFailed: {
          title: `인증이 일치하지 않음`,
          summary: `제공한 자격 증명이 거부되었습니다. 가장 흔한 원인은 오래된 토큰이거나 다른 Gateway URL에서 복사한 토큰입니다.`,
          stepDashboard: `openclaw dashboard --no-open을 실행하고 새 URL을 열거나 해당 토큰을 붙여넣으세요.`,
          stepReplace: `오래된 토큰/비밀번호 값을 교체하세요. 다른 Gateway URL의 토큰을 재사용하지 마세요.`,
          stepMode: `한 번에 하나의 일치하는 인증 모드만 사용하세요. 토큰 모드에는 gateway token, 비밀번호 모드에는 비밀번호를 사용합니다.`,
        },
        rateLimited: {
          title: `실패한 시도가 너무 많음`,
          ...
          stepCheckClients: `공유 호스트라면 다른 클라이언트가 잘못된 재시도를 반복하는지 확인하세요.`,
        },
        pairing: {
          title: `장치 페어링 필요`,
          ...
          upgradeSummary: `이 브라우저는 이미 알려져 있지만 요청한 액세스가 변경되어 새 승인이 필요합니다.`,
        ...
        insecure: {
          ...
          summary: `이 페이지는 일반 HTTP에서 실행 중이므로 브라우저가 Gateway가 기대하는 장치 ID를 만들 수 없습니다.`,
          stepHttps: `HTTPS/Tailscale Serve를 사용하거나 Gateway 호스트에서 http://127.0.0.1:18789를 여세요.`,
          stepLocalCompat: `로컬 토큰 전용 호환성을 위해 gateway.controlUi.allowInsecureAuth: true를 설정하세요.`,
        ...
        origin: {
          ...
          stepFullOrigin: `와일드카드 패턴이 아니라 http://localhost:5173 같은 전체 origin을 사용하세요.`,
          stepRestart: `허용 origin을 변경한 뒤 Gateway를 다시 시작하거나 다시 로드하세요.`,
        },
        protocol: {
          title: `프로토콜 불일치`,
          summary: `제공된 Control UI와 실행 중인 Gateway가 지원되는 연결 프로토콜에 동의하지 않습니다.`,
          stepDashboard: `UI와 Gateway가 같은 설치에서 오도록 openclaw dashboard로 제공된 dashboard를 다시 여세요.`,
          stepDevUi: `pnpm ui:dev를 사용하는 경우 현재 checkout 기준으로 개발 UI를 다시 빌드하거나 다시 시작하세요.`,
          stepRestart: `OpenClaw를 업데이트한 뒤 Gateway를 다시 시작하여 현재 프로토콜을 제공하게 하세요.`,
        },
        network: {
          title: `연결할 수 없음`,
          summary: `브라우저가 Gateway 연결을 완료할 수 없습니다. 자격 증명을 다시 시도하기 전에 대상과 전송 방식을 확인하세요.`,
          stepGateway: `openclaw status 또는 openclaw gateway run으로 Gateway가 실행 중인지 확인하세요.`,
          stepUrl: `WebSocket URL을 확인하고 Gateway가 HTTPS/Tailscale Serve 뒤에 있으면 wss://를 사용하세요.`,
          stepDashboard: `openclaw dashboard --no-open으로 dashboard를 다시 열어 현재 URL과 인증 세부 정보를 다시 복사하세요.`,
        },
      },
    },
    chat: {
      ...
      pullRequests: {
        ...
        rateLimited: `GitHub API 속도 제한에 도달했습니다. 제한이 재설정될 때까지 Pull request 상태가 최신이 아닐 수 있습니다.`,
      },
      usageRemaining: `남은 사용량`,
      view: { menu: `보기`, reasoning: `추론`, toolCalls: `도구 호출`, commentary: `해설 유지` },
      sendShortcut: `전송 단축키`,
      ...
      compaction: {
        ...
        description: `압축된 대화 기록은 체크포인트로 보존됩니다. 세션 체크포인트를 열어 해당 압축 보기에서 분기하거나 복원하세요.`,
      ...
      commands: {
        ...
        categories: { session: `세션`, model: `모델`, agents: `에이전트`, tools: `도구` },
      },
      splitView: {
        open: `분할 보기 열기`,
        splitRight: `오른쪽으로 분할`,
      ...
      modelControls: { default: `기본값`, reasoning: `추론`, speed: `속도` },
      sideChat: {
        title: `사이드 채팅`,
      ...
      composer: {
        ...
        microphonePageInactive: `이 페이지가 비활성 상태인 동안에는 마이크 입력을 사용할 수 없습니다.`,
        microphonePermissionBlocked: `마이크 액세스가 차단되었습니다. 입력 목록을 표시하려면 브라우저 사이트 설정에서 허용하세요.`,
        realtimeTalkRequiresMicrophone: `실시간 음성 입력을 사용하려면 브라우저 마이크 접근 권한이 필요합니다.`,
        selectedMicrophoneUnavailable: `선택한 마이크를 사용할 수 없습니다. 다른 입력 또는 시스템 기본값을 선택하세요.`,
    ...
    cron: {
      tabs: { filterLabel: `자동화 상태`, all: `전체`, active: `활성`, paused: `일시중지됨` },
      list: {
        viewLabel: `자동화 보기`,
      ...
      suggestions: {
        ...
        ideas: {
          repoPulse: {
            ...
            prompt: `내 저장소의 밤새 활동을 검토하세요: 새 이슈, 풀 리퀘스트, CI 실패. 오늘 가장 주의가 필요한 세 가지를 링크와 한 줄 이유와 함께 요약해 주세요.`,
          },
          standupGhostwriter: {
            name: `스탠드업 대필`,
            tagline: `어제 작업을 바탕으로 스탠드업 업데이트를 자동 작성.`,
            prompt: `어제의 커밋, 병합된 풀 리퀘스트, 열린 리뷰 스레드를 바탕으로 스탠드업 업데이트를 작성해 주세요. 최대 세 줄: 완료, 진행 중, 블로킹.`,
          },
          hackerNewsScout: {
            name: `Hacker News 스카우트`,
            tagline: `커피 한 잔과 함께 읽을 링크 세 개, 핫한 의견 포함.`,
            prompt: `오늘 Hacker News 프론트 페이지에서 AI 에이전트, 개발자 도구, TypeScript 관련 게시물을 찾아 주세요. 가장 흥미로운 링크 세 개를 각각 한 줄 핫테이크와 함께 보내 주세요.`,
          },
          dependencyRadar: {
            name: `의존성 레이더`,
            tagline: `오래되었거나 취약한 의존성을 업그레이드 메모와 함께 정리.`,
            prompt: `내 메인 프로젝트에서 오래되었거나 취약한 의존성을 확인해 주세요. 주목할 업데이트를 각각 한 줄 위험 메모와 함께 나열하고, 업그레이드 명령어를 작성해 주세요.`,
          },
          watchdog: {
            name: `야간 감시`,
            tagline: `한 줄 결과로 확인하는 시간별 상태 점검.`,
            prompt: `서비스와 Gateway의 상태를 확인해 주세요: 최근 로그에서 새로운 오류, 재시작, 또는 비정상적인 부하를 스캔하세요. 모든 것이 정상이면 짧은 이상 없음 한 줄로 답하고, 문제가 있어 보이면 무엇이 실패했는지와 어디서부터 살펴봐야 할지 보고해 주세요.`,
          },
          polyglotMinute: {
            name: `폴리글랏 미닛`,
            tagline: `아침 커피와 함께하는 유용한 외국어 표현 하나.`,
            prompt: `일본어에서 유용한 표현 하나를 알려주세요: 표현, 발음 방법, 직역 의미, 그리고 언제 사용하는지. 다섯 줄 이내로 간단하게 설명해 주세요.`,
          },
        },
      },
      stats: { tasks: `작업`, failing: `실패 중`, nextWake: `다음 실행` },
      detail: {
        tabsLabel: `자동화 세부 정보`,
        newTitle: `새 예약 작업`,
...
export { t as ko };
//# sourceMappingURL=ko-CYrtX1K5.js.map
