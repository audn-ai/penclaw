import { html as y, t as b } from "./dist-D_xBiePP.js";
import { a as g, n as _, r as v } from "./dist-DcfhWyQe.js";
import {
  A as t,
  E as n,
  L as r,
...
function S(e, t, n) {
  return new _((r) => {
    ...
    for (;;) {
      ...
      if ((r.advance(), t == 10)) break;
    }
    r.pos > i && r.acceptToken(t);
  });
}
function C(e, t) {
  return e.split(` `).map((e) => ({ label: e, type: t }));
}
function w(e) {
  let { state: t, pos: r } = e,
    i = n(t).resolveInner(r, -1).enterUnfinishedNodesBefore(r),
    a = i.childBefore(r)?.name || i.name;
  if (i.name == `FilterName`) return { type: `filter`, node: i };
  if (e.explicit && a == `|`) return { type: `filter` };
  if (i.name == `TagName`) return { type: `tag`, node: i };
  if (e.explicit && a == `{%`) return { type: `tag` };
  if (i.name == `PropertyName` && i.parent.name == `MemberExpression`)
    return { type: `property`, node: i, target: i.parent };
  if (i.name == `.` && i.parent.name == `MemberExpression`)
    return { type: `property`, target: i.parent };
  if (i.name == `MemberExpression` && a == `.`) return { type: `property`, target: i };
  if (i.name == `VariableName`) return { type: `expression`, from: i.from };
  let o = e.matchBefore(/[\w\u00c0-\uffff]+$/);
  return o
    ? { type: `expression`, from: o.from }
    : e.explicit &&
        i.name != `CommentText` &&
        i.name != `StringLiteral` &&
        i.name != `NumberLiteral` &&
        i.name != `InlineComment`
      ? { type: `expression` }
      : null;
}
function T(e, t, n, r) {
  let i = [];
  for (;;) {
    let n = t.getChild(`Expression`);
    if (!n) return [];
    if (n.name == `VariableName` || n.name == `forloop` || n.name == `tablerowloop`) {
      let t = e.sliceDoc(n.from, n.to);
      if (t == `forloop`) return i.length ? [] : J;
      if (t == `tablerowloop`) return i.length ? [] : Y;
      i.unshift(t);
      break;
    } else if (n.name == `MemberExpression`) {
      let r = n.getChild(`PropertyName`);
      (r && i.unshift(e.sliceDoc(r.from, r.to)), (t = n));
    } else if (n.name == `SubscriptExpression`) {
      let r = n.getChildren(`Expression`)[1];
      (i.unshift(r?.name == `StringLiteral` ? e.sliceDoc(r.from + 1, r.to - 1) : `[]`), (t = n));
    } else return [];
  }
  return r ? r(i, e, n) : [];
}
function te(e = {}) {
  let t = e.filters ? e.filters.concat(G) : G,
    n = e.tags ? e.tags.concat(K) : K,
    r = e.variables ? e.variables.concat(q) : q,
    { properties: i } = e;
  return (e) => {
    let a = w(e);
    if (!a) return null;
    let o = a.from ?? (a.node ? a.node.from : e.pos),
      s;
    return (
      (s =
        a.type == `filter`
...
function E(e) {
  return (t) => {
    let n = e.test(t.textAfter);
    return t.lineIndent(t.node.from) + (n ? 0 : t.unit);
  };
}
function D(e) {
  return Z.configure(
    {
      wrap: f((t) =>
        t.type.isTop
          ? { parser: e.parser, overlay: (e) => e.name == `Text` || e.name == `RawText` }
          : null,
      ),
    },
    `liquid`,
...
function O(e = {}) {
  let t = e.base || Q,
    n = t.language == Q.language ? $ : D(t.language);
  return new o(n, [
    t.support,
    n.data.of({ autocomplete: te(e) }),
    t.language.data.of({ closeBrackets: { brackets: [`{`] } }),
    X,
  ]);
}
var k, A, j, M, N, P, F, I, L, R, z, B, V, H, U, W, G, K, q, J, Y, X, Z, Q, $;
e(() => {
  (h(),
    b(),
    u(),
        ...
        if (n == 123) {
          ...
          if (n == 123) {
            ...
            for (;;) {
              ...
              else if (t == 45 && r == 2) r = ++n;
              else {
                let i = t == 101 && e.peek(n + 1) == 110 && e.peek(n + 2) == 100;
                e.acceptToken(i ? j : A, r);
                return;
              }
            }
          }
        }
        if ((e.advance(), n == 10)) break;
      }
      e.pos > t && e.acceptToken(M);
    })),
    (z = S(`endcomment`, I, F)),
    (B = S(`endraw`, P, N)),
    (V = new _((e) => {
      if (e.next == 35) {
        for (
          e.advance();
          !(e.next == 10 || e.next < 0 || ((e.next == 37 || e.next == 125) && e.peek(1) == 125));
        )
          e.advance();
        e.acceptToken(L);
      }
    ...
    (q = C(`empty forloop tablerowloop in with as`, `keyword`)),
    (J = C(`first index index0 last length rindex`, `property`)),
    (Y = C(
      `col col0 col_first col_last first index index0 last length rindex rindex0 row`,
      `property`,
    )),
    (X = i.inputHandler.of((e, t, n, r) =>
      r != `%` || t != n || e.state.doc.sliceString(t - 1, n + 1) != `{}`
        ? !1
        : (e.dispatch(
            e.state.changeByRange((e) => ({
              changes: { from: e.from, to: e.to, insert: `%%` },
              range: a.cursor(e.from + 1),
            })),
      ...
      parser: W.configure({
        props: [
          t({
            ...
            TagName: l.tagName,
            FilterName: l.function(l.variableName),
          ...
          m.add({
            ...
            "UnlessDirective ForDirective TablerowDirective CaptureDirective":
              E(/^\s*(\{%-?\s*)?end\w/),
            IfDirective: E(/^\s*(\{%-?\s*)?(endif|else|elsif)\b/),
            CaseDirective: E(/^\s*(\{%-?\s*)?(endcase|when)\b/),
          }),
          ee.add({
            "UnlessDirective ForDirective TablerowDirective CaptureDirective IfDirective CaseDirective RawDirective Comment"(
              ...
              return !t || t.name != `Tag`
                ? null
                : { from: t.to, to: n.name == `EndTag` ? n.from : e.to };
            },
          }),
        ],
      }),
...
export { O as liquid };
//# sourceMappingURL=dist-C1g5IDt7.js.map
