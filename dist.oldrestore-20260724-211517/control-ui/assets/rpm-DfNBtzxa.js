...
e(() => {
  ((t = /^-+$/),
    ...
    (i = {
      name: `rpmchanges`,
      token: function (e) {
        return e.sol() && (e.match(t) || e.match(n))
          ? `tag`
          : e.match(r)
            ? `string`
            : (e.next(), null);
    ...
    (d = {
      ...
      token: function (e, t) {
        ...
        if (e.sol()) {
          if (e.match(o)) return `header`;
          if (e.match(s)) return `atom`;
        }
        if (e.match(/^\$\w+/) || e.match(/^\$\{\w+\}/)) return `def`;
        if (e.match(l)) return `keyword`;
        if (e.match(c)) return ((t.controlFlow = !0), `keyword`);
        if (t.controlFlow) {
          if (e.match(u)) return `operator`;
          if (e.match(/^(\d+)/)) return `number`;
          e.eol() && (t.controlFlow = !1);
        }
        if (e.match(a)) return (e.eol() && (t.controlFlow = !1), `number`);
        if (e.match(/^%[\w]+/)) return (e.match(`(`) && (t.macroParameters = !0), `keyword`);
        if (t.macroParameters) {
          if (e.match(/^\d+/)) return `number`;
          if (e.match(`)`)) return ((t.macroParameters = !1), `keyword`);
        }
        return e.match(/^%\{\??[\w \-\:\!]+\}/)
          ? (e.eol() && (t.controlFlow = !1), `def`)
          : (e.next(), null);
      },
    }));
})();
export { i as rpmChanges, d as rpmSpec };
//# sourceMappingURL=rpm-DfNBtzxa.js.map
