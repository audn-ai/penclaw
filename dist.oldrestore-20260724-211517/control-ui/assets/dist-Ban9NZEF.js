import { a as oe, r as se } from "./dist--Ncd3Hap.js";
import { a as re, n as c, r as ie, t as ae } from "./dist-DcfhWyQe.js";
import {
  A as t,
  a as n,
  j as r,
...
function d(e) {
  let t;
  return e.next == H && ((t = e.peek(1)) == H || t == U);
}
function f(e, t) {
  ((this.parent = e),
    (this.depth = t),
...
var p,
  ...
  be = e(() => {
    (re(),
      ...
      (J = new c(
        (e, t) => {
          if (t.dialectEnabled(A)) {
            let n;
            if (e.next < 0 && t.canShift(w)) e.acceptToken(w);
            else if (((n = e.peek(-1)) == W || n < 0) && t.canShift(C)) {
              let t = 0;
              for (; e.next != W && j.includes(e.next);) (e.advance(), t++);
              e.next == W || d(e) ? e.acceptToken(C, -t) : t && e.acceptToken(T);
            } else if (e.next == W) e.acceptToken(le, 1);
            else if (j.includes(e.next)) {
              for (e.advance(); e.next != W && j.includes(e.next);) e.advance();
              e.acceptToken(T);
            }
          } else {
            let t = 0;
            for (; j.includes(e.next);) (e.advance(), t++);
            t && e.acceptToken(T);
          }
        },
        { contextual: !0 },
      ...
      (Y = new c((e, t) => {
        if (d(e)) {
          if ((e.advance(), t.dialectEnabled(A))) {
            let t = -1;
            for (let n = 1; ; n++) {
              let r = e.peek(-n - 1);
              if (r == W || r < 0) {
                t = n + 1;
                break;
              } else if (!j.includes(r)) break;
            }
            if (t > -1) {
              let n = e.next == U,
                r = 0;
              for (e.advance(); e.next >= 0;)
                if (e.next == W) {
                  e.advance();
                  let n = 0;
                  for (; e.next != W && j.includes(e.next);) (n++, e.advance());
                  if (n < t) {
                    r = -n - 1;
                    break;
                  }
                } else if (n && e.next == U && e.peek(1) == H) {
                  r = 2;
                  break;
                } else e.advance();
              e.acceptToken(n ? D : E, r);
              return;
            }
          }
          if (e.next == H) {
            for (; e.next != W && e.next >= 0;) e.advance();
            e.acceptToken(E);
          } else {
            for (e.advance(); e.next >= 0;) {
              let { next: t } = e;
              if ((e.advance(), t == U && e.next == H)) {
                e.advance();
                break;
              }
            }
      ...
      (X = new c((e, t) => {
        (e.next == K || e.next == G) &&
          t.dialectEnabled(A) &&
          e.acceptToken(e.next == G ? O : k, 1);
      })),
      (Z = new c((e, t) => {
        if (!t.dialectEnabled(A)) return;
        ...
        if (e.peek(-1) == W) {
          let t = 0;
          for (; e.next != W && j.includes(e.next);) (e.advance(), t++);
          t != n && e.next != W && !d(e) && (t < n ? e.acceptToken(m, -t) : e.acceptToken(p));
        }
      })),
      (Q = new c((e, t) => {
        for (let n = !1, r = 0, i = 0; ; i++) {
          let { next: a } = e;
          if (l(a) || a == I || a == P || (n && u(a)))
            (!n && (a != I || i > 0) && (n = !0), r === i && a == I && r++, e.advance());
          else if (a == R && e.peek(1) == B) {
            e.acceptToken(ce, 2);
            break;
          } else {
            n && e.acceptToken(r == 2 && t.canShift(x) ? x : t.canShift(S) ? S : a == N ? y : b);
            break;
          }
        }
      })),
      (ue = new c((e) => {
        if (e.next == V) {
          for (e.advance(); l(e.next) || e.next == I || e.next == P || u(e.next);) e.advance();
          e.next == R && e.peek(1) == B ? e.acceptToken(_, 2) : e.acceptToken(g);
        }
      })),
      (de = new c((e) => {
        if (j.includes(e.peek(-1))) {
          let { next: t } = e;
          (l(t) ||
            t == P ||
            t == R ||
      ...
      (fe = new c((e) => {
        if (!j.includes(e.peek(-1))) {
          let { next: t } = e;
          if ((t == z && (e.advance(), e.acceptToken(v)), l(t))) {
            do e.advance();
            while (l(e.next) || u(e.next));
            e.acceptToken(v);
          }
        }
      })),
      (pe = new ae({
        start: new f(null, 0),
        shift(e, t, n, r) {
          return t == p ? new f(e, n.pos - r.pos) : t == m ? e.parent : e;
        },
        hash(e) {
          return e.hash;
        },
      ...
      (me = t({
        "AtKeyword import charset namespace keyframes media supports include mixin use forward extend at-root":
          r.definitionKeyword,
        "Keyword selector": r.keyword,
        ControlKeyword: r.controlKeyword,
        NamespaceName: r.namespace,
        KeyframeName: r.labelName,
        KeyframeRangeName: r.operatorKeyword,
        TagName: r.tagName,
        "ClassName Suffix": r.className,
      ...
      (ve = { __proto__: null, layer: 166, not: 184, only: 184, selector: 190 }),
      (ye = ie.deserialize({
        version: 14,
        states:
        ...
        tokenizers: [Z, de, ue, fe, Q, J, Y, X, 0, 1, 2, 3, 4],
        topRules: { StyleSheet: [0, 10], Styles: [1, 145] },
        dialects: { indented: 0 },
        specialized: [
          { term: 172, get: (e) => he[e] || -1 },
          { term: 171, get: (e) => ge[e] || -1 },
          { term: 80, get: (e) => _e[e] || -1 },
          { term: 173, get: (e) => ve[e] || -1 },
        ],
        tokenPrec: 3217,
      })));
  });
function xe(e) {
  return new n(e?.indented ? Se : $, $.data.of({ autocomplete: Ce }));
}
var $, Se, Ce;
e(() => {
  (be(),
      ...
      parser: ye.configure({
        props: [
          o.add({
            Block: te,
            Comment(e, t) {
              return { from: e.from + 2, to: t.sliceDoc(e.to - 2, e.to) == `*/` ? e.to - 2 : e.to };
            },
          }),
          s.add({ Declaration: a() }),
        ],
    ...
    (Se = $.configure({
      dialect: `indented`,
      props: [
        s.add({ "Block RuleSet": (e) => e.baseIndent + e.unit }),
        o.add({ Block: (e) => ({ from: e.from, to: e.to }) }),
      ],
    })),
    (Ce = se((e) => e.name == `VariableName` || e.name == `SassVariableName`)));
})();
export { xe as sass };
//# sourceMappingURL=dist-Ban9NZEF.js.map
