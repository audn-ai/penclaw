...
function t(e) {
  ...
  function h(e, o) {
    ...
    if (
      (l.hexNumber &&
        ((s == `0` && e.match(/^[xX][0-9a-fA-F]+/)) ||
          ((s == `x` || s == `X`) && e.match(/^'[0-9a-fA-F]*'/)))) ||
      (l.binaryNumber &&
        (((s == `b` || s == `B`) && e.match(/^'[01]+'/)) || (s == `0` && e.match(/^b[01]*/))))
    )
      return `number`;
    if (s.charCodeAt(0) > 47 && s.charCodeAt(0) < 58)
      return (
        e.match(/^[0-9]*(\.[0-9]+)?([eE][-+]?[0-9]+)?/),
        l.decimallessFloat && e.match(/^\.(?!\.)/),
        `number`
      );
    if (s == `?` && (e.eatSpace() || e.eol() || e.eat(`;`))) return `macroName`;
    if (s == `'` || (s == `"` && l.doubleQuote)) return ((o.tokenize = g(s)), o.tokenize(e, o));
    if (
      ((l.nCharCast && (s == `n` || s == `N`)) ||
        (l.charsetCast && s == `_` && e.match(/[a-z][a-z0-9]*/i))) &&
      (e.peek() == `'` || e.peek() == `"`)
    )
      return `keyword`;
    if (
    ...
    if (
      (l.commentSlashSlash && s == `/` && e.eat(`/`)) ||
      (l.commentHash && s == `#`) ||
      (s == `-` && e.eat(`-`) && (!l.commentSpaceRequired || e.eat(` `)))
    )
      return (e.skipToEnd(), `comment`);
    if (s == `/` && e.eat(`*`)) return ((o.tokenize = _(1)), o.tokenize(e, o));
    if (s == `.`) {
      if (l.zerolessFloat && e.match(/^(?:\d+(?:e[+-]?\d+)?)/i)) return `number`;
      if (e.match(/^\.+/)) return null;
      if (l.ODBCdotTable && e.match(/^[\w\d_$#]+/)) return `type`;
    } else if (a.test(s)) return (e.eatWhile(a), `operator`);
    else if (p.test(s)) return `bracket`;
    else if (m.test(s)) return (e.eatWhile(m), `punctuation`);
    else if (
      s == `{` &&
      ...
      return d.hasOwnProperty(f) && (e.match(/^( )+'[^']*'/) || e.match(/^( )+"[^"]*"/))
        ? `number`
        : n.hasOwnProperty(f)
          ? `atom`
          : r.hasOwnProperty(f)
  ...
  return {
    ...
    token: function (e, t) {
      if (
        (e.sol() && t.context && t.context.align == null && (t.context.align = !1),
        t.tokenize == h && e.eatSpace())
      )
        return null;
      var n = t.tokenize(e, t);
      if (n == `comment`) return n;
      t.context && t.context.align == null && (t.context.align = !0);
      var r = e.current();
      return (
        r == `(`
          ? v(e, t, `)`)
          : r == `[`
            ? v(e, t, `]`)
            : t.context && t.context.type == r && y(t),
        n
      );
    },
    indent: function (e, t, n) {
      var r = e.context;
      if (!r) return null;
      var i = t.charAt(0) == r.type;
      return r.align ? r.col + +!i : r.indent + (i ? 0 : n.unit);
    },
    languageData: {
      commentTokens: {
        line: l.commentSlashSlash ? `//` : l.commentHash ? `#` : `--`,
        block: { open: `/*`, close: `*/` },
      },
...
function n(e) {
  for (var t; (t = e.next()) != null;) if (t == "`" && !e.eat("`")) return `string.special`;
...
var s, c, l;
e(() => {
  ((s = `alter and as asc between by count create delete desc distinct drop from group having in insert into is join like not on or order select set table union update values where limit `),
    (c = `bool boolean bit blob enum long longblob longtext medium mediumblob mediumint mediumtext time timestamp tinyblob tinyint tinytext text bigint int int1 int2 int3 int4 int8 integer float float4 float8 double char varbinary varchar varcharacter precision real date datetime year unsigned signed decimal numeric`),
    t({
      keywords: o(s + `begin`),
      builtin: o(c),
...
export { l as esper };
//# sourceMappingURL=sql-Bi4SE94B.js.map
