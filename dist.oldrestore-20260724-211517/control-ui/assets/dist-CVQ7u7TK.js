import { a as p, i as le, n as ue, r as de } from "./dist-D9GsM5qA.js";
import { a as ae, i as f, n as oe, r as se, t as ce } from "./dist-DcfhWyQe.js";
import {
  A as t,
  E as n,
  a as r,
    ...
    (ae(),
      ...
      (F = new oe(
        (e, t) => {
          for (
            let n = 0, r = e.next;
            ((t.context && (r < 0 || r == O || r == k || (r == M && e.peek(n + 1) == M))) ||
              r == N ||
              r == P) &&
              e.acceptToken(m),
              !(r != A && r != j);
      ...
      (I = new Set([C, g, b, _, y, T, E, w, D, v, S, x])),
      (L = new ce({ start: !1, shift: (e, t) => (t == h ? e : I.has(t)) })),
      (R = t({
        "func interface struct chan map const type var": c.definitionKeyword,
        "import package": c.moduleKeyword,
        ...
        DefName: c.definition(c.variableName),
        TypeName: c.typeName,
        LabelName: c.labelName,
        FieldName: c.propertyName,
        "FunctionDecl/DefName": c.function(c.definition(c.variableName)),
        "TypeSpec/DefName": c.definition(c.typeName),
        "CallExpr/VariableName": c.function(c.variableName),
        ...
        "UpdateOp IncDecOp": c.updateOperator,
      ...
      (B = se.deserialize({
        ...
        tokenizers: [F, 1, 2, new f(`j~RQYZXz{^~^O$|~~aP!P!Qd~iO$}~~`, 25, 181)],
        topRules: { SourceFile: [0, 3] },
        dynamicPrecedences: { 19: 1, 51: -1, 55: 2, 69: -1, 108: -1 },
        specialized: [{ term: 184, get: (e) => z[e] || -1 }],
        tokenPrec: 5451,
      })));
  });
function H(e, t) {
  return (n, r) => {
    outer: for (let i = n.node.firstChild, a = 0, o = null; ;) {
      for (; !i;) {
        if (!a) break outer;
        (a--, (i = o.nextSibling), (o = o.parent));
      }
      (t && i.name == t) || i.name == `SpecList`
        ? (a++, (o = i), (i = i.firstChild))
        : (i.name == `DefName` && r(i, e), (i = i.nextSibling));
    }
    return !0;
  };
}
  ...
  return (
    t.cursor(i.IncludeAnonymous).iterate((t) => {
      if (a) a = !1;
        ...
        let e = q[t.name];
        if ((e && e(t, o)) || K.has(t.name)) return !1;
      } else if (t.to - t.from > 8192) {
        for (let n of U(e, t.node)) r.push(n);
        return !1;
      }
    }),
    G.set(t, r),
...
function fe() {
  let e = W.concat($);
  return new r(Z, [Z.data.of({ autocomplete: de(Y, ue(e)) }), Z.data.of({ autocomplete: X })]);
}
...
e(() => {
  (V(),
    ...
    s(),
    (W = [
      p("func ${name}(${params}) ${type} {\n	${}\n}", {
        label: `func`,
        detail: `declaration`,
        type: `keyword`,
      }),
      p("func (${receiver}) ${name}(${params}) ${type} {\n	${}\n}", {
        label: `func`,
        detail: `method declaration`,
        type: `keyword`,
      }),
      p("var ${name} = ${value}", { label: `var`, detail: `declaration`, type: `keyword` }),
      p("type ${name} ${type}", { label: `type`, detail: `declaration`, type: `keyword` }),
      p("const ${name} = ${value}", { label: `const`, detail: `declaration`, type: `keyword` }),
      p("type ${name} = ${type}", { label: `type`, detail: `alias declaration`, type: `keyword` }),
      p("for ${init}; ${test}; ${update} {\n	${}\n}", {
        label: `for`,
        detail: `loop`,
        type: `keyword`,
      }),
    ...
    (q = {
      FunctionDecl: H(`function`),
      VarDecl: H(`var`, `VarSpec`),
      ConstDecl: H(`constant`, `ConstSpec`),
      TypeDecl: H(`type`, `TypeSpec`),
      ImportDecl: H(`constant`, `ImportSpec`),
      Parameter: H(`var`),
      __proto__: null,
    }),
    (J = /^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/),
    (Y = [`String`, `LineComment`, `BlockComment`, `DefName`, `LabelName`, `FieldName`, `.`, `?.`]),
    (X = (e) => {
      let t = n(e.state).resolveInner(e.pos, -1);
      if (Y.indexOf(t.name) > -1) return null;
      let r =
        t.name == `VariableName` || (t.to - t.from < 20 && J.test(e.state.sliceDoc(t.from, t.to)));
      if (!r && !e.explicit) return null;
      let i = [];
      for (let n = t; n; n = n.parent) K.has(n.name) && (i = i.concat(U(e.state.doc, n)));
      return { options: i, from: r ? t.from : e.pos, validFor: J };
    }),
    (Z = te.define({
      name: `go`,
      parser: B.configure({
        props: [
          re.add({
            ...
            LabeledStatement: o,
            "SwitchBlock SelectBlock": (e) => {
              let t = e.textAfter,
                n = /^\s*\}/.test(t),
                r = /^\s*(case|default)\b/.test(t);
              return e.baseIndent + (n || r ? 0 : e.unit);
            },
            Block: a({ closing: `}` }),
            BlockComment: () => null,
            Statement: u({ except: /^{/ }),
          ...
          d.add({
            "Block SwitchBlock SelectBlock LiteralValue InterfaceType StructType SpecList": ne,
            BlockComment(e) {
...
export { fe as go };
//# sourceMappingURL=dist-CVQ7u7TK.js.map
