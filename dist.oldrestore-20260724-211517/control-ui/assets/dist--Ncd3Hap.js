import { a as se, i as ce, n as l, r as le } from "./dist-DcfhWyQe.js";
import {
  A as n,
  E as r,
  a as i,
...
function f(e) {
  return d(e) || (e >= 97 && e <= 102) || (e >= 65 && e <= 70);
}
var p,
  m,
  h,
    ...
    (se(),
      ...
      (M = (e, t, n) => (r, i) => {
        for (let a = !1, o = 0, s = 0; ; s++) {
          let { next: c } = r;
          if (u(c) || c == w || c == S || (a && d(c)))
            (!a && (c != w || s > 0) && (a = !0), o === s && c == w && o++, r.advance());
          else if (c == k && r.peek(1) != A) {
            if ((r.advance(), f(r.next))) {
              do r.advance();
              while (f(r.next));
              r.next == 32 && r.advance();
            ...
            a && r.acceptToken(o == 2 && i.canShift(g) ? t : c == x ? n : e);
            break;
          }
        }
      }),
      (N = new l(M(h, g, ue), { contextual: !0 })),
      (P = new l(M(de, _, v), { contextual: !0 })),
      (F = new l((e) => {
      ...
      (fe = new l((e) => {
        if (!y.includes(e.peek(-1))) {
          let { next: t } = e;
          if ((t == D && (e.advance(), e.acceptToken(m)), u(t))) {
            do e.advance();
            while (u(e.next) || d(e.next));
            e.acceptToken(m);
          }
        }
      })),
      (I = n({
        "AtKeyword import charset namespace keyframes media supports font-feature-values":
          s.definitionKeyword,
        "from to selector scope MatchFlag": s.keyword,
        ...
        "CallTag ValueName FontName": s.atom,
      ...
      (R = { __proto__: null, or: 104, and: 104, not: 112, only: 112, layer: 206 }),
      (z = { __proto__: null, selector: 118, style: 124, layer: 202 }),
      (B = {
        __proto__: null,
        "@import": 198,
        "@media": 210,
      ...
      (H = le.deserialize({
        ...
        tokenizers: [
          ...
          3,
          4,
          new ce("m~RRYZ[z{a~~g~aO$_~~dP!P!Qg~lO$`~~", 28, 152),
        ],
        topRules: { StyleSheet: [0, 6], Styles: [1, 126] },
        dynamicPrecedences: { 94: 1 },
        specialized: [
          { term: 147, get: (e) => L[e] || -1 },
          { term: 148, get: (e) => R[e] || -1 },
          { term: 4, get: (e) => z[e] || -1 },
          { term: 28, get: (e) => B[e] || -1 },
          { term: 146, get: (e) => V[e] || -1 },
        ],
        tokenPrec: 2405,
      })));
  }),
  me = t({
    css: () => G,
    cssCompletionSource: () => Q,
    cssLanguage: () => $,
    defineCSSCompletionSource: () => Z,
  });
function U() {
  if (!K && typeof document == `object` && document.body) {
    ...
    K = t.sort().map((e) => ({ type: `property`, label: e, apply: e + `: ` }));
  }
  return K || [];
}
function he(e, t) {
  ...
  return n?.name == `Callee` ? t.sliceString(n.from, n.to) == `var` : !1;
}
function ge(e) {
  for (let t = e; ;) {
...
function W(e, t, n) {
  if (t.to - t.from > 4096) {
    ...
    let i = [],
      o = new Set(),
      s = t.cursor(a.IncludeAnonymous);
    if (s.firstChild())
      do for (let t of W(e, s.node, n)) o.has(t.label) || (o.add(t.label), i.push(t));
      while (s.nextSibling());
    return (X.set(t, i), i);
  } else {
    let r = [],
    ...
    return (
      t.cursor().iterate((t) => {
        if (n(t) && t.matchContext(be) && t.node.nextSibling?.name == `:`) {
          let n = e.sliceString(t.from, t.to);
          i.has(n) || (i.add(n), r.push({ label: n, type: `variable` }));
        }
      }),
      r
    );
...
function G() {
  return new i($, $.data.of({ autocomplete: Q }));
}
var K,
  q,
  J,
  ...
  xe = e(() => {
    (pe(),
      ...
      (K = null),
      (q =
        `active.after.any-link.autofill.backdrop.before.checked.cue.default.defined.disabled.empty.enabled.file-selector-button.first.first-child.first-letter.first-line.first-of-type.focus.focus-visible.focus-within.fullscreen.has.host.host-context.hover.in-range.indeterminate.invalid.is.lang.last-child.last-of-type.left.link.marker.modal.not.nth-child.nth-last-child.nth-last-of-type.nth-of-type.only-child.only-of-type.optional.out-of-range.part.placeholder.placeholder-shown.read-only.read-write.required.right.root.scope.selection.slotted.target.target-text.valid.visited.where`
          .split(`.`)
      ...
      (_e =
        `a.abbr.address.article.aside.b.bdi.bdo.blockquote.body.br.button.canvas.caption.cite.code.col.colgroup.dd.del.details.dfn.dialog.div.dl.dt.em.figcaption.figure.footer.form.header.hgroup.h1.h2.h3.h4.h5.h6.hr.html.i.iframe.img.input.ins.kbd.label.legend.li.main.meter.nav.ol.output.p.pre.ruby.section.select.small.source.span.strong.sub.summary.sup.table.tbody.td.template.textarea.tfoot.th.thead.tr.u.ul`
          .split(`.`)
      ...
      (Z = (e) => (t) => {
        let { state: n, pos: i } = t,
          a = r(n).resolveInner(i, -1),
          o = a.type.isError && a.from == a.to - 1 && n.doc.sliceString(a.from, a.to) == `-`;
        if (
          a.name == `PropertyName` ||
          ((o || a.name == `TagName`) && /^(Block|Styles)$/.test(a.resolve(a.to).name))
        )
          return { from: a.from, options: U(), validFor: Y };
        if (a.name == `ValueName`) return { from: a.from, options: J, validFor: Y };
        if (a.name == `PseudoClassName`) return { from: a.from, options: q, validFor: Y };
        if (e(a) || ((t.explicit || o) && he(a, n.doc)))
          return { from: e(a) || o ? a.from : i, options: W(n.doc, ge(a), e), validFor: ye };
        if (a.name == `TagName`) {
          for (let { parent: e } = a; e; e = e.parent)
            if (e.name == `Block`) return { from: a.from, options: U(), validFor: Y };
          return { from: a.from, options: _e, validFor: Y };
        }
        if (a.name == `AtKeyword`) return { from: a.from, options: ve, validFor: Y };
        if (!t.explicit) return null;
        let s = a.resolve(i),
          c = s.childBefore(i);
        return c && c.name == `:` && s.name == `PseudoClassSelector`
          ? { from: i, options: q, validFor: Y }
          : (c && c.name == `:` && s.name == `Declaration`) || s.name == `ArgList`
            ? { from: i, options: J, validFor: Y }
            : s.name == `Block` || s.name == `Styles`
              ? { from: i, options: U(), validFor: Y }
              : null;
      }),
      (Q = Z((e) => e.name == `VariableName`)),
      ($ = re.define({
        name: `css`,
        parser: H.configure({
          props: [ae.add({ Declaration: ee() }), ne.add({ "Block KeyframeList": ie })],
        }),
        languageData: {
...
export { xe as a, me as i, $ as n, Z as r, G as t };
//# sourceMappingURL=dist--Ncd3Hap.js.map
