...
function r(e, t) {
  ...
  var u = e.next();
  if (/[\[\]{}\(\),\.]/.test(u))
    return (u == `(` && r ? (t.inParams = !0) : u == `)` && (t.inParams = !1), null);
  if (/\d/.test(u)) return (e.eatWhile(/[\w\.]/), `number`);
  if (u == `\\`) return (e.eat(`\\`), e.eat(/./), `number`);
  if (u == `/` && e.eat(`*`)) return n(e, t, i);
  if (u == `;` && e.match(/ *\( *\(/)) return n(e, t, a);
  if (u == `;` && !t.inParams) return (e.skipToEnd(), `comment`);
  if (u == `"`) return (e.eat(/"/), `keyword`);
  if (u == `$`)
    return (
      e.eatWhile(/[$_a-z0-9A-Z\.:]/),
      o && o.propertyIsEnumerable(e.current().toLowerCase())
        ? `keyword`
        : ((t.beforeParams = !0), `builtin`)
    );
  if (u == `%`) return (e.eatWhile(/[^,\s()]/), (t.beforeParams = !0), `string`);
  if (l.test(u)) return (e.eatWhile(l), `operator`);
  e.eatWhile(/[\w\$_{}]/);
  var d = e.current().toLowerCase();
  return s && s.propertyIsEnumerable(d)
    ? `keyword`
    : c && c.propertyIsEnumerable(d)
...
e(() => {
  ((o = t(
    ...
    (c = t(`if elseif else and not or eq ne in ni for foreach while switch`)),
    (l = /[+\-*&%=<>!?^\/\|]/),
    (u = {
      name: `mirc`,
      startState: function () {
        return { tokenize: r, beforeParams: !1, inParams: !1 };
      },
      token: function (e, t) {
...
export { u as mirc };
//# sourceMappingURL=mirc-JpXmO_jl.js.map
