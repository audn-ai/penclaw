import { html as x, t as S } from "./dist-D_xBiePP.js";
import { a as _, i as v, n as y, r as b } from "./dist-DcfhWyQe.js";
import {
  A as t,
  E as n,
  L as r,
...
function E(e) {
  ...
  if (e.explicit && (a == `FilterOp` || a == `filter`)) return { type: `filter` };
  ...
  if (i.name == `PropertyName` && i.parent.name == `MemberExpression`)
    return { type: `prop`, node: i, target: i.parent };
  if (i.name == `.` && i.parent.name == `MemberExpression`)
    return { type: `prop`, target: i.parent };
  if (i.name == `MemberExpression` && a == `.`) return { type: `prop`, target: i };
  if (i.name == `VariableName`) return { type: `expr`, from: i.from };
  if (i.name == `Comment` || i.name == `StringLiteral` || i.name == `NumberLiteral`) return null;
  let o = e.matchBefore(/[\w\u00c0-\uffff]+$/);
  return o ? { type: `expr`, from: o.from } : e.explicit ? { type: `expr` } : null;
}
function D(e, t, n, r) {
  let i = [];
  for (;;) {
    ...
    if (n.name == `VariableName`) {
      i.unshift(e.sliceDoc(n.from, n.to));
      break;
...
function O(e = {}) {
  let t = e.tags ? e.tags.concat(q) : q,
    n = e.variables ? e.variables.concat(K) : K,
    { properties: r } = e;
  return (e) => {
    let i = E(e);
    if (!i) return null;
    let a = i.from ?? (i.node ? i.node.from : e.pos),
      o;
    return (
      (o =
        i.type == `filter`
          ...
          : i.type == `tag`
            ...
            : i.type == `expr`
              ? n
              : r
                ? D(e.state, i.target, e, r)
                : []),
      o.length ? { options: o, from: a, validFor: /^[\w\u00c0-\uffff]*$/ } : null
    );
  };
}
function k(e) {
...
function j(e = {}) {
  let t = e.base || X,
    n = t.language == X.language ? Z : A(t.language);
  return new o(n, [
    t.support,
    n.data.of({ autocomplete: O(e) }),
...
e(() => {
  (g(),
    ...
    (H = b.deserialize({
      ...
      tokenizers: [R, z, 1, 2, 3, 4, 5, new v(`b~RPstU~XP#q#r[~aO$Q~~`, 17, 173)],
      topRules: { Template: [0, 5] },
      specialized: [
        { term: 161, get: (e) => B[e] || -1 },
        { term: 55, get: (e) => V[e] || -1 },
      ],
      tokenPrec: 3602,
    })),
        ...
        props: [
          t({
            "TagName raw endraw filter endfilter as trans pluralize endtrans with endwith autoescape endautoescape":
              l.keyword,
            "required scoped recursive with without context ignore missing": l.modifier,
            self: l.self,
            "loop super": l.standard(l.variableName),
            "if elif else endif for endfor call endcall": l.controlKeyword,
            "block endblock set endset macro endmacro import from include": l.definitionKeyword,
            "Comment/...": l.blockComment,
            VariableName: l.variableName,
            Definition: l.definition(l.variableName),
            PropertyName: l.propertyName,
            FilterName: l.special(l.variableName),
            ArithOp: l.arithmeticOperator,
            AssignOp: l.definitionOperator,
            "not and or": l.logicOperator,
            CompareOp: l.compareOperator,
            "in is": l.operatorKeyword,
            "FilterOp ConcatOp": l.operator,
          ...
          h.add({
            Tag: s({ closing: `%}` }),
            "IfStatement ForStatement": k(/^\s*(\{%-?\s*)?(endif|endfor|else|elif)\b/),
            Statement: k(/^\s*(\{%-?\s*)?end\w/),
          }),
          d.add({
            "Statement Comment"(e) {
              let t = e.firstChild,
                n = e.lastChild;
              return !t || (t.name != `Tag` && t.name != `{#`)
                ? null
                : { from: t.to, to: n.name == `EndTag` || n.name == `#}` ? n.from : e.to };
            },
          }),
        ],
      }),
...
export { j as jinja };
//# sourceMappingURL=dist-y_2sg6Uf.js.map
