...
function o(e) {
  var o = e.statementIndentUnit,
    ...
    _ = e.indentStatements !== !1,
    v = e.indentSwitch !== !1,
    y = e.namespaceSeparator,
    b = e.isPunctuationChar || /[\[\]{}\(\),;\:\.]/,
    x = e.numberStart || /[\d\.]/,
    S = e.number || /^(?:0x[a-f\d]+|0b[01]+|(?:\d+\.?\d*|\.\d+)(?:e[-+]?\d+)?)(u|ll?|l|f)?/i,
    C = e.isOperatorChar || /[+\-*&%=<>!?|\/]/,
    w = e.isIdentifierChar || /[\w\$_\xa1-\uffff]/,
    T = e.isReservedIdentifier || !1,
    E,
    D;
  function O(e, t) {
    var n = e.next();
    ...
    if (n == `"` || n == `'`) return ((t.tokenize = k(n)), t.tokenize(e, t));
    if (x.test(n)) {
      if ((e.backUp(1), e.match(S))) return `number`;
      e.next();
    }
    if (b.test(n)) return ((E = n), null);
    if (n == `/`) {
    ...
    if ((e.eatWhile(w), y)) for (; e.match(y);) e.eatWhile(w);
    var i = e.current();
    return c(l, i)
      ? (c(f, i) && (E = `newstatement`), c(p, i) && (D = !0), `keyword`)
      : c(u, i)
        ? `type`
        : c(d, i) || (T && T(i))
          ? (c(f, i) && (E = `newstatement`), `builtin`)
          : c(m, i)
            ? `atom`
            : `variable`;
  }
  function k(e) {
    return function (t, n) {
      ...
      return ((a || !(r || g)) && (n.tokenize = null), `string`);
    };
  }
  function A(e, t) {
  ...
  function j(t, n) {
    e.typeFirstDefinitions && t.eol() && a(n.context) && (n.typeAtEndOfLine = i(t, n, t.pos));
  }
  return {
    name: e.name,
    ...
    token: function (t, o) {
      var s = o.context;
      if (
        (t.sol() && ((s.align ??= !1), (o.indented = t.indentation()), (o.startOfLine = !0)),
        t.eatSpace())
      )
        return (j(t, o), null);
      E = D = null;
      var c = (o.tokenize || O)(t, o);
      if (c == `comment` || c == `meta`) return c;
      if (((s.align ??= !0), E == `;` || E == `:` || (E == `,` && t.match(/^\s*(?:\/\/.*)?$/, !1))))
        for (; o.context.type == `statement`;) r(o);
      else if (E == `{`) n(o, t.column(), `}`);
      else if (E == `[`) n(o, t.column(), `]`);
      else if (E == `(`) n(o, t.column(), `)`);
      else if (E == `}`) {
        for (; s.type == `statement`;) s = r(o);
        for (s.type == `}` && (s = r(o)); s.type == `statement`;) s = r(o);
      } else
        E == s.type
          ? r(o)
          : _ &&
            (((s.type == `}` || s.type == `top`) && E != `;`) ||
              (s.type == `statement` && E == `newstatement`)) &&
            n(o, t.column(), `statement`, t.current());
      if (
        (c == `variable` &&
          (o.prevToken == `def` ||
      ...
      return (
        c == `def` && e.styleDefs === !1 && (c = `variable`),
        (o.startOfLine = !1),
        (o.prevToken = D ? `def` : c || E),
        j(t, o),
        c
      );
    },
    indent: function (t, n, r) {
      if ((t.tokenize != O && t.tokenize != null) || (t.typeAtEndOfLine && a(t.context)))
        return null;
      var i = t.context,
        c = n && n.charAt(0),
        l = c == i.type;
      if ((i.type == `statement` && c == `}` && (i = i.prev), e.dontIndentStatements))
        for (; i.type == `statement` && e.dontIndentStatements.test(i.info);) i = i.prev;
      if (h.indent) {
        var u = h.indent(t, i, n, r.unit);
        if (typeof u == `number`) return u;
      }
      var d = i.prev && i.prev.info == `switch`;
      if (e.allmanIndentation && /[{(]/.test(c)) {
        for (; i.type != `top` && i.type != `}`;) i = i.prev;
        return i.indented;
      }
      return i.type == `statement`
        ? i.indented + (c == `{` ? 0 : o || r.unit)
        : i.align && (!s || i.type != `)`)
          ? i.column + +!l
          : i.type == `)` && !l
            ? i.indented + (o || r.unit)
            : i.indented +
              (l ? 0 : r.unit) +
              (!l && d && !/^(?:case|default)\b/.test(n) ? r.unit : 0);
    },
    languageData: {
...
function h(e, t) {
  if ((e.backUp(1), e.match(/^(?:R|u8R|uR|UR|LR)/))) {
    ...
    return n ? ((t.cpp11RawStringDelim = n[1]), (t.tokenize = v), v(e, t)) : !1;
  }
    ...
    for (var r; (r = t.next());)
      if (r == `*` && t.eat(`/`))
        if (e == 1) {
          n.tokenize = null;
          break;
        } else return ((n.tokenize = b(e - 1)), n.tokenize(t, n));
      else if (r == `/` && t.eat(`*`)) return ((n.tokenize = b(e + 1)), n.tokenize(t, n));
    return `comment`;
  };
}
function x(e) {
...
function C(e) {
  (e.interpolationStack ||= []).push(e.tokenize);
}
function w(e) {
  return (e.interpolationStack ||= []).pop();
}
function T(e) {
...
function E(e, t, n, r) {
  ...
  function a(t, n) {
    for (var a = !1; !t.eol();) {
      if (!r && !a && t.peek() == `$`) return (C(n), (n.tokenize = D), `string`);
      var o = t.next();
      if (o == e && !a && (!i || t.match(e + e))) {
        n.tokenize = null;
        break;
      }
      a = !r && !a && o == `\\`;
...
function D(e, t) {
  return (e.eat(`$`), e.eat(`{`) ? (t.tokenize = null) : (t.tokenize = O), null);
}
function O(e, t) {
  return (e.eatWhile(/[\w_]/), (t.tokenize = w(t)), `variable`);
}
var k, A, j, M, N, P, F, I, L, R, z, B, V, H, U, W;
e(() => {
  ((k = `auto if break case register continue return default do sizeof static else struct switch extern typedef union for goto while enum const volatile inline restrict asm fortran`),
    (A = `alignas alignof and and_eq audit axiom bitand bitor catch class compl concept constexpr const_cast decltype delete dynamic_cast explicit export final friend import module mutable namespace new noexcept not not_eq operator or or_eq override private protected public reinterpret_cast requires static_assert static_cast template this thread_local throw try typeid typename using virtual xor xor_eq`),
    (j = `bycopy byref in inout oneway out self super atomic nonatomic retain copy readwrite readonly strong weak assign typeof nullable nonnull null_resettable _cmd @interface @implementation @end @protocol @encode @property @synthesize @dynamic @class @public @package @private @protected @required @optional @try @catch @finally @import @selector @encode @defs @synchronized @autoreleasepool @compatibility_alias @available`),
    (M = `FOUNDATION_EXPORT FOUNDATION_EXTERN NS_INLINE NS_FORMAT_FUNCTION  NS_RETURNS_RETAINEDNS_ERROR_ENUM NS_RETURNS_NOT_RETAINED NS_RETURNS_INNER_POINTER NS_DESIGNATED_INITIALIZER NS_ENUM NS_OPTIONS NS_REQUIRES_NIL_TERMINATION NS_ASSUME_NONNULL_BEGIN NS_ASSUME_NONNULL_END NS_SWIFT_NAME NS_REFINED_FOR_SWIFT`),
    (N = s(`int long char short double float unsigned signed void bool`)),
    (P = s(`SEL instancetype id Class Protocol BOOL`)),
    (F = `case do else for if switch while struct enum union`),
    (I = `struct enum union`),
    o({
      name: `c`,
      keywords: s(k),
    ...
    o({
      ...
      keywords: s(k + ` ` + A),
      types: l,
      blockKeywords: s(F + ` class try catch`),
      defKeywords: s(I + ` class namespace`),
      typeFirstDefinitions: !0,
    ...
    o({
      ...
      hooks: {
        ...
        '"': function (e, t) {
          return e.match(/""$/) ? ((t.tokenize = y), t.tokenize(e, t)) : !1;
        },
      },
    }),
    (L = o({
    ...
    (R = o({
      ...
      hooks: {
        ...
        "/": function (e, t) {
          return e.eat(`*`) ? ((t.tokenize = b(1)), t.tokenize(e, t)) : !1;
        },
      },
    ...
    (B = o({
      ...
      types: u,
      builtin: s(M),
      blockKeywords: s(F + ` @synthesize @try @catch @finally @autoreleasepool @synchronized`),
      defKeywords: s(I + ` @interface @implementation @protocol @class`),
    ...
    (V = o({
      ...
      builtin: s(M),
      blockKeywords: s(
        F + ` @synthesize @try @catch @finally @autoreleasepool @synchronized class try catch`,
      ),
      defKeywords: s(I + ` @interface @implementation @protocol @class class namespace`),
      dontIndentStatements: /^@.*$|^template$/,
      typeFirstDefinitions: !0,
      atoms: s(`YES NO NULL Nil nil true false nullptr`),
      isReservedIdentifier: p,
    ...
    o({
      ...
      hooks: {
        ...
        '"': function (e, t) {
          return ((t.tokenize = S(e.match(`""`) ? `triple` : `single`)), t.tokenize(e, t));
        },
        "`": function (e, t) {
          return !U || !e.match("`") ? !1 : ((t.tokenize = U), (U = null), t.tokenize(e, t));
        },
        "'": function (e) {
          return e.match(/^(\\[^'\s]+|[^\\'])'/)
            ? `string.special`
    ...
    (W = o({
      ...
      blockKeywords: s(`try catch finally do else for if switch while`),
      builtin: s(`void bool num int double dynamic var String Null Never`),
      atoms: s(`true false null`),
      ...
      hooks: {
        ...
        r: function (e, t) {
          var n = e.peek();
          return n == `'` || n == `"` ? E(e.next(), e, t, !0) : !1;
        },
        "}": function (e, t) {
          return T(t) > 0 ? ((t.tokenize = w(t)), null) : !1;
        },
        "/": function (e, t) {
          return e.eat(`*`) ? ((t.tokenize = b(1)), t.tokenize(e, t)) : !1;
        },
        token: function (e, t, n) {
          if (n == `variable` && RegExp(`^[_$]*[A-Z][a-zA-Z0-9_$]*$`, `g`).test(e.current()))
            return `type`;
        },
      },
    })));
...
//# sourceMappingURL=clike-BB8l_VVD.js.map
