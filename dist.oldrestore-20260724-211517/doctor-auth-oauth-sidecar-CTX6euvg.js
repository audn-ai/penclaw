...
import { r as resolveAuthStorePath } from "./path-resolve-8wkj2l4Z.js";
import { _ as resolveOAuthDir, y as resolveStateDir } from "./paths-DEklnbzU.js";
import { N as log, m as isLegacyOAuthRef } from "./persisted-B2tK2f7d.js";
import { o as isRecord } from "./record-coerce-DHZ4bFlT.js";
import { r as clearRuntimeAuthProfileStoreSnapshots } from "./store-B7DoDdVM.js";
...
function isLegacyOAuthSidecarPayload(raw) {
  ...
  if (
    ...
    readNonEmptyString$1(raw.profileId) === void 0 ||
    raw.provider !== "openai-codex"
  )
    return false;
  return (
...
function resolveLegacyOAuthSecretKeyFileCandidates(env) {
  if (process.platform === "win32") {
    ...
    return uniquePaths([
      root ? path.join(root, "OpenClaw", LEGACY_OAUTH_SECRET_KEY_FILE_NAME) : void 0,
      home
...
function readLegacyMacOAuthSecretKeychainKey(params) {
  if (
    ...
    params.env.VITEST_WORKER_ID !== void 0
  )
    return;
  try {
...
function emitKeychainOnlyMigrationHintOnce(profileId) {
  ...
  log.warn(
    "Legacy Codex OAuth credentials are stored only in macOS Keychain on this host. Headless paths cannot prompt for Keychain access; run `openclaw doctor --fix` from an interactive terminal to migrate them back to inline auth-profiles.json credentials.",
    { profileId },
  );
}
...
async function maybeRepairLegacyOAuthSidecarProfiles(params) {
  ...
  const unreferencedSidecars = listUnreferencedLegacyOAuthSidecars(
    new Set(stores.flatMap((entry) => entry.profiles.map((p) => p.ref.id))),
    env,
  );
  const result = {
    detected: [
  ...
  for (const store of stores) {
    ...
    for (const profile of store.profiles) {
      ...
      if (
        applyLegacyOAuthSidecarMaterial({
          raw: store.raw,
          profile,
          material,
        })
...
(legacyOAuthSidecarTestUtils.buildLegacyOAuthSecretAad,
  legacyOAuthSidecarTestUtils.buildLegacyOAuthSecretKey);
//#endregion
