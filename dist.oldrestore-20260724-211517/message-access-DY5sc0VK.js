...
function activationMetadata(params) {
  ...
  return {
    ...
    ...(params.activation?.allowedImplicitMentionKinds !== void 0
...
function defineStableChannelIngressIdentity(params = {}) {
  ...
  return {
    ...
    resolveEntryId:
      resolveEntryId ??
      (entryIdPrefix ? ({ entryIndex }) => `${entryIdPrefix}-${entryIndex + 1}` : void 0),
  };
}
...
function createIdentitySubject(identity, input) {
  return {
    identifiers: identityFields(identity).flatMap((field, index) => {
  ...
  return (
    await Promise.all(
      params.subject.identifiers.map(async (identifier, identifierIndex) => {
        return (
          await params.input.adapter.normalizeEntries({
...
async function originSubjectMatched(input) {
  ...
  if (originEntries.length > 0) {
    if (
      (
        await input.adapter.matchSubject({
...
function routeDescriptorForGate(params) {
  const baseGateId = params.gate.id.endsWith(":sender")
    ? params.gate.id.slice(0, -7)
    : params.gate.id;
...
function projectActivationAccess(params) {
  ...
  return {
    ...
    reasonCode: gate?.reasonCode ?? params.ingress.reasonCode,
    ...(gate?.activation?.effectiveWasMentioned !== void 0
      ? { effectiveWasMentioned: gate.activation.effectiveWasMentioned }
      : {}),
    ...(gate?.activation?.shouldBypassMention !== void 0
...
async function resolveChannelMessageIngress(params) {
  ...
  const normalizeEffective = (entries, context) =>
  ...
  const accessGroupMembership = [
    ...(await resolveRuntimeAccessGroupMembershipFacts({
      input: params,
      channelId,
...
//#endregion
export {
  resolveChannelIngressState as a,
  resolveStableChannelMessageIngress as i,
  createChannelIngressResolver as n,
  defineStableChannelIngressIdentity as o,
  resolveChannelMessageIngress as r,
  decideChannelIngress as s,
  channelIngressRoutes as t,
};
