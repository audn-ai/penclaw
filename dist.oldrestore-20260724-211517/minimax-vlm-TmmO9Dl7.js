...
async function readResponseBodySnippet(response, limits) {
  try {
    ...
    return truncateUtf16Safe(
      (await readResponseTextPrefix(response, limits.maxBytes)).text,
      limits.maxChars,
    );
  } catch {
    return "";
...
async function minimaxUnderstandImage(params) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `MiniMax VLM request failed (${res.status} ${res.statusText}).${trace}${body ? ` Body: ${body}` : ""}`,
    );
  }
  const json = await readProviderJsonResponse(
    res,
    traceId ? `MiniMax VLM response [Trace-Id=${traceId}]` : "MiniMax VLM response",
  );
  if (!isRecord(json)) {
    const trace = traceId ? ` Trace-Id: ${traceId}` : "";
