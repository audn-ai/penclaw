...
const GENERIC_CLI_CONTEXT_ENGINE_HOST_CAPABILITIES = ["bootstrap", "after-turn", "maintain"];
...
function assertContextEngineHostSupport(params) {
  ...
  const guidance = evaluation.requirements.unsupportedMessage
    ...
    : "";
  throw new Error(
    `Context engine "${engineId}" cannot run operation "${params.operation}" on ${params.host.label}. Missing host capabilities: ${evaluation.missingCapabilities.join(", ")}. Required capabilities: ${required}. Host capabilities: ${actual}.${guidance}`,
  );
}
//#endregion
export {
  evaluateContextEngineHostSupport as a,
  buildGenericCliContextEngineHostSupport as i,
  OPENCLAW_EMBEDDED_CONTEXT_ENGINE_HOST as n,
  assertContextEngineHostSupport as r,
  CODEX_APP_SERVER_CONTEXT_ENGINE_HOST as t,
};
