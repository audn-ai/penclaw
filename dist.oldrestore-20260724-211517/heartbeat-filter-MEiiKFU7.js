...
import {
  ...
  i as HEARTBEAT_RESPONSE_TOOL_PROMPT,
...
function collectVisibleHeartbeatResponseToolCalls(message) {
  ...
  return [...collectMessageToolCalls(message), ...collectToolCallBlocks(message.content)].filter(
    (block) =>
      readToolCallName(block) === "heartbeat_respond" && isVisibleHeartbeatResponseToolCall(block),
  );
}
...
function isRemovableHeartbeatResponseToolCall(message) {
  ...
  for (const call of collectMessageToolCalls(message))
    if (readToolCallName(call) === "heartbeat_respond" && !isVisibleHeartbeatResponseToolCall(call))
      return true;
...
//#endregion
export {
  isRealNonHeartbeatUserMessage as i,
  isHeartbeatOkResponse as n,
  isHeartbeatUserMessage as r,
  filterHeartbeatTranscriptArtifacts as t,
};
