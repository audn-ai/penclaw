...
import { n as isInstalledPluginEnabled } from "./installed-plugin-index-lBfkb-7L.js";
import { t as planManifestModelCatalogRows } from "./manifest-planner-DSmSdykb.js";
import {
  c as recordGatewayModelPricingSourceFailure,
  i as getCachedGatewayModelPricing,
  l as replaceGatewayModelPricingCache,
  n as clearGatewayModelPricingFailures,
  o as getGatewayModelPricingCacheMeta,
  r as clearGatewayModelPricingSourceFailure,
...
async function readPricingJsonObject(response, source) {
  ...
  const buffer = await readResponseWithLimit(response, MAX_PRICING_CATALOG_BYTES, {
    onOverflow: ({ size }) =>
      /* @__PURE__ */ new Error(`${source} pricing response too large: ${size} bytes`),
  });
  let payload;
  try {
