...
import { t as normalizeMatrixAllowList } from "./allowlist-D7LjwHLa.js";
import {
...
const matrixNetworkSchema = object({ dangerouslyAllowPrivateNetwork: boolean().optional() })
  .strict()
  .optional();
const matrixStreamingSchema = object({
  mode: _enum(["partial", "quiet", "progress", "off"]).optional(),
...
const MatrixChannelConfigSchema = buildChannelConfigSchema(
  object({
    ...
    replyToMode: _enum(["off", "first", "all", "batched"]).optional(),
    threadReplies: _enum(["off", "inbound", "always"]).optional(),
    ...
    autoJoin: _enum(["always", "allowlist", "off"]).optional(),
