...
import {
  c as markCodeModeControlTool,
  n as CODE_MODE_WAIT_TOOL_NAME,
  o as isCodeModeControlTool,
...
import {
  d as addClientToolsToToolCatalog,
  p as applyToolCatalogCompaction,
  u as ToolSearchRuntime,
...
import {
  b as describeCodeModeNamespacesForPrompt,
  v as createCodeModeApiVirtualFiles,
  x as toCodeModeJsonSafe,
  y as createCodeModeNamespaceRuntime,
} from "./registry-B14M_H8G.js";
import { v as uniqueValues } from "./string-normalization-CRyoFBPt.js";
import { t as jsonResult } from "./tool-results-BCM3fdVS.js";
import { o as optionalStringEnum } from "./typebox-D411BFJf.js";
...
function readCodeModeRawConfig(config, agentId) {
  const globalRaw =
    normalizeCodeModeRawConfig((isRecord(config?.tools) ? config.tools : void 0)?.codeMode) ?? {};
  const agentRaw =
    config && agentId
...
function enforceResultLimit(params) {
  ...
  if (params.value !== void 0 && jsonByteLength(params.value) > params.config.maxOutputBytes)
    throw new CodeModeLimitError("output_limit_exceeded", "code mode output limit exceeded");
}
function readCode(args) {
  ...
  if (language !== void 0 && language !== "javascript" && language !== "typescript")
    throw new ToolInputError("language must be javascript or typescript.");
  const restartSafe = params.restartSafe;
  if (restartSafe !== void 0 && typeof restartSafe !== "boolean")
...
async function prepareSource(input) {
  ...
  if (diagnostics.some((diagnostic) => diagnostic.category === ts.DiagnosticCategory.Error))
    throw new ToolInputError(
      `typescript transform failed: ${diagnostics.map((diagnostic) => ts.flattenDiagnosticMessageText(diagnostic.messageText, "\n")).join("\n")}`,
    );
        ...
        if (code !== 0)
          finish(
            failedCodeModeWorkerResult(
              /* @__PURE__ */ new Error(`code mode worker exited with code ${code}`),
              "runtime_unavailable",
            ),
          );
      });
...
function pendingBridgeRequestsReplaySafe(pending, runtime) {
  return pending.every((request) => {
    if (request.method === "search" || request.method === "describe" || request.method === "yield")
      return true;
...
async function settleCodeModeResult(params) {
  ...
  while (
    ...
    try {
      ...
      if (!(await waitForPending(pending, remainingMs)))
        return storeSnapshotState({
          pending,
          replaySafe: false,
          snapshotBytes: result.snapshotBytes,
  ...
  return [
    markCodeModeControlTool({
      ...
      parameters: Type.Object({
        ...
        language: optionalStringEnum(["javascript", "typescript"], {
          description:
...
function applyCodeModeCatalog(params) {
  if (!resolveCodeModeConfig(params.config, params.agentId).enabled)
...
//#endregion
export {
  createCodeModeTools as a,
  applyCodeModeCatalog as i,
  CodeModeHeadlessTimeoutError as n,
  resolveCodeModeConfig as o,
  addClientToolsToCodeModeCatalog as r,
  runCodeModeScriptHeadless as s,
  CodeModeHeadlessAbortError as t,
};
