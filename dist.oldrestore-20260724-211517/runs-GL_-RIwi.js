...
import {
  d as logSessionStateChange,
  s as logMessageQueued,
  w as updateDiagnosticSessionFile,
} from "./diagnostic-DE-wbBb7.js";
import {
  a as getDiagnosticSessionActivitySnapshot,
  o as markDiagnosticEmbeddedRunEnded,
  s as markDiagnosticEmbeddedRunStarted,
  u as resolveRunStaleThresholdMs,
} from "./diagnostic-run-activity-C7EqD7j0.js";
import { t as diagnosticLogger } from "./diagnostic-runtime-BWVVh2ZW.js";
import { j as resolveTimerTimeoutMs } from "./number-coercion-CJQ8TR--.js";
import {
  E as resolveReplyRunPhaseForSessionId,
  T as resolveReplyBackendQueueMessageMismatch,
  ...
  p as isReplyRunAbortableForCompaction,
  u as expireStaleReplyRunBySessionId,
  v as isReplyRunStreamingForSessionId,
  x as queueReplyRunMessage,
  y as listActiveReplyRunSessionIds,
} from "./reply-run-registry-CmZS-dyq.js";
import {
  a as ACTIVE_EMBEDDED_RUNS_BY_RUN_ID,
  c as ACTIVE_EMBEDDED_RUN_SNAPSHOTS,
  d as getActiveEmbeddedRunCount,
  i as ACTIVE_EMBEDDED_RUNS,
  l as EMBEDDED_RUN_WAITERS,
  n as ABANDONED_EMBEDDED_RUN_SESSION_IDS_BY_FILE,
  o as ACTIVE_EMBEDDED_RUN_SESSION_IDS_BY_FILE,
  r as ABANDONED_EMBEDDED_RUN_SESSION_IDS_BY_KEY,
  s as ACTIVE_EMBEDDED_RUN_SESSION_IDS_BY_KEY,
  t as ABANDONED_EMBEDDED_RUNS_BY_SESSION_ID,
  u as RETAINED_EMBEDDED_RUN_ABORTABILITY_RUN_IDS,
...
function prepareEmbeddedAgentQueueMessage(sessionId, text, options) {
  ...
  if (deliveryModeMismatch) {
    diagnosticLogger.debug(
      `queue message failed: sessionId=${sessionId} reason=${deliveryModeMismatch}`,
    );
    return {
      kind: "complete",
...
function abortEmbeddedAgentRun(sessionId, opts) {
  ...
  if (mode === "compacting") {
    ...
    const replyAborted = abortActiveReplyRuns({
      mode,
      onAbortError: (id, err) =>
        diagnosticLogger.warn(`abort failed: sessionId=${id} owner=reply_run err=${String(err)}`),
    });
...
function isEmbeddedAgentRunAbortableForCompaction(sessionId) {
  const active = ACTIVE_EMBEDDED_RUNS.get(sessionId)
    ? true
    : isReplyRunAbortableForCompaction(sessionId);
  if (active)
    diagnosticLogger.debug(`run compact coordination check: sessionId=${sessionId} active=true`);
  return active;
}
...
function clearActiveEmbeddedRun(
  sessionId,
  handle,
  sessionKey,
  sessionFile,
...
//#endregion
export {
  waitForEmbeddedAgentRunEnd as A,
  resolveEmbeddedAgentReplyRunPhase as C,
  updateActiveEmbeddedRunSessionFile as D,
  testing as E,
  updateActiveEmbeddedRunSnapshot as O,
  resolveActiveEmbeddedRunSessionIdBySessionFile as S,
  setActiveEmbeddedRun as T,
  queueEmbeddedAgentMessage as _,
  clearEmbeddedRunAbandonment as a,
  resolveActiveEmbeddedRunHandleSessionId as b,
  getActiveEmbeddedRunSnapshot as c,
  isEmbeddedAgentRunActive as d,
  isEmbeddedAgentRunHandleActive as f,
  markEmbeddedRunAbandoned as g,
  markActiveEmbeddedRunAbandoned as h,
  clearEmbeddedAgentRunAbortabilityForRunId as i,
  resolveEmbeddedSessionFileKey as j,
  waitForActiveEmbeddedRuns as k,
  isEmbeddedAgentRunAbortableForCompaction as l,
  isEmbeddedRunAbandoned as m,
  abortEmbeddedAgentRun as n,
  forceClearEmbeddedAgentRun as o,
  isEmbeddedAgentRunStreaming as p,
  clearActiveEmbeddedRun as r,
  formatEmbeddedAgentQueueFailureSummary as s,
  abortAndDrainEmbeddedAgentRun as t,
  isEmbeddedAgentRunAbortableForRunId as u,
  queueEmbeddedAgentMessageWithOutcome as v,
  retainEmbeddedAgentRunAbortabilityForRunId as w,
  resolveActiveEmbeddedRunHandleSessionIdBySessionFile as x,
  queueEmbeddedAgentMessageWithOutcomeAsync as y,
};
