...
import { s as getSandboxBackendWorkdirResolver } from "./browser-bridges-B_JokzBZ.js";
import { t as formatCliCommand } from "./command-format-H_Arqann.js";
import { i as resolveSandboxConfigForAgent } from "./config-l1rpbjvM.js";
import "./agent-scope-y9xQv_q1.js";
import { r as buildSandboxFsMounts } from "./context-CEFGZ0oc.js";
import { S as resolveSandboxWorkspaceLayoutPaths } from "./docker-yvxsWR8P.js";
...
import { r as resolveSandboxToolPolicyForAgent } from "./tool-policy-fLngst7L.js";
...
async function sandboxExplainCommand(opts, runtime) {
  ...
  const effectiveAgentWorkspaceDir =
  ...
  lines.push(
    `  ${key("backend:")} ${value(payload.sandbox.backend)} ${key("runtimeWorkdir:")} ${value(payload.sandbox.runtimeWorkdir ?? "(direct host)")} ${key("workspaceSource:")} ${value(payload.sandbox.workspaceSource)}`,
  );
  if (payload.sandbox.workspaceMounts.length > 0) {
    ...
    for (const mount of payload.sandbox.workspaceMounts)
      lines.push(
        `    - ${value(mount.hostRoot)} -> ${value(mount.containerRoot)} ${key(mount.writable ? "rw" : "ro")} ${key(`(${mount.source})`)}`,
      );
  }
  lines.push("");
