...
import {
  a as isValidFileSecretRefId,
  i as isValidExecSecretRefId,
  r as formatExecSecretRefIdValidationMessage,
} from "./ref-contract-DVV0zYUk.js";
import {
  At as boolean,
...
const ConfiguredProviderRequestProxySchema = union([
  object({
...
const ModelMediaInputSchema = object({
  image: object({
    ...
    tokenMode: union([literal("tile"), literal("detail"), literal("provider")]).optional(),
  })
    .strict()
    .optional(),
}).strict();
...
const ModelDefinitionSchema = object({
  ...
  input: array(
    union([literal("text"), literal("image"), literal("video"), literal("audio")]),
  ).optional(),
  cost: object({
...
const QueueDropSchema = union([literal("old"), literal("new"), literal("summarize")]);
...
const GroupPolicySchema = _enum(["open", "disabled", "allowlist"]);
const DmPolicySchema = _enum(["pairing", "allowlist", "open", "disabled"]);
const ContextVisibilityModeSchema = _enum(["all", "allowlist", "allowlist_quote"]);
...
const MarkdownConfigSchema = object({
  tables: _enum(["off", "bullets", "code", "block"]).optional(),
})
  .strict()
  .optional();
const TtsProviderSchema = string().min(1);
const TtsModeSchema = _enum(["final", "all"]);
const TtsAutoSchema = _enum(["off", "always", "inbound", "tagged"]);
const TtsProviderConfigSchema = object({
  apiKey: SecretInputSchema.optional().register(sensitive),
}).catchall(
  union([string(), number(), boolean(), _null(), array(unknown()), record(string(), unknown())]),
);
...
const HumanDelaySchema = object({
  mode: union([literal("off"), literal("natural"), literal("custom")]).optional(),
...
const CliBackendWatchdogModeSchema = object({
  ...
  noOutputTimeoutRatio: number().min(0.05).max(0.95).optional(),
...
const CliBackendSchema = object({
  command: string(),
  args: array(string()).optional(),
  output: union([literal("json"), literal("text"), literal("jsonl")]).optional(),
  resumeOutput: union([literal("json"), literal("text"), literal("jsonl")]).optional(),
  ...
  sessionMode: union([literal("always"), literal("existing"), literal("none")]).optional(),
  ...
  systemPromptWhen: union([literal("first"), literal("always"), literal("never")]).optional(),
...
const ExecutableTokenSchema = string().refine(
  isSafeExecutableValue,
  "expected safe executable name or path",
);
const MediaUnderstandingScopeSchema = createAllowDenyChannelRulesSchema();
const MediaUnderstandingCapabilitiesSchema = array(
  union([literal("image"), literal("audio"), literal("video")]),
).optional();
const MediaUnderstandingAttachmentsSchema = object({
  ...
  prefer: union([literal("first"), literal("last"), literal("path"), literal("url")]).optional(),
})
  .strict()
  .optional();
const DeepgramAudioSchema = object({
  ...
  .strict()
  .optional();
const ProviderOptionValueSchema = union([string(), number(), boolean()]);
const ProviderOptionsSchema = record(
  string(),
  record(string(), ProviderOptionValueSchema),
).optional();
...
//#endregion
export {
  TextChunkModeSchema as A,
  evaluateDmPolicyAllowFromDependency as B,
  ReplyRuntimeConfigSchemaShape as C,
  SecretProviderSchema as D,
  SecretInputSchema as E,
  TtsConfigSchema as F,
  requireAllowlistAllowFrom as H,
  TtsModeSchema as I,
  TtsProviderSchema as L,
  ToolsMediaSchema as M,
  TranscribeAudioSchema as N,
  SecretRefSchema as O,
  TtsAutoSchema as P,
  TypingModeSchema as R,
  QueueSchema as S,
  RetryConfigSchema as T,
  requireOpenAllowFrom as U,
  isBuiltInModelProviderOverlayId as V,
  createAllowDenyChannelRulesSchema as W,
  MarkdownConfigSchema as _,
  CliBackendSchema as a,
  NativeCommandsSettingSchema as b,
  DmPolicySchema as c,
  GroupPolicySchema as d,
  HexColorSchema as f,
  MSTeamsReplyStyleSchema as g,
  InboundDebounceSchema as h,
  ChannelStreamingBlockSchema as i,
  ToolsLinksSchema as j,
  SecretsConfigSchema as k,
  ExecutableTokenSchema as l,
  IdentitySchema as m,
  BlockStreamingCoalesceSchema as n,
  ContextVisibilityModeSchema as o,
  HumanDelaySchema as p,
  ChannelDeliveryStreamingConfigSchema as r,
  DmConfigSchema as s,
  BlockStreamingChunkSchema as t,
  GroupChatSchema as u,
  MentionPatternsPolicySchema as v,
  ReplyToModeSchema as w,
  ProviderCommandsSchema as x,
  ModelsConfigSchema as y,
  VisibleRepliesSchema as z,
};
