...
import {
  i as noteOpencodeProviderOverrides,
  n as noteImplicitFallbackClobberWarnings,
  o as stripUnknownConfigKeys,
} from "./doctor-config-analysis-B1_kTc5u.js";
import "./target-registry-Db69vEew.js";
import { t as runDoctorConfigPreflight } from "./doctor-config-preflight-kJdqjbMu.js";
import { t as DEFAULT_GOOGLE_API_BASE_URL } from "./google-api-base-url-UBNiBOzj.js";
import { V as resolveNormalizedProviderModelMaxTokens } from "./io-B3ne6NxF.js";
...
} from "./legacy-config-migrate-C7yKZF7d.js";
import {
  a as legacyRuntimeModelAliasRequiresRuntimePolicy,
  i as migrateLegacyXSearchConfig,
  o as listLegacyRuntimeModelProviderAliases,
  r as migrateLegacyWebSearchConfig,
  s as migrateLegacyRuntimeModelRef,
} from "./legacy-config-migrations-BtAaXFTL.js";
import {
  c as mergeMissing,
  r as isLegacyModelsAddCodexMetadataModel,
} from "./legacy-config-migrations.runtime.models-B1-lVeJ7.js";
import {
  n as ensureRecord,
  r as hasOwnKey,
  t as cloneRecord,
} from "./legacy-config-record-shared-COKcbmOX.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-BIKs-96s.js";
import { t as note } from "./note-Uvqr4R1d.js";
import { i as setPathExistingStrict } from "./path-utils-h0yoktZC.js";
import { t as CONFIG_PATH } from "./paths-DEklnbzU.js";
...
import {
  n as isCommonSingleAccountPromotionKey,
  t as collectSingleAccountPromotionEntries,
} from "./setup-promotion-keys-Oj7oZvpC.js";
import { a as runPluginSetupConfigMigrations } from "./setup-registry-B--wI5Zy.js";
import {
...
import { r as normalizeTalkSection } from "./talk-I2KUqKNv.js";
import { n as discoverConfigSecretTargets } from "./target-registry-query-CH3KJb3I.js";
import {
...
const AUTH_PROFILE_MODES = /* @__PURE__ */ new Set(["api_key", "aws-sdk", "oauth", "token"]);
...
function removeDerivedRealtimeSpeakerVoice(rawTalk, normalizedTalk) {
  ...
  if (
    ...
    !normalizedRealtime ||
    rawRealtime.speakerVoice !== void 0 ||
    normalizedRealtime.speakerVoice === void 0 ||
...
function normalizeLegacyCommandsConfig(cfg, changes) {
  ...
  if (!isRecord(rawCommands) || !("modelsWrite" in rawCommands)) return cfg;
...
function normalizeLegacyBrowserConfig(cfg, changes) {
  ...
  if (isRecord(rawProfiles)) {
    ...
    for (const [profileName, rawProfile] of Object.entries(rawProfiles)) {
      ...
      if (
        (normalizeOptionalString(rawProfile.driver) ?? "") !== "extension" ||
        !normalizeOptionalString(rawProfile.cdpUrl)
      )
        continue;
...
function resolveLegacyWholeAgentRuntimePolicy(raw) {
  ...
  const alias = listLegacyRuntimeModelProviderAliases().find(
    (entry) => entry.cli && normalizeProviderId$1(entry.runtime) === runtime,
  );
  return alias
    ? {
...
function selectedCanonicalModelRefsForRuntimePolicy(
  ...
  if (Array.isArray(rawModel.fallbacks))
    for (const fallback of rawModel.fallbacks) addRef(fallback);
  return refs;
}
...
function normalizeLegacyNanoBananaSkill(cfg, changes) {
  ...
  const legacyEnvApiKey =
    normalizeOptionalString(
      (isRecord(rawLegacyEntry.env) ? rawLegacyEntry.env : void 0)?.GEMINI_API_KEY,
    ) ?? "";
  const legacyApiKey =
    legacyEnvApiKey ||
...
function resolveConfiguredOllamaModelNumCtxBudget(params) {
  ...
  if (providerContextWindow !== void 0)
    return params.providerNumCtxApplies ? void 0 : providerContextWindow;
  ...
  if (providerMaxTokens !== void 0)
    return params.providerNumCtxApplies ? void 0 : providerMaxTokens;
}
...
function applyLegacyOllamaProviderNumCtxParams(params) {
  ...
  if (rawParams !== void 0 && !isRecord(rawParams))
    return {
      provider: params.provider,
      changed: false,
    };
...
const DANGEROUS_RECORD_KEYS = /* @__PURE__ */ new Set(["__proto__", "prototype", "constructor"]);
...
function repairNullAgentWorkspaces(cfg, changes) {
  ...
  changes.push(
    `Removed null workspace value${repaired === 1 ? "" : "s"} from agents.list entr${repaired === 1 ? "y" : "ies"}.`,
  );
  return {
    ...cfg,
    agents: {
...
function collectUnsupportedInternalHookEntryWarnings(cfg) {
  ...
  const unsupportedKeysByEntry = Object.entries(entries)
    ...
    .map(([hookKey, entry]) => {
      return {
        hookKey,
        unsupportedKeys: ["handler", "module", "extraDirs", "installs"].filter((key) =>
...
function emitDoctorChangesPanel(changeLines, shouldRepair, options = {}) {
  ...
  note(
    options.sanitize ? sanitizeDoctorNote(body) : body,
    shouldRepair ? "Doctor changes" : "Doctor changes preview",
  );
}
...
async function loadAndMaybeMigrateDoctorConfig(params) {
  ...
  const { applyPluginAutoEnable } = await import("./plugin-auto-enable-CooZiS9L.js");
  ...
  const { collectPluginToolAllowlistWarnings } =
    await import("./plugin-tool-allowlist-warnings-DJyI9pnH.js");
  ...
  if (hasConfiguredChannels) {
    const channelDoctor = await import("./channel-doctor-BINiK6s5.js");
  ...
  const { repairHooksTokenReuseGatewayAuth } =
    await import("./hooks-token-reuse-repair-DNNAExE0.js");
  ...
  if (shouldRepair) {
    const { runDoctorRepairSequence } = await import("./repair-sequencing-BRaTsFcw.js");
    ...
    const { collectDoctorPreviewNotes } = await import("./preview-warnings-DIrpFXlW.js");
