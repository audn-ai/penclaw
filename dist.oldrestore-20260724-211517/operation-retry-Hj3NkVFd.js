...
function resolveTransientProviderDelayMs(options, attemptNumber) {
  ...
  return Math.min(
    Math.max(baseDelayMs, Math.round(Number.isFinite(rawMaxDelayMs) ? rawMaxDelayMs : 1e3)),
    baseDelayMs * 2 ** Math.max(attemptNumber - 1, 0),
  );
}
...
async function executeProviderOperationWithRetry(params) {
  const retryOptions = resolveTransientProviderRetryOptions(
    providerOperationRetryConfig(params.stage, params.retry),
  );
...
//#endregion
export {
  resolveTransientProviderRetryOptions as a,
  resolveTransientProviderDelayMs as i,
  providerOperationRetryConfig as n,
  shouldRetrySameKeyProviderOperation as o,
  resolveTransientProviderAttempts as r,
  executeProviderOperationWithRetry as t,
};
