...
const secretsApplyLoader = createLazyImportLoader(() => import("./apply-DkrO4wE2.js"));
async function readPlanFile(pathname) {
  const [{ readFileSync }, { isSecretsApplyPlan }] = await Promise.all([
    fsModuleLoader.load(),
    import("./plan-aaAoUkX8.js"),
  ]);
...
function registerSecretsCli(program) {
    ...
    .action(async (opts) => {
      try {
        const { resolveSecretsAuditExitCode, runSecretsAudit } =
          await import("./audit-BxVjgaxH.js");
  ...
  secrets
    ...
    .action(async (opts) => {
      try {
        const { runSecretsConfigureInteractive } = await import("./configure-UDRqQ0gZ.js");
