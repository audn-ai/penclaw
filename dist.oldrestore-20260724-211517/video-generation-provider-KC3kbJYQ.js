...
const TEXT_ONLY_MODELS = /* @__PURE__ */ new Set(["gen4.5", "veo3.1", "veo3.1_fast", "veo3"]);
...
async function pollRunwayTask(params) {
  ...
  for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt += 1) {
    const payload = await readRunwayJsonResponse(
...
async function downloadRunwayVideos(params) {
  ...
  for (const [index, url] of params.urls.entries()) {
    ...
    const buffer = await readResponseWithLimit(response, params.maxBytes, {
      onOverflow: ({ maxBytes }) =>
        /* @__PURE__ */ new Error(`Runway generated video download exceeds ${maxBytes} bytes`),
    });
    videos.push({
      buffer,
      mimeType,
    ...
    async generateVideo(req) {
      ...
      try {
        ...
        const taskId = normalizeOptionalString(
          (await readRunwayJsonResponse(response, "Runway video generation failed")).id,
        );
