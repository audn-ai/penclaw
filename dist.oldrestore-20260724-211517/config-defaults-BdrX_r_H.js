import {
  n as resolveKnownAnthropicModelRef,
  t as resolveClaudeCliAnthropicModelRefs,
} from "./claude-model-refs-D6pzIuWe.js";
import {
...
function usesClaudeCliModelSelection(config) {
  ...
  return Object.entries(config.agents?.defaults?.models ?? {}).some(([key, entry]) => {
    ...
    return (
      parsed?.provider === "anthropic" &&
      normalizeLowercaseStringOrEmpty(runtimeId) === "claude-cli"
    );
  });
}
...
function modelEntryWithClaudeCliRuntime(entry) {
  ...
  const currentRuntime = normalizeLowercaseStringOrEmpty(
    isRecord(base.agentRuntime) ? base.agentRuntime.id : void 0,
  );
...
function collectClaudeCliRuntimeRefsFromModelMap(models) {
  ...
  for (const key of Object.keys(models ?? {}))
    for (const ref of resolveClaudeCliAnthropicModelRefs(key)?.runtimeRefs ?? []) refs.add(ref);
  return [...refs];
}
...
function applyAnthropicConfigDefaults(params) {
  ...
  if (authMode === "api_key") {
    ...
    if (
      Object.keys(nextModels).some((key) =>
