...
import {
  $ as listMessageReactionUsers,
  At as moveGuildChannels,
  Ct as getGuildMember,
  Dt as listGuildEmojis,
  Et as listGuildChannels,
  Lt as ChannelType,
  Mt as removeGuildMember,
  Nt as removeGuildMemberRole,
  Ot as listGuildRoles,
  Pt as timeoutGuildMember,
  Q as deleteOwnMessageReaction,
  St as getGuild,
  Tt as listGuildActiveThreads,
  Z as createOwnMessageReaction,
  _ as readDiscordMessage,
  _t as createGuildChannel,
  at as editChannelMessage,
  bt as createGuildSticker,
  ct as listChannelArchivedThreads,
  dt as pinChannelMessage,
  et as createChannelMessage,
  ft as searchGuildMessages,
  g as readDiscordCode,
  gt as createGuildBan,
  h as isUnknownDiscordVoiceStateError,
  ht as addGuildMemberRole,
  it as editChannel,
  jt as putChannelPermission,
  kt as listGuildScheduledEvents,
  lt as listChannelMessages,
  m as RateLimitError,
  mt as unpinChannelMessage,
  nt as deleteChannel,
  ot as getChannel,
  p as DiscordError,
  pt as sendChannelTyping,
  rt as deleteChannelMessage,
  st as getChannelMessage,
  tt as createThread,
  ut as listChannelPins,
  v as readRetryAfter,
  vt as createGuildEmoji,
  wt as getGuildVoiceState,
  xt as deleteChannelPermission,
  yt as createGuildScheduledEvent,
...
import { o as unlinkIfExists } from "./media-runtime-BhmZORKM.js";
import {
  E as MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS,
  T as runFfprobe,
  w as runFfmpeg,
  x as parseFfprobeCodecAndSampleRate,
} from "./media-services-Czws84B5.js";
import { r as rewriteDiscordKnownMentions } from "./mentions-DPSZx-Fv.js";
...
import {
  g as readResponseTextLimited,
  m as readProviderJsonResponse,
} from "./provider-http-errors-DLMRBkZU.js";
import { n as createDiscordSendResult } from "./send.receipt-Dc6uxRHl.js";
import {
  C as DISCORD_MAX_STICKER_BYTES,
  F as createDiscordClient,
  P as parseAndResolveChannelRecipient,
  R as resolveDiscordClientAccountContext,
  S as DISCORD_MAX_EVENT_COVER_BYTES,
  g as createDiscordMessageNonce,
  ...
  x as DISCORD_MAX_EMOJI_BYTES,
  z as resolveDiscordRest,
...
import "./send.outbound-CrioyPr2.js";
import { n as DISCORD_ATTACHMENT_TOTAL_TIMEOUT_MS } from "./timeouts-Bj-qI9VL.js";
import { n as resolvePreferredOpenClawTmpDir } from "./tmp-openclaw-dir-uPgNO8da.js";
import { r as truncateUtf16Safe } from "./utf16-slice-lH-m0h6-.js";
import { r as loadWebMediaRaw } from "./web-media-3hQ71Ejp.js";
...
async function timeoutMemberDiscord(payload, opts) {
  ...
  if (!until && payload.durationMinutes) {
    until = timestampMsToIsoString(
      resolveExpiresAtMsFromDurationMs(payload.durationMinutes * 60 * 1e3),
    );
...
async function readMessagesDiscord(channelId, query, opts) {
  ...
  return assertDiscordResponseArray(
...
async function throwWebhookResponseError(response) {
  const parsed = coerceWebhookErrorBody(
    await readResponseTextLimited(response, DISCORD_WEBHOOK_ERROR_BODY_LIMIT_BYTES).catch(() => ""),
  );
...
async function sendWebhookMessageDiscord(text, opts) {
  ...
  const payload =
    response.status === 204
      ? {}
      : await readProviderJsonResponse(response, "Discord webhook send").catch(() => ({}));
  try {
...
async function getAudioDuration(filePath) {
  try {
    const duration = parseStrictFiniteNumber(
      await runFfprobe([
        "-v",
        "error",
        "-show_entries",
...
async function createVoiceRequestError(response, fallbackMessage) {
  const parsed = coerceDiscordErrorBody(
    await readResponseTextLimited(response, DISCORD_VOICE_ERROR_BODY_LIMIT_BYTES).catch(() => ""),
  );
  if (response.status === 429)
...
//#endregion
export {
  removeRoleDiscord as A,
  removeChannelPermissionDiscord as B,
  fetchGuildInfoDiscord as C,
  kickMemberDiscord as D,
  fetchVoiceStatusDiscord as E,
  uploadStickerDiscord as F,
  createChannelDiscord as I,
  deleteChannelDiscord as L,
  timeoutMemberDiscord as M,
  listGuildEmojisDiscord as N,
  listGuildChannelsDiscord as O,
  uploadEmojiDiscord as P,
  editChannelDiscord as R,
  fetchChannelInfoDiscord as S,
  fetchRoleInfoDiscord as T,
  setChannelPermissionDiscord as V,
  ...
  resolveEventCoverImage as j,
  listScheduledEventsDiscord as k,
  ...
  fetchMemberInfoDiscord as w,
  createScheduledEventDiscord as x,
  addRoleDiscord as y,
  moveChannelDiscord as z,
};
