...
import { r as resolveBundledPluginGeneratedPath } from "./bundled-plugin-metadata-Dl89Jc_X.js";
...
import {
  c as isJavaScriptModulePath,
  n as getCachedPluginModuleLoader,
} from "./plugin-module-loader-cache-CDpLvLFE.js";
import { n as loadPluginManifestRegistryForPluginRegistry } from "./plugin-registry-BckMdz4f.js";
...
const SOURCE_MODULE_EXTENSIONS = /* @__PURE__ */ new Set([".ts", ".tsx", ".mts", ".cts"]);
...
function isPackageLocalBundledDistModulePath(params) {
  ...
  ].some((root) => isPathInsideCanonicalRoot(root, params.modulePath));
}
...
function resolveBundledChannelBoundaryRoot(params) {
  ...
  if (overrideBoundaryRoot) boundaryRoot = overrideBoundaryRoot;
    ...
    const distBoundaryRoot = resolveMatchingRoot(
      path.resolve(params.packageRoot, "dist", "extensions", params.metadata.dirName),
    );
    if (distBoundaryRoot) boundaryRoot = distBoundaryRoot;
    else
      boundaryRoot =
        resolveMatchingRoot(
          path.resolve(params.packageRoot, "dist-runtime", "extensions", params.metadata.dirName),
        ) ??
    ...
    if (
      !(
        ...
        (isPackageLocalBundledDistModulePath({
          ...
          findMissingModuleCodeInChain(error) !== void 0)
      )
    )
      throw error;
    return getCachedPluginModuleLoader({
...
function describeBundledChannelLoadError(error, channelId) {
  ...
  if (findMissingModuleCodeInChain(error) !== void 0)
