import { r as stripAnsi, t as sanitizeForLog } from "./ansi-D4OHEz5F.js";
...
import {
  n as normalizeConfiguredProviderCatalogModelId,
  r as normalizeStaticProviderModelId,
} from "./model-ref-shared-Bs7ONe2Q.js";
import {
  a as normalizeModelRef,
  c as parseModelRef,
  i as modelKey,
  n as findNormalizedProviderValue,
  o as normalizeProviderId$1,
...
function resolveManifestPluginsForModelIdNormalization(params) {
  if (params.allowManifestNormalization === false || params.manifestPlugins !== void 0)
...
function findModelAliasCandidate(cfg, raw) {
  ...
  for (const candidate of listModelAliasCandidates(cfg))
    if (normalizeLowercaseStringOrEmpty(candidate.alias) === aliasKey) match = candidate;
  return match;
}
...
function findExactConfiguredProviderRefParts(params) {
  ...
  const apiOwner =
    typeof providerConfig?.api === "string" ? normalizeProviderId$1(providerConfig.api) : "";
...
function buildAllowedModelSetWithFallbacks(params) {
  ...
  if (
    ...
    ((visibility.exactModelRefs.length > 0 && visibility.providerWildcards.size === 0) ||
      (defaultRef && visibility.providerWildcards.has(normalizeProviderId$1(defaultRef.provider))))
  ) {
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg) {
  ...
  return Object.entries(providers).some(
    ([providerRaw, provider]) =>
      Array.isArray(provider?.models) && normalizeProviderId$1(providerRaw) !== "openai",
  );
}
...
function resolveAllowedModelSelection(params) {
  const normalizeSelectionRef = (provider, model) =>
    resolveExactConfiguredProviderRef({
      cfg: params.cfg,
      raw: `${provider}/${model}`,
      allowManifestNormalization: params.allowManifestNormalization,
...
function createModelVisibilityPolicyWithFallbacks(params) {
  ...
  for (const raw of [...visibility.exactModelRefs, ...(params.additionalConfiguredModelRefs ?? [])])
    ...
    visibleCatalog: ({ catalog, defaultVisibleCatalog, view }) => {
      ...
      return dedupeModelCatalogEntries([
        ...defaultVisibleCatalog.filter((entry) =>
          visibility.providerWildcards.has(normalizeProviderId$1(entry.provider)),
        ),
...
//#endregion
export {
  findModelInCatalog as C,
  findModelCatalogEntry as S,
  resolveConfiguredProviderFallback as T,
  resolveBareModelDefaultProvider as _,
  createModelVisibilityPolicyWithFallbacks as a,
  resolveHooksGmailModel as b,
  hasConfiguredProviderModelRows as c,
  isModelKeyAllowedBySet as d,
  modelCatalogLogicalKey as f,
  resolveAllowlistModelKey as g,
  resolveAllowedModelRefFromAliasIndex as h,
  buildModelAliasIndex as i,
  inferUniqueProviderFromCatalog as l,
  parseConfiguredModelVisibilityEntries as m,
  buildConfiguredAllowlistKeys as n,
  dedupeModelCatalogEntries as o,
  normalizeModelSelection as p,
  buildConfiguredModelCatalog as r,
  getModelRefStatusWithFallbackModels as s,
  buildAllowedModelSetWithFallbacks as t,
  inferUniqueProviderFromConfiguredModels as u,
  resolveConfiguredModelRef as v,
  modelSupportsInput as w,
  resolveModelRefFromString as x,
  resolveConfiguredOpenRouterCompatAlias as y,
};
