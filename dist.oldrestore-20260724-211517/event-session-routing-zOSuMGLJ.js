...
import {
  ...
  g as scopedHeartbeatWakeOptions,
...
function resolveEventSessionAllowFrom(params) {
  ...
  const accountConfig =
    accountId && isRecord(channelConfig.accounts) ? channelConfig.accounts[accountId] : void 0;
...
function resolveEventSessionRoutingPolicy(params) {
  ...
  const accountId =
    normalizeLowercaseStringOrEmpty(params.accountId ?? target?.accountId) || void 0;
  const allowFrom =
    params.allowFrom ??
...
function scopedHeartbeatWakeOptionsForPolicy(sessionKey, wakeOptions, policy) {
  if (resolveEventSessionKey(sessionKey, policy?.mainKey, policy?.sessionScope) !== sessionKey)
...
//#endregion
export {
  scopedHeartbeatWakeOptionsForPolicy as i,
  resolveEventSessionRoutingPolicy as n,
  resolveMainScopedEventSessionKey as r,
  resolveEventSessionKeyForPolicy as t,
};
