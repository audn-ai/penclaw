...
function resolveChannelCatalogPluginId(candidate) {
  return (
    normalizeOptionalString(candidate.bundledManifest?.id) ??
    normalizeOptionalString(candidate.bundledManifestId) ??
    normalizeOptionalString(candidate.packageManifest?.plugin?.id) ??
    normalizeOptionalString(candidate.idHint)
  );
}
