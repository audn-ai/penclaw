...
import { t as forceKillChildProcessTree } from "./child-process-tree-10JBMlVM.js";
import {
...
import { r as CONTROL_PLANE_UPDATE_SENTINEL_META_ENV } from "./update-control-plane-sentinel-DS3ELeta.js";
...
function resolveUpdateCliArgv(params) {
  ...
  if (execPath && argv1) return [execPath, argv1, ...updateArgs];
...
async function resolveExecutableOnPath(name, env, fallbackPaths) {
  ...
  if (pathValue) {
    for (const dir of pathValue.split(path.delimiter))
      if (dir.trim()) candidates.add(path.join(dir, name));
  }
...
async function waitForHandoffReady(child) {
  ...
  await new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      finish(
        /* @__PURE__ */ new Error(
          "managed update handoff did not signal readiness within 30 seconds",
        ),
      );
...
async function startManagedServiceUpdateHandoff(params) {
  ...
  const helperParams = {
    ...
    parentExitTimeoutMs:
      params.restartDrainTimeoutMs === void 0
        ? null
...
//#endregion
export {
  formatManagedServiceUpdateCommand as n,
  startManagedServiceUpdateHandoff as r,
  buildManagedServiceHandoffUnavailableMessage as t,
};
