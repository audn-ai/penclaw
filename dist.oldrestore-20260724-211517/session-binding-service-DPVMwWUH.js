...
} from "./session-binding-normalization-sQL-8bRz.js";
...
function resolveChannelSupportsCurrentConversationBinding(params) {
  ...
  const bindingSupport = (getActivePluginChannelRegistryFromState()?.channels ?? []).find((entry) =>
    matchesPluginId(entry.plugin),
  )?.plugin?.conversationBindings;
    ...
    if (params?.deletePersistedFile)
      runOpenClawStateWriteTransaction(
        ({ db }) => {
          executeSqliteQuerySync(
            db,
            getNodeSqliteKysely(db).deleteFrom("current_conversation_bindings"),
          );
        },
...
export {
  testing as a,
  registerSessionBindingAdapter as i,
  getSessionBindingService as n,
  unregisterSessionBindingAdapter as o,
  isSessionBindingError as r,
  normalizeConversationText as s,
