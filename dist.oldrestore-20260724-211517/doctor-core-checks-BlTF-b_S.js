...
import { n as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-DKWQE9Br.js";
import { n as resolveGatewayAuth } from "./auth-resolve-PY0Y7GYi.js";
import "./auth-D-Aebsmj.js";
import { t as resolveGatewayAuthToken } from "./auth-token-resolution--RLjXsYk.js";
import { n as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-BgeH4rqx.js";
import { t as resolveSkillWorkshopConfig } from "./config-XlfFMqhc.js";
import {
  n as maybeArchiveLegacyClawdBrowserProfileResidue,
  r as noteChromeMcpBrowserReadiness,
  t as detectLegacyClawdBrowserProfileResidue,
} from "./doctor-browser-QE_BrBJp.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-DRDGZm_u.js";
import {
  a as shellCompletionStatusToRepairEffects,
  i as shellCompletionStatusToHealthFindings,
  t as checkShellCompletionStatus,
} from "./doctor-completion-CCbtOaHX.js";
import {
  i as sessionLockToRepairEffect,
  r as sessionLockToHealthFinding,
  t as detectStaleSessionLocks,
} from "./doctor-session-locks-DH7cbCin.js";
import {
  n as disableUnavailableSkillsInConfig,
  r as formatMissingSkillSummary,
} from "./doctor-skills-core-DZQVCoNO.js";
import {
  i as uiProtocolFreshnessIssueToRepairEffects,
  r as uiProtocolFreshnessIssueToHealthFinding,
  t as detectUiProtocolFreshnessIssues,
} from "./doctor-ui-Co9JTIe6.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-CT-fkHIP.js";
import { o as registerHealthCheck } from "./health-check-registry-CBs_fO63.js";
import { t as detectSkillWorkshopToolPolicyDiagnostic } from "./tool-policy-diagnostic-C6GE2E7N.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-BBdv1ZLp.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
  const { collectSecurityWarnings } = await import("./doctor-security-CxeSh8PI.js");
  return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
  const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-ssAKnITK.js");
...
async function collectProviderCatalogProjectionFindingsWithRuntime(ctx) {
  return (await loadDoctorCoreChecksRuntimeModule()).collectProviderCatalogProjectionFindings(
    ctx.cfg,
  );
}
...
const hooksModelCheck = {
  ...
  async detect(ctx) {
    ...
    const { loadModelCatalog } = await import("./model-catalog-CKXmb4DJ.js");
    const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } =
      await import("./model-selection-Dc7njIe_.js");
...
const legacyStateCheck = {
  ...
  async detect(ctx) {
    const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-Cuhh0aLt.js");
    const detected = await detectLegacyStateMigrations({ cfg: ctx.cfg });
    return [
      ...detected.preview.map((line) => ({
...
const bootstrapSizeCheck = {
  ...
  async detect(ctx) {
    const { buildBootstrapInjectionStats, analyzeBootstrapBudget } =
      await import("./bootstrap-budget-BSpyOjMp.js");
    const { resolveBootstrapContextForRun } = await import("./bootstrap-files-Czz7zXyA.js");
    const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } =
      await import("./embedded-agent-helpers-_Relon_S.js");
...
function createSecurityCheck(deps) {
  return {
    ...
    source: "doctor",
    async detect(ctx) {
      return (await deps.collectSecurityWarnings(ctx.cfg)).map((warning) =>
        noteTextToFinding({
...
const legacyWhatsAppCrontabCheck = {
  ...
  defaultEnabled: false,
  async detect() {
    const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-By4ujAvV.js");
...
const legacyCronStoreCheck = {
  ...
  defaultEnabled: false,
  async detect(ctx) {
    const { collectLegacyCronStoreHealthFindings } = await import("./cron-By4ujAvV.js");
...
const gatewayPlatformNotesCheck = {
  ...
  async detect(ctx) {
    ...
    return (await collectMacGatewayPlatformWarnings(ctx.cfg)).map((warning) =>
      noteTextToFinding({
...
function createSkillsReadinessCheck(deps) {
  return {
    ...
    async detect(ctx, scope) {
      return filterUnavailableSkillsForScope(
        await deps.detectUnavailableSkills(ctx.cfg),
        scope?.paths,
      ).map(unavailableSkillToFinding);
    },
...
function createWorkspaceSuggestionsCheck(deps) {
  return {
    ...
    async detect(ctx) {
      ...
      return (await deps.collectWorkspaceSuggestionNotes(workspaceDir)).map((text) =>
        noteTextToFinding({
...
//#endregion
export {
  createCoreHealthChecks as a,
  configValidationIssuesToHealthFindings as i,
  buildGatewayTokenSecretRefFixHint as n,
  registerCoreHealthChecks as o,
  buildGatewayTokenSecretRefUnavailableMessage as r,
  resetCoreHealthChecksForTest as s,
  CORE_HEALTH_CHECKS as t,
};
