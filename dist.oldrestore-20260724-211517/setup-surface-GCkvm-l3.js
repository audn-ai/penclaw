...
import { t as formatResolvedUnresolvedNote } from "./setup-CM3hyaUe.js";
import {
...
import { l as patchScopedAccountConfig } from "./setup-helpers-Dl0hnWtQ.js";
import {
  ...
  prepare: async ({ cfg, accountId, prompter, options }) => {
    ...
    if (!(await checkZcaAuthenticated(account.profile, { credentialPersistence: "read-only" }))) {
      ...
      if (start.qrDataUrl) {
        const qrPath = await writeQrDataUrlToTempFile(start.qrDataUrl, account.profile);
        await prompter.note(
          [start.message, qrPath ? t("wizard.zalouser.qrImageSaved", { path: qrPath }) : void 0]
            .filter(Boolean)
            .join("\n"),
          t("wizard.zalouser.qrLoginTitle"),
        );
