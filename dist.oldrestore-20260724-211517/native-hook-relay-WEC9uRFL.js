...
import {
  ...
  n as cancelDeferredPluginToolApproval,
...
async function resolveNativeHookRelayPreToolUseApproval(pendingApproval, signal) {
  ...
  if (nativeHookRelayParamsWereRewritten(pendingApproval.originalParamsFingerprint, outcome.params))
    return {
      handled: true,
      outcome: "denied",
      reason:
...
function pruneExpiredNativeHookRelays(now = Date.now()) {
  for (const [relayId, registration] of relays)
    if (now > registration.expiresAtMs) unregisterNativeHookRelay(relayId, registration);
}
...
function readCodexToolInput(rawPayload) {
  ...
  if (isJsonObject(toolInput))
    return normalizeCodexToolInput(
      normalizeNativeHookToolName(readOptionalString(payload.tool_name)),
      toolInput,
    );
...
function readNativeHookRelayApprovalMode(rawPayload) {
  return (isJsonObject(rawPayload) ? rawPayload : {}).openclaw_approval_mode === "report"
    ? "report"
    : void 0;
}
...
//#endregion
export {
  isNativeHookRelayBridgeStaleRegistrationError as a,
  resolveNativeHookRelayDeferredToolApproval as c,
  invokeNativeHookRelayBridge as i,
  testing as l,
  hasNativeHookRelayInvocation as n,
  registerNativeHookRelay as o,
  invokeNativeHookRelay as r,
  renderNativeHookRelayUnavailableResponse as s,
  buildNativeHookRelayCommand as t,
};
