...
import {
  i as waitForNodeReconnect,
  n as maybeWakeNodeWithApns,
  t as maybeSendNodeWakeNudge,
} from "./nodes-CPGg5qjY.js";
import {
  ...
  return (
    resolveExpiresAtMsFromDurationMs(
      typeof expiresInMs === "number" && Number.isFinite(expiresInMs)
        ? Math.max(1e3, Math.trunc(expiresInMs))
...
function drainNodePendingWork(nodeId, opts = {}) {
  ...
  if (state && explicitReturnedCount > 0) {
    for (const item of items)
      if (item.id !== DEFAULT_STATUS_ITEM_ID) state.itemsById.delete(item.id);
    state.revision += 1;
    pruneStateIfEmpty(normalizedNodeId, state);
  }
  return {
