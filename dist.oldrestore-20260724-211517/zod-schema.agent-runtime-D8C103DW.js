...
import {
  ...
  M as ToolsMediaSchema,
  j as ToolsLinksSchema,
  m as IdentitySchema,
...
const AgentToolModelSchema = union([
  string(),
  object({
    primary: string().optional(),
    fallbacks: array(string()).optional(),
...
const AgentEntryEmbeddedAgentConfigSchema = object({
...
const SandboxDockerSchema = object({
  ...
  setupCommand: union([string(), array(string())])
...
const AgentContextLimitsSchema = object({
  ...
  .strict()
  .optional();
const AgentSkillsLimitsSchema = object({ maxSkillsPromptChars: number().int().min(0).optional() })
  .strict()
  .optional();
...
const TrimmedOptionalConfigStringSchema = string()
  .transform((value) => {
    const trimmed = value.trim();
    return trimmed.length > 0 ? trimmed : void 0;
  })
  .optional();
const CodexAllowedDomainsSchema = array(string())
  .transform((values) => {
  ...
  search: preprocess(
    ...
    object({
      ...
      openaiCodex: object({
        ...
        contextSize: union([literal("low"), literal("medium"), literal("high")]).optional(),
...
const ToolExecBaseShape = {
  host: _enum(["auto", "sandbox", "gateway", "node"]).optional(),
  mode: _enum(["deny", "allowlist", "ask", "auto", "full"]).optional(),
  security: _enum(["deny", "allowlist", "full"]).optional(),
  ask: _enum(["off", "on-miss", "always"]).optional(),
...
const AgentToolExecSchema = object({
  ...
  .optional();
const ToolExecSchema = object(ToolExecBaseShape)
  .strict()
  .superRefine(addExecPolicyModeConflictIssue)
  .optional();
...
const ToolLoopDetectionDetectorSchema = object({
  ...
  .strict()
  .optional();
const ToolLoopPostCompactionGuardSchema = object({
  windowSize: number().int().positive().optional(),
})
  .strict()
  .optional();
const ToolLoopDetectionSchema = object({
  ...
  .optional();
const ToolSearchSchema = union([
  boolean(),
  object({
    enabled: boolean().optional(),
    mode: _enum(["code", "tools", "directory"]).optional(),
...
const AgentSandboxSchema = object({
  mode: union([literal("off"), literal("non-main"), literal("all")]).optional(),
  backend: string().min(1).optional(),
  workspaceAccess: union([literal("none"), literal("ro"), literal("rw")]).optional(),
  sessionToolsVisibility: union([literal("spawned"), literal("all")]).optional(),
  scope: union([literal("session"), literal("agent"), literal("shared")]).optional(),
...
const MessageToolConfigSchema = object({
  ...
  crossContext: object({
    ...
    .strict()
    .optional(),
  actions: object({ allow: array(string()).optional() })
    .strict()
    .optional(),
...
const MemorySearchSchema = object({
  ...
  multimodal: object({
    enabled: boolean().optional(),
    modalities: array(union([literal("image"), literal("audio"), literal("all")])).optional(),
  ...
  store: object({
    ...
    fts: object({ tokenizer: union([literal("unicode61"), literal("trigram")]).optional() })
      .strict()
      .optional(),
    vector: object({
      enabled: boolean().optional(),
...
const AgentRuntimeAcpSchema = object({
  ...
  .strict()
  .optional();
const AgentRuntimeSchema = union([
  object({ type: literal("embedded") }).strict(),
  object({
    type: literal("acp"),
...
const AgentEntrySchema = object({
  ...
  verboseDefault: _enum(["off", "on", "full"]).optional(),
  toolProgressDetail: _enum(["explain", "raw"]).optional(),
  reasoningDefault: _enum(["on", "off", "stream"]).optional(),
...
const ToolsSchema = object({
  ...
  sessions: object({ visibility: _enum(["self", "tree", "agent", "all"]).optional() })
    .strict()
    .optional(),
  loopDetection: ToolLoopDetectionSchema,
...
export {
  ...
  AgentToolModelSchema as f,
  AgentRunRetriesConfigSchema as i,
  ToolPolicySchema as l,
  AgentEntrySchema as n,
  ElevatedAllowFromSchema as o,
  AgentModelRuntimeEntrySchema as r,
