...
function collectToolPolicySources(policy, label, out) {
  ...
  collectToolPolicySources(
    isRecord(policy.sandbox) ? policy.sandbox.tools : void 0,
    `${label}.sandbox.tools`,
    out,
  );
  collectToolPolicySources(
    isRecord(policy.subagents) ? policy.subagents.tools : void 0,
...
function collectActiveSandboxToolPolicies(cfg, serverNames) {
  ...
  if (Array.isArray(agentList))
    agentList.forEach((agent, index) => {
      ...
      if (!(explicitMode === void 0 ? defaultSandboxActive : isSandboxModeActive(explicitMode)))
        return;
...
function buildMcpToolNamePrefixes(serverNames) {
  const usedNames = /* @__PURE__ */ new Set();
  return serverNames
    .map((serverName) => normalizeToolName(`${sanitizeServerName(serverName, usedNames)}__`))
    .filter(Boolean);
}
function entriesMatchMcpTool(entries, serverNames, mode) {
  ...
  const prefixOrPatternMatches = (prefix, index) =>
...
function sandboxPolicyAllowsAllMcpServers(policy, serverNames) {
  ...
  return entriesMatchEveryMcpTool(
    [...(allow ?? []), ...(getList(policy, "alsoAllow") ?? [])],
    serverNames,
  );
}
...
function profileToolPolicyBlocksMcp(policy, serverNames) {
  const profilePolicy = mergeAlsoAllowPolicy(
    resolveToolProfilePolicy(
      isRecord(policy) && typeof policy.profile === "string" ? policy.profile : "",
    ),
