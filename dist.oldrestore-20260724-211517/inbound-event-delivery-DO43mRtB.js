...
function notifyDiscordInboundEventOutboundPayloadSuccess(params) {
  const metadata = asOptionalRecord(
    asOptionalRecord(asOptionalRecord(params.payload.channelData)?.discord)?.[
      DISCORD_INBOUND_EVENT_DELIVERY_KEY
    ],
  );
  if (!metadata) return;
...
//#endregion
export {
  withDiscordInboundEventDeliveryMetadata as i,
  notifyDiscordInboundEventOutboundPayloadSuccess as n,
  notifyDiscordInboundEventOutboundSuccess as r,
  beginDiscordInboundEventDeliveryCorrelation as t,
};
