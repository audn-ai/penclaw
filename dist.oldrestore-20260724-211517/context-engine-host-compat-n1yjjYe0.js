...
import {
  a as evaluateContextEngineHostSupport,
...
import { R as normalizeEmbeddedAgentRuntime } from "./openai-routing-Cu28Ynzk.js";
import { t as resolveAgentHarnessPolicy } from "./policy-C704eijA.js";
import { o as isRecord } from "./record-coerce-DHZ4bFlT.js";
import {
  n as getContextEngineRegistration,
...
function listModelRefs(value) {
  ...
  if (Array.isArray(value.fallbacks)) {
    for (const fallback of value.fallbacks)
      if (typeof fallback === "string" && fallback.trim()) refs.push(fallback.trim());
  }
  return refs;
}
...
function collectSelectedModelRefs(cfg) {
  ...
  if (cfg.agents?.defaults?.model !== void 0)
...
async function resolveSelectedContextEngineInfo(params) {
  ...
  if (engineId === defaultSlotIdForKey("contextEngine") || engineId === "none")
    return {
      info: {
        id: engineId,
        name: engineId,
  ...
  if (getContextEngineRegistration(engineId)?.lifecycle !== "runtime") {
    try {
      ...
      if (getContextEngineRegistration(engineId)?.lifecycle !== "runtime")
        return {
          warnings: [
            `- plugins.slots.contextEngine: could not inspect context engine "${engineId}" host requirements because its plugin failed to load: ${error instanceof Error ? error.message : String(error)}`,
          ],
        };
    }
    if (getContextEngineRegistration(engineId)?.lifecycle !== "runtime")
  ...
  try {
    ...
    return {
      warnings: [
        `- plugins.slots.contextEngine: could not inspect context engine "${engineId}" host requirements: ${error instanceof Error ? error.message : String(error)}`,
      ],
    };
  }
}
...
//#endregion
export {
  collectContextEngineHostCompatibilityWarnings as n,
  maybeRepairContextEngineHostCompatibility as r,
  collectConfiguredContextEngineAgentRunHosts as t,
};
