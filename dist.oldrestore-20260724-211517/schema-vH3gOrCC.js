...
function mapSensitivePathsMut(schema, path, hints) {
  ...
  if (currentSchema instanceof ZodObject) {
    ...
    const catchallSchema = currentSchema["_def"].catchall;
    if (catchallSchema && !(catchallSchema instanceof ZodNever))
      mapSensitivePathsMut(catchallSchema, path ? `${path}.*` : "*", hints);
  } else if (currentSchema instanceof ZodArray) {
    const nextPath = path ? `${path}[]` : "[]";
    ...
    for (const option of currentSchema.options) mapSensitivePathsMut(option, path, hints);
  else if (currentSchema instanceof ZodIntersection) {
...
const LOOKUP_SCHEMA_COMPOSITION_KEYS = ["anyOf", "oneOf", "allOf"];
