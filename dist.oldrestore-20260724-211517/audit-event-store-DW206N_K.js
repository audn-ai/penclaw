...
const CONVERSATION_KINDS = ["direct", "group", "channel", "unknown"];
const DELIVERY_KINDS = ["text", "media", "other"];
const FAILURE_STAGES = ["platform_send", "queue", "unknown"];
...
function requiredInteger(row, value, field, minimum) {
  ...
  if (normalized === void 0 || !Number.isSafeInteger(normalized) || normalized < minimum)
...
function parseAgentRecordFields(row) {
  ...
  return {
    ...
    ...(optionalText(row, row.session_key, "sessionKey") !== void 0
      ? { sessionKey: requiredText(row, row.session_key, "sessionKey") }
      : {}),
    ...(optionalText(row, row.session_id, "sessionId") !== void 0
...
function projectMessageIdentities(db, input) {
  ...
  const conversationId =
    input.conversationId ?? (input.direction === "outbound" ? input.targetId : void 0);
