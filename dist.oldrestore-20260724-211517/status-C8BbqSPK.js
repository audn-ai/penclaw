...
import { n as resolveControlUiLinks } from "./control-ui-links-BImlWgwk.js";
import { i as renderGatewayServiceCleanupHints } from "./inspect-CD2p_qWO.js";
...
import { t as formatGatewayRestartHandoffDiagnostic } from "./restart-handoff-C_HtGsPl.js";
import {
...
} from "./status.gather-Bx6ljU66.js";
import { h as classifySystemdUnavailableDetail } from "./systemd-BI3sgDKJ.js";
import { n as isRich, r as theme, t as colorize } from "./theme-vjDs9tao.js";
import { d as isWSLEnv } from "./undici-runtime-Bm6J6FAw.js";
...
function printDaemonStatus(status, opts) {
  ...
  if (service.runtime?.missingUnit) {
    ...
    defaultRuntime.error(
      errorText(
        startLimitHit
          ? `systemd stopped restarting the gateway after repeated crashes; run ${formatCliCommand("openclaw gateway restart")} or inspect logs.`
  ...
  if (drift && drift.drifts.length > 0) {
    defaultRuntime.log(
      warnText(
        `Plugin version drift: ${drift.drifts.length} active official plugin${drift.drifts.length === 1 ? "" : "s"} not on gateway ${drift.gatewayVersion}`,
      ),
    );
    if (opts.deep) {
