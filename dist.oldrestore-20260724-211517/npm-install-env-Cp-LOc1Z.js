...
function resolveNpmConfigPath(rawPath, env) {
  ...
  const home = env.HOME?.trim() || env.USERPROFILE?.trim() || os.homedir();
  return (process.platform === "win32" ? /^~(\/|\\)/u : /^~\//u).test(expanded) && home
...
function createNpmConfigPathProbeEnv(env) {
  ...
  for (const key of NPM_CONFIG_PATH_PROBE_PARENT_ENV_KEYS)
    if (probeEnv[key] == null && process.env[key] != null) probeEnv[key] = process.env[key];
  return probeEnv;
}
 ...
 */
function createNpmFreshnessBypassArgs(
  env = process.env,
...
//#endregion
export {
  createNpmProjectInstallEnv as i,
  applyPosixNpmScriptShellEnv as n,
  createNpmFreshnessBypassArgs as r,
  applyNpmFreshnessBypassEnv as t,
};
