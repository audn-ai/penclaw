...
function splitPresentationText(value, limits) {
  if (!positiveInteger(limits?.maxLength) || truncatePresentationText(value, limits) === value)
    return [value];
  const chunks = [];
  let remaining = value;
  while (remaining) {
...
function buttonCapacityAfterReservedSelects(limits, reservedSelects) {
  ...
  const remainingActions =
    maxActions === void 0 ? void 0 : Math.max(0, maxActions - reservedSelects);
  const remainingRows = maxRows === void 0 ? void 0 : Math.max(0, maxRows - reservedSelects);
  const rowCapacity =
    remainingRows !== void 0 && maxActionsPerRow !== void 0
      ? remainingRows * maxActionsPerRow
      : void 0;
  if (remainingActions !== void 0 && rowCapacity !== void 0)
...
function buttonCapacity(budget) {
  ...
  const rowCapacity =
    budget.remainingRows && budget.maxActionsPerRow
      ? budget.remainingRows * budget.maxActionsPerRow
      : void 0;
  if (budget.remainingActions !== void 0 && rowCapacity !== void 0)
...
function consumeButtonBudget(budget, count) {
  if (count <= 0) return;
  if (budget.remainingActions !== void 0)
...
function adaptButtonsBlock(block, limits, budget, fallbackBlockType, buttonSelection) {
  ...
  const selectedCandidates =
    capacity !== void 0 && eligibleCandidates.length > capacity
      ? eligibleCandidates
          .map((candidate, index) => ({
            candidate,
            index,
  ...
  return blocks.filter((block) => {
    ...
    return (
      block.options
        ...
        .filter(Boolean)
        .slice(0, maxOptions ?? void 0).length > 0
    );
  }).length;
}
...
function applyPresentationActionLimits(buttons, capabilities) {
  return adaptButtonsBlock(
    ...
    void 0,
  ).flatMap((entry) => (entry.type === "buttons" ? entry.buttons : []));
}
...
//#endregion
export {
  presentationPageSize as i,
  adaptMessagePresentationForChannel as n,
  applyPresentationActionLimits as r,
  reduceInteractiveReply as t,
};
