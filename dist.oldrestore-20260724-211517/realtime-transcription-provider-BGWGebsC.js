...
import { t as createRealtimeTranscriptionWebSocketSession } from "./realtime-transcription-DeEtIU2b.js";
...
function readNestedElevenLabsConfig(rawConfig) {
  ...
  return (
    asOptionalRecord(asOptionalRecord(raw?.providers)?.elevenlabs ?? raw?.elevenlabs ?? raw) ?? {}
  );
}
...
function normalizeIntegerRange(value, min, max) {
  const parsed = parseFiniteNumber(value);
  return parsed !== void 0 && Number.isSafeInteger(parsed) && parsed >= min && parsed <= max
    ? parsed
    : void 0;
}
