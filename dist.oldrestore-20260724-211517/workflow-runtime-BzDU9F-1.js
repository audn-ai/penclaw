...
import { i as canResolveEnvSecretRefInReadOnlyPath } from "./extension-shared-DqZe9psR.js";
...
import { o as normalizeBaseUrl } from "./provider-request-config-DpW-uV6A.js";
...
function resolveComfyRemainingMs(deadline, timeoutMs, defaultTimeoutMs = timeoutMs) {
  ...
  if (remainingMs <= 0)
    throw new Error(`Comfy workflow did not finish within ${Math.ceil(timeoutMs / 1e3)}s`);
  ...
  try {
    ...
    return {
      buffer: await readResponseWithLimit(firstResponse.response, params.maxBytes, {
        ...
        onOverflow: ({ maxBytes }) =>
          /* @__PURE__ */ new Error(
            `Comfy ${params.capability} output download exceeds ${maxBytes} bytes`,
          ),
...
export {
  ...
  readJsonResponseForTest as i,
  getComfyConfig as n,
  setComfyFetchGuardForTesting as o,
  isComfyCapabilityConfigured as r,
