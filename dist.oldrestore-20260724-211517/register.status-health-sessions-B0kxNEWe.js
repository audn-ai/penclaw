...
import {
  i as parseStrictPositiveIntOrUndefined,
  n as parsePositiveIntOrUndefined,
} from "./helpers-WGaRKVcg.js";
...
const loadCommitmentsCommands = createModuleLoader(() => import("./commitments-CnkGS5mP.js"));
const loadTasksCommands = createModuleLoader(() => import("./tasks-C7y5T5h8.js"));
const loadFlowsCommands = createModuleLoader(() => import("./flows-E6-JObIg.js"));
...
function registerStatusHealthSessionsCommands(program) {
  ...
  sessionsCmd
    ...
    .action(async (opts, command) => {
      const parentOpts = command.parent?.opts();
      await runCommandWithRuntime(defaultRuntime, async () => {
        const { sessionsCleanupCommand } = await import("./sessions-cleanup-CwmbVEhy.js");
  ...
  sessionsCmd
    ...
    .action(async (opts, command) => {
      const parentOpts = command.parent?.opts();
      await runCommandWithRuntime(defaultRuntime, async () => {
        const { sessionsTailCommand } = await import("./sessions-tail-CKIwREIr.js");
  ...
  sessionsCmd
    ...
    .action(async (opts, command) => {
      const parentOpts = command.parent?.opts();
      await runCommandWithRuntime(defaultRuntime, async () => {
        const { exportTrajectoryCommand } = await import("./export-trajectory-BneHaDux.js");
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        ...
        ])}\n\n${theme.muted("Backed by the sessions.compact gateway RPC; exits non-zero when compaction fails.")}`,
    )
    .action(async (key, opts, command) => {
      ...
      await runCommandWithRuntime(defaultRuntime, async () => {
        const { sessionsCompactCommand } = await import("./sessions-compact-CNYLyMW2.js");
