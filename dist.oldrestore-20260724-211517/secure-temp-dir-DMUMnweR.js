...
} from "./fs-safe-defaults-B7hUN42l.js";
...
async function syncDirectoryBestEffort(dirPath) {
  ...
  try {
    const flags =
      ...
      ("O_DIRECTORY" in fs.constants ? fs.constants.O_DIRECTORY : 0) |
...
async function mkdirPathComponentsWithGuards(params) {
  ...
  for (const part of relative.split(path.sep).filter(Boolean)) {
    ...
    if (!isSameOrChildPath(path.resolve(await fs$1.realpath(next)), rootCanonical))
...
function getWorker() {
  ...
  child.once("close", (code, signal) => {
    rejectPending(
      new FsSafeError(
        "helper-failed",
        currentWorker.stderr.trim() ||
          `pinned helper exited with code ${code ?? "null"} (${signal ?? "?"})`,
      ),
      currentWorker,
    );
  });
...
async function assertMoveMutationAllowed(root, params) {
  await assertMutationNotDenied(
    (await resolvePathInRoot(root, params.fromRelative)).resolved,
    params.denyMutations,
    { protectAncestors: true },
  );
  await assertMutationNotDenied(
    (await resolvePathInRoot(root, params.toRelative)).resolved,
    params.denyMutations,
    { protectAncestors: true },
  );
}
...
export {
  root as a,
  runPinnedWriteHelper as c,
  expandHomePrefix as d,
  resolveHomeRelativePath as f,
  readLocalFileSafely as i,
  getFsSafeTestHooks as l,
  DEFAULT_ROOT_MAX_BYTES as n,
  PATH_ALIAS_POLICIES as o,
  resolveUserPath as p,
  openLocalFileSafely as r,
  assertNoPathAliasEscape as s,
  resolveSecureTempRoot as t,
  resolveOpenedFileRealPathForHandle as u,
};
