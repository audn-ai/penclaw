...
function formatNodeApprovalNotice(notice) {
  const lines = [
    `Node ${notice.node.approvalState === "pending-reapproval" ? "reapproval" : "approval"} pending for ${sanitizeForLog(normalizeOptionalString(notice.node.displayName) ?? notice.node.nodeId)}. Run ${sanitizeForLog(notice.command)}`,
  ];
...
async function findPairedDevicePendingNodeApprovalNotices(opts, paired) {
  ...
  return buildPendingNodeApprovalNoticesForOpts(
    (await tryReadPendingNodeApprovals(opts)).filter((node) =>
      nodeDevices.some((device) => nodeMatchesPairedDevice(node, device)),
    ),
    opts,
  );
}
async function findQueryPendingNodeApprovalNotices(opts, query) {
  ...
  const pairedMatches = ((await tryReadGatewayPairingList(opts))?.paired ?? []).filter((device) =>
...
function replacementScopesCoverOriginal(original, replacement, paired) {
  ...
  return replacementScopes.every(
    (scope) => originalScopes.includes(scope) || scope === "operator.pairing",
  );
}
