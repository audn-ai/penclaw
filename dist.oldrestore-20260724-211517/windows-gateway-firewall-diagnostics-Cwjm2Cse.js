...
function localRulesAreAllowed(params) {
  const explicitActiveProfiles = findProfileSettings(
    params.activeProfiles,
    params.activeProfileNames,
  ).filter(
    (profile) =>
  ...
  const explicitLocalProfiles = findProfileSettings(
    params.localProfiles,
    params.activeProfileNames,
  ).filter(
    (profile) =>
      profile.allowLocalFirewallRules && profile.allowLocalFirewallRules !== "notconfigured",
...
function classifyWindowsGatewayFirewallState(state) {
  ...
  const programAgnosticLocalRules = state.localMatchingRules
    .filter((rule) => ruleMatchesActiveProfile(rule, state.activeProfileNames))
    .filter((rule) => isProgramAgnosticRule(rule) && isAddressAgnosticRule(rule));
  ...
  if (programAgnosticMatchingRules.length > 0) {
    ...
    if (
      programAgnosticMatchingRules.filter(isLocalRule).length ===
        programAgnosticMatchingRules.length &&
      !localRulesAreAllowed(state)
    ) {
...
async function inspectWindowsGatewayFirewall(params) {
  if ((params.platform ?? process.platform) !== "win32" || params.bind !== "lan")
    return {
      applies: false,
      severity: "info",
  ...
  if (mode === "quick") {
    ...
    const state = buildClassifiedState(
      stateJson,
      "",
      [...managedActiveRules, ...(localRulesAreAllowed(policyState) ? localRules : [])],
      localRules,
    );
    return state
