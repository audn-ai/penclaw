...
} from "./fences-rLVnT2kD.js";
import { M as resolveChannelStreamingChunkMode } from "./streaming-D_YE0SPS.js";
...
function chunkByNewline(text, maxLineLength, opts) {
  ...
  for (const line of lines) {
    ...
    const firstLimit = avoidTrailingHighSurrogateBreak(
      lineValue,
      0,
      Math.max(1, maxLineLength - prefix.length),
    );
  ...
  while (start < text.length) {
    ...
    if (initialFence) {
      ...
      if (maxIdxIfNeedNewline <= start) breakIdx = windowEnd;
        ...
        if (!pickedNewline)
          if (minProgressIdx > maxIdxIfAlreadyNewline) breakIdx = windowEnd;
    ...
    if (safeBreakIdx !== breakIdx) {
      ...
      if (fenceToSplit) {
        const fenceAtBreak = findFenceSpanAt(spans, breakIdx);
        fenceToSplit =
          fenceAtBreak && fenceAtBreak.start === fenceToSplit.start ? fenceAtBreak : void 0;
      }
    }
