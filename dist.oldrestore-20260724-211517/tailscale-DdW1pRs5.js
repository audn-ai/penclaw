...
async function getTailnetHostname(exec = runExec, detectedBinary) {
  ...
  throw toErrorObject(
    lastError ?? /* @__PURE__ */ new Error("Could not determine Tailscale DNS or IP"),
    "Non-Error thrown",
  );
}
/**
...
async function hasTailscaleFunnelRouteForPort(port, exec = runExec) {
  ...
  try {
    stdout = (
      await exec(await getTailscaleBinary(), ["funnel", "status", "--json"], {
        maxBuffer: 2e5,
        timeoutMs: 5e3,
      })
    ).stdout;
...
function parseWhoisIdentity(payload) {
  const userProfile =
    asNullableObjectRecord(payload.UserProfile) ??
    asNullableObjectRecord(payload.userProfile) ??
    asNullableObjectRecord(payload.User);
  const login =
...
async function readTailscaleWhoisIdentity(ip, exec = runExec, opts) {
  ...
  try {
    const result = await exec(await getTailscaleBinary(), ["whois", "--json", normalized], {
      timeoutMs: opts?.timeoutMs ?? 5e3,
      maxBuffer: 2e5,
    });
    const identity = parseWhoisIdentity(
      result.stdout ? parsePossiblyNoisyJsonObject(result.stdout) : {},
    );
