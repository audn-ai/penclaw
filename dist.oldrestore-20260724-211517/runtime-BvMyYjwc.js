...
import "./runtime-api-C15FJE4y.js";
...
import { n as AcpRuntimeError$1 } from "./errors-BTjUwYsc.js";
import { b as parseStrictPositiveInteger } from "./number-coercion-CJQ8TR--.js";
import {
  i as createAcpxProcessLeaseId,
  o as hashAcpxProcessCommand,
  u as withAcpxLeaseEnvironment,
} from "./process-lease-DiKkFj6F.js";
import {
  c as splitCommandParts,
  n as isOpenClawLeaseAwareAcpxProcessCommand,
  t as cleanupOpenClawOwnedAcpxProcessTree,
} from "./process-reaper-BUFiavaJ.js";
...
function isUsageLike(error) {
  ...
  return (
    ...
    asRecord$7(error)?.code === "commander.invalidArgument"
  );
}
...
function assignParsedModelState(state, record) {
  ...
  if (state.model_control === void 0 && state.available_models !== void 0)
...
function assignDesiredConfigOptions(state, raw) {
  ...
  const parsed = Object.fromEntries(
    Object.entries(desiredConfigOptions).filter((entry) => {
      const [, value] = entry;
...
const TOOL_KIND_TITLE_MATCHERS = [
  ...
  {
    kind: "search",
    needles: ["search", "find", "grep"],
  },
  {
    kind: "edit",
    needles: ["write", "edit", "patch"],
  },
  {
    kind: "delete",
  ...
  {
    kind: "execute",
    needles: ["run", "execute", "bash"],
  },
  {
    kind: "fetch",
    needles: ["fetch", "http", "url"],
  },
  {
    kind: "think",
    needles: ["think"],
...
async function resolveEscalatingPermissionRequest(params, policyMatch, allowOption, rejectOption) {
  ...
  return {
    response: withEscalationMetadata(
...
async function resolvePermissionRequestWithDetails(
  params,
  mode,
  ...
  const resolvedByPolicy = await resolvePolicyMatch(
    params,
    matchPermissionPolicy(params, policy),
    options,
    allowOption,
    rejectOption,
  );
...
const QODER_BENIGN_STDOUT_LINES = /* @__PURE__ */ new Set([
...
function assignSessionEnv(env, key, value) {
  ...
  for (const existingKey of Object.keys(env))
    if (protectedEnvKey(existingKey) === normalizedKey) delete env[existingKey];
  env[key] = value;
}
...
function isRequestedModelUnsupportedError(value) {
  ...
  return (
    ...
    candidate.code === "ACP_MODEL_UNSUPPORTED" &&
...
function assertRequestedModelSupported(params) {
  if (!params.models) {
    ...
    throw new RequestedModelUnsupportedError(
      `Cannot ${params.context === "replay" ? "replay saved model" : "apply --model"} "${params.requestedModel}": the ACP agent did not advertise model support through a session config option or legacy models metadata, and the adapter does not support a startup model flag.`,
      "missing-capability",
    );
  }
  if (
...
async function spawnTerminalProcess(params, defaultCwd) {
  ...
  try {
    ...
    const fallbackCommand =
      params.args === void 0 && isNotFoundSpawnError(error)
...
async function runProcessListCommand() {
  ...
  return await new Promise((resolve, reject) => {
    const child = spawn("ps", ["-eo", "pid=,ppid="], { stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    child.stdout.setEncoding("utf8");
    child.stderr.setEncoding("utf8");
...
async function rememberProcessGroupPids(terminal) {
  ...
  for (const pid of await listProcessGroupPids(processGroupId))
    if (pid !== processGroupId) terminal.descendantPids.add(pid);
}
...
async function runProcessGroupListCommand() {
  return await new Promise((resolve, reject) => {
    const child = spawn("ps", ["-eo", "pid=,pgid="], { stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    child.stdout.setEncoding("utf8");
    child.stderr.setEncoding("utf8");
...
async function runWindowsProcessListCommand() {
  return await new Promise((resolve, reject) => {
    ...
    child.once("close", (code, signal) => {
      ...
      reject(
        /* @__PURE__ */ new Error(
          `powershell process list exited with code ${code ?? "null"} signal ${signal ?? "null"}: ${stderr}`,
        ),
      );
    });
  });
...
var AcpClient = class {
  ...
  updateRuntimeOptions(options) {
    const shouldRefreshPermissionPolicy =
      options.permissionMode !== void 0 || options.nonInteractivePermissions !== void 0;
  ...
  async setSessionModelThroughLegacyMethod(sessionId, modelId) {
    ...
    try {
      await this.runConnectionRequest(() =>
        connection.extMethod("session/set_model", {
          sessionId,
          modelId,
        }),
      );
  ...
  hostPermissionErrorResponse(params, signal, error) {
    ...
    this.log(
      `onPermissionRequest threw, falling through to mode-based resolver: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
...
function hasPersistedSessionOptions(options) {
  return (
    ...
    options.allowed_tools !== void 0 ||
    options.max_turns !== void 0 ||
    options.system_prompt !== void 0 ||
    options.env !== void 0
  );
}
...
function applyConfigOptionsModelState(state, configOptions) {
  ...
  const preservesLegacyControl =
    ...
    (state.model_control === void 0 &&
      previousConfigModels === void 0 &&
      legacyModelState(state) !== void 0);
...
const SESSION_UPDATE_HANDLERS = {
  ...
  config_option_update: (_conversation, acpx, update) => {
    if (update.sessionUpdate === "config_option_update")
      applyConfigOptionsModelState(acpx, deepClone(update.configOptions));
  },
};
...
function trimConversationForRuntime(conversation) {
  ...
  if (requestUsageEntries.length > MAX_RUNTIME_REQUEST_TOKEN_USAGE)
    conversation.request_token_usage = Object.fromEntries(requestUsageEntries.slice(-100));
}
...
async function connectAndLoadSession(options) {
  ...
  logReconnectAttempt(
    record,
    storedProcessAlive,
    Boolean(record.pid) && !storedProcessAlive,
    options.verbose,
  );
...
function applyDesiredConfigOptionToRecord(record, configId, value) {
  if (configId === modelStateFromConfigOptions(record.acpx?.config_options)?.configId)
...
var AcpRuntimeManager = class {
  ...
  buildRuntimeTurnController(task, turn) {
    return {
      ...
      setSessionConfigOption: async (configId, value) => {
        return (await task.state.activeController.setResolvedSessionConfigOption(configId, value))
          .response;
      },
  ...
  async finalizeRuntimeTurn(task, turn) {
    ...
    if (!(turn ? await this.finalizeRuntimeTurnRecord(turn) : false))
    ...
    const result = await this.withRuntimeControlSession(
      ...
      async ({ client, sessionId, record: connectedRecord }) => {
        ...
        applyConfigOptionsToRecord(
          connectedRecord,
          await client.setSessionConfigOption(sessionId, configId, value),
        );
...
function readOpenClawGatewayInstanceIdFromRecord(record) {
  ...
  return typeof openclawGatewayInstanceId === "string"
    ? openclawGatewayInstanceId.trim() || void 0
    : void 0;
}
...
const CODEX_ACP_REASONING_EFFORTS = /* @__PURE__ */ new Set(["low", "medium", "high", "xhigh"]);
...
async function ensureDelegateSessionWithModelFallback(delegate, input) {
  try {
    ...
    return await delegate.ensureSession(
...
var AcpxRuntime = class {
  ...
  async withCodexWrapperDiagnostics(params) {
    try {
      ...
      throw new AcpRuntimeError$1(params.fallbackCode, `Internal error: ${stderrTail}`, {
        cause: error,
      });
    }
  }
  ...
  async *runTurn(input) {
    ...
    try {
      ...
      const stderrTail = await this.readCodexTurnFailureStderr({ handle: input.handle });
      if (!stderrTail) throw error;
      throw new AcpRuntimeError$1("ACP_TURN_FAILED", `Internal error: ${stderrTail}`, {
        cause: error,
      });
    }
  }
  startTurn(input) {
    const readCodexTurnFailureStderr = () =>
      this.readCodexTurnFailureStderr({ handle: input.handle });
