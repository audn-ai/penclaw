...
import {
  a as isDiscordRateLimitResponseBody,
  o as summarizeDiscordResponseBody,
...
import {
  n as formatDiscordApprovalDisplayValue,
  t as DISCORD_APPROVAL_ALLOWED_MENTIONS,
} from "./approval-message-safety-C-8BL6Kv.js";
import {
...
} from "./channel-access-C12aDZ0p.js";
...
import { t as chunkItems } from "./chunk-items-BB3E9W12.js";
...
import { t as resolveDirectStatusReplyForSession } from "./command-status-runtime-QRrcAA2O.js";
import {
...
import {
  ...
  v as encodeCustomIdComponent,
} from "./components-Dpcn95GZ.js";
import { t as resolveDiscordConversationIdentity } from "./conversation-identity-B8ctpwXe.js";
import { n as isDangerousNameMatchingEnabled } from "./dangerous-name-matching-Z6nhxFXz.js";
import {
  Bt as GatewayDispatchEvents,
  C as User,
  Ft as ApplicationCommandOptionType,
  H as StringSelectMenu,
  Ht as GatewayOpcodes,
  It as ButtonStyle,
  K as BaseMessageInteractiveComponent,
  Lt as ChannelType,
  N as Container,
  O as Modal,
  Rt as ComponentType,
  ...
  U as TextDisplay,
  V as Separator,
  Vt as GatewayIntentBits,
  a as PresenceUpdateListener,
  b as CommandWithSubcommands,
  ...
  y as Command,
  z as Row,
  zt as GatewayCloseCodes,
} from "./discord-bgbU23Hx.js";
...
import { n as wrapFetchWithAbortSignal } from "./fetch-B_uBy4WI.js";
...
import { i as resolveHumanDelayConfig } from "./identity-CceraXdn.js";
import "./fetch-runtime-DvrgqKVd.js";
...
} from "./inbound-context-DxpTwKP9.js";
...
import { n as logError, t as logDebug } from "./logger-BegyOJlO.js";
import { t as resolveMarkdownTableMode } from "./markdown-tables-pi6oiKnf.js";
import { n as formatMention } from "./mentions-DPSZx-Fv.js";
import { u as resolveDiscordChannelInfo } from "./message-utils-prcoeLfb.js";
...
import {
  ...
  t as MAX_DATE_TIMESTAMP_MS,
  v as parseStrictInteger,
  y as parseStrictNonNegativeInteger,
} from "./number-coercion-CJQ8TR--.js";
import {
  n as authorizeDiscordVoiceIngress,
  r as resolveDiscordVoiceEnabled,
  t as resolveDiscordVoiceOwnerAccess,
} from "./owner-access-GsoYFpqc.js";
import { t as buildPairingReply } from "./pairing-messages-DwLSgJ3x.js";
import "./reply-chunking-aJZ4F_cw.js";
import { c as upsertChannelPairingRequest } from "./pairing-store-C4j5pnVS.js";
...
import "./conversation-binding-runtime-BRZS3fzi.js";
...
import { o as resolveEnvHttpProxyAgentOptions } from "./proxy-env-5nRGQyQD.js";
import "./runtime-fetch-BONfpP9A.js";
import "./native-command-config-runtime-C9X5Lkfv.js";
import "./native-command-registry-C6VaSi3n.js";
import "./state-paths-DU65nx5N.js";
import {
  a as buildGuildLabel,
  i as buildDirectLabel,
  t as deliverDiscordReply,
} from "./reply-delivery-CcImzyZT.js";
import { n as dispatchReplyWithDispatcher } from "./reply-dispatch-runtime-vBmhhmkr.js";
import {
...
import { i as chunkDiscordTextWithMode } from "./reply-reference-CeynpHZu.js";
...
import { i as resolveAgentRoute } from "./resolve-route-oiOq6bK9.js";
...
} from "./route-resolution-BHnTdoDR.js";
import { n as createNonExitingRuntime } from "./runtime-Bz6o617W.js";
import { n as captureWsEvent, t as captureHttpExchange } from "./runtime-eYTs6VkC.js";
...
import {
  B as DISCORD_REST_TIMEOUT_MS,
  G as registerGateway,
  H as withValidatedDiscordProxy,
  I as createDiscordRestClient,
  K as unregisterGateway,
  V as validateDiscordProxyUrl,
} from "./send.shared-Ciix19qx.js";
import { t as resolveDiscordSenderIdentity } from "./sender-identity-dZhk6b7k.js";
import {
...
import { t as readChannelIngressStoreAllowFromForDmPolicy } from "./store-allow-from-Cwd6erbH.js";
...
import { j as resolveChannelStreamingBlockEnabled } from "./streaming-D_YE0SPS.js";
import {
...
import { a as isThreadArchived } from "./thread-bindings.discord-api-3Lk5SIht.js";
import { s as resolveDiscordThreadParentInfo } from "./threading-UQvctHcU.js";
import { d as withAbortTimeout, l as raceWithTimeout } from "./timeouts-Bj-qI9VL.js";
...
import { s as resolveProfilesUnavailableReason } from "./usage-BZgGbRoq.js";
import { o as isProfileInCooldown, t as clearExpiredCooldowns } from "./usage-state-BQLO_Yob.js";
...
async function resolveDiscordNativeAutocompleteAuthorized(params) {
  ...
  if (params.skipCommandOwnerAllowFrom !== true) {
    ...
    if (
      ...
      !(commandOwnerOk || commandOwnerAllowAll) &&
...
async function readDiscordModelPickerRecentModels(params) {
  ...
  try {
    const recent = (await openPreferenceStore(params.env).entries())
      .map((entry) => ({
        key: entry.key,
        value: sanitizeStoredPreferenceEntry(entry.value),
      }))
      .filter((entry) => entry.value?.scopeKey === key)
...
async function recordDiscordModelPickerRecentModel(params) {
  ...
  try {
    ...
    const timestamp = nextPreferenceTimestamp(
      (await store.entries())
...
const loadModelsProviderRuntime = createLazyRuntimeModule(
  () => import("./plugin-sdk/models-provider-runtime.js"),
);
...
function computeAlphaBuckets(sortedItems) {
  ...
  if (sortedItems.every((item) => firstLetter(item) === firstLetter(firstItem)))
...
function findProviderBucketLocation(data, provider) {
  ...
  if (idx < 0) return;
  const containing = computeAlphaBuckets(sorted).find(
    (bucket) => idx >= bucket.start && idx < bucket.end,
  );
...
function resolveDiscordModelPickerPageForModel(params) {
  ...
  const pageSize = clampPageSize(params.pageSize, 25, 25);
  const containingBucket = computeAlphaBuckets(sorted).find(
...
function buildModelRows(params) {
  ...
  if (!((params.modelPage.buckets?.length ?? 0) > 1))
    rows.push(
      new Row([
        createModelSelect({
          customId: buildDiscordModelPickerCustomId({
      ...
      new Row([
        createModelSelect({
          customId: buildDiscordModelPickerCustomId({
            ...
            ...(params.pendingModelIndex === void 0 && activeModelBucket
...
function resolveDiscordModelPickerRuntimeForProvider(params) {
  ...
  if (runtime === "auto" || runtime === "default")
    return params.allowResetRuntime ? runtime : void 0;
...
async function handleDiscordModelPickerInteraction(params) {
  ...
  if (!deferredUpdate) {
    if (
      (await params.safeInteractionCall("model picker defer", () => interaction.acknowledge())) ===
      null
    )
      return;
    deferredUpdate = true;
    ...
    await updatePicker(
      toDiscordModelPickerMessagePayload(
        renderDiscordModelPickerModelsView({
          ...
          providerBucket:
            parsed.providerBucket ?? findProviderBucketId(pickerData, selectedProvider),
          currentModel: currentModelRef,
          currentRuntime,
          quickModels,
        }),
    ...
    const autocomplete =
      ...
      (resolvedChoices.length > 0 &&
        ...
        ? async (interaction) => {
            ...
            const choiceCatalog =
              command.key === "think" ? await loadModelCatalog({ cacheOnly: true }) : void 0;
            ...
            if (command.key === "think" && !choiceCatalog?.length)
              loadModelCatalog({ config: currentCfg });
          }
        : void 0;
    const choices =
...
async function dispatchDiscordCommandInteraction(params) {
  ...
  if (pluginMatch && commandName !== "status") {
    ...
    const pluginCommandAgentId =
      (isThreadChannel ? threadBindings.getByThreadId(rawChannelId)?.agentId : void 0) ||
...
var GatewayIdentifyLimiter = class {
  ...
  async wait(params) {
    ...
    const nextAllowedAt =
      state === void 0 ? now : clockMovedBackward ? now + IDENTIFY_WINDOW_MS : state.nextAllowedAt;
...
var DiscordGatewayVoiceStateCache = class {
  ...
  listVoiceChannelStates(guildId, channelId) {
    ...
    for (const state of states.values())
      if (state.channel_id === channelId) result.push({ ...state });
    return result;
  }
    ...
    switch (payload.op) {
      case GatewayOpcodes.Hello: {
        ...
        if (resumeState)
          ...
          this.identifyWithConcurrency(sourceSocket).catch((error) => {
            this.emitter.emit(
              "error",
  ...
  scheduleReconnect(options) {
    ...
    if (this.reconnectAttempts > (this.options.reconnect?.maxAttempts ?? 50)) {
      ...
      this.emitter.emit(
        "error",
        /* @__PURE__ */ new Error(
          `Max reconnect attempts (${maxAttempts}) reached${options.closeCode !== void 0 ? ` after close code ${options.closeCode}` : ""}`,
        ),
      );
      return;
    }
...
function shouldLogDiscordGatewayTransportClose(params) {
  return (
    params.code === DISCORD_GATEWAY_POLICY_VIOLATION_CLOSE_CODE ||
    (params.code !== 1e3 && params.code !== 1001) ||
    params.reason.length > 0 ||
    params.lastError !== void 0
  );
}
  ...
  class OpenClawGatewayPlugin extends GatewayPlugin {
    ...
    createWebSocket(url) {
      ...
      socket.on?.("error", (error) => {
        ...
        params.runtime?.log?.(
          warn(
            formatDiscordGatewayTransportErrorLog({
              flowId: wsFlowId,
...
function formatDiscordDeployErrorMessage(err) {
  ...
  if (timeoutMs !== void 0)
    timing.push(
      `timeout=${formatDurationSeconds(timeoutMs, { decimals: timeoutMs >= 1e3 ? 1 : 0 })}`,
    );
  if (requestMs !== void 0)
    timing.push(
      `observed=${formatDurationSeconds(requestMs, { decimals: requestMs >= 1e3 ? 1 : 0 })}`,
    );
...
const loadConversationRuntime$1 = createLazyRuntimeModule(
  () => import("./agent-components.runtime.js"),
);
const loadTypingRuntime = createLazyRuntimeModule(() => import("./typing-CKSyqn-x.js"));
...
const loadComponentsRuntime = createLazyRuntimeModule(() => import("./components-Jivz2zH7.js"));
...
async function runDiscordGatewayLifecycle(params) {
  ...
  try {
    ...
    if (params.voiceManager) {
      ...
      const { setDiscordTranscriptsVoiceManager } =
        await import("./transcripts-source-CszH3stu.js");
...
function createDiscordRestFetchWithDispatcher(dispatcher) {
  return wrapFetchWithAbortSignal((input, init) =>
    fetchWithRuntimeDispatcher(input, {
      ...init,
      dispatcher,
    }).then((response) => {
      captureHttpExchange({
...
async function monitorDiscordProvider(opts = {}) {
  ...
  const applicationId =
    ...
    (await fetchDiscordApplicationId(token, 4e3, discordRestFetch));
