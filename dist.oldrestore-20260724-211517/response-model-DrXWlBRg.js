 ...
 */
const InboundPolicySchema = _enum(["disabled", "allowlist", "pairing", "open"]);
...
const VoiceCallTailscaleConfigSchema = object({
   ...
   */
  mode: _enum(["off", "serve", "funnel"]).default("off"),
...
const VoiceCallTunnelConfigSchema = object({
   ...
   */
  provider: _enum(["none", "ngrok", "tailscale-serve", "tailscale-funnel"]).default("none"),
...
const VoiceCallRealtimeProvidersConfigSchema = record(
  string(),
  record(string(), unknown()),
).default({});
const VoiceCallRealtimeToolPolicySchema = _enum(REALTIME_VOICE_AGENT_CONSULT_TOOL_POLICIES);
const VoiceCallRealtimeConsultPolicySchema = _enum(["auto", "substantive", "always"]);
...
const VoiceCallRealtimeAgentContextConfigSchema = object({
  ...
  files: array(string().min(1)).default(["SOUL.md", "IDENTITY.md", "USER.md"]),
})
  .strict()
  .default({
    enabled: false,
...
const VoiceCallStreamingProvidersConfigSchema = record(
  string(),
  record(string(), unknown()),
).default({});
...
const VoiceCallConfigSchema = object({
  ...
  provider: _enum(["telnyx", "twilio", "plivo", "mock"]).optional(),
...
//#endregion
export {
  resolveTwilioAuthToken as a,
  resolveVoiceCallNumberRouteKeyForCall as c,
  normalizeVoiceCallConfig as i,
  resolveVoiceCallSessionKey as l,
  resolveCallAgentId as n,
  resolveVoiceCallConfig as o,
  VoiceCallConfigSchema as r,
  resolveVoiceCallEffectiveConfig as s,
  resolveVoiceResponseModel as t,
  validateProviderConfig as u,
};
