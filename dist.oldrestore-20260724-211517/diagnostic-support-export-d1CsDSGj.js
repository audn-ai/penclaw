...
import {
  a as readLatestDiagnosticStabilityBundleSync,
  i as readDiagnosticStabilityBundleFileSync,
...
import { o as parseConfigJson5 } from "./io-B3ne6NxF.js";
...
const LOGTAPE_META_STRING_FIELDS = /* @__PURE__ */ new Map([
  ["logLevelName", "level"],
  ["name", "logger"],
]);
...
function isBonjourLogRecord(record) {
  if (
    ["subsystem", "logger", "module", "pluginId", "component"].some((field) =>
      logString(record, field)?.toLowerCase().includes("bonjour"),
    )
  )
    return true;
