...
function resolveShellBasename(shellPath, platform = process.platform) {
  ...
  return normalizeLowercaseStringOrEmpty(
    (winBasename.length < platformBasename.length ? winBasename : platformBasename).replace(
      /\.(?:exe|cmd|bat)$/i,
      "",
    ),
  );
...
function resolveCompletionProfilePath(shell, options = {}) {
  ...
  if (platform === "win32") {
    ...
    const profileDirectory =
      (shellPath ? resolveShellBasename(shellPath, platform) : "") === "powershell"
        ? "WindowsPowerShell"
        : "PowerShell";
...
//#endregion
export {
  installCompletion as a,
  resolveCompletionCachePath as c,
  usesSlowDynamicCompletion as d,
  formatCompletionReloadCommand as i,
  resolveCompletionProfilePath as l,
  COMPLETION_SKIP_PLUGIN_COMMANDS_ENV as n,
  isCompletionInstalled as o,
  completionCacheExists as r,
  isCompletionShell as s,
  COMPLETION_SHELLS as t,
  resolveShellFromEnv as u,
};
