...
import {
  ...
  i as isPathInside,
  m as assertNoUnsafeDeviceReadPath,
  o as isPathRelativeEscape,
  r as isNotFoundPathError,
  s as isSymlinkOpenError,
  x as safeFileURLToPath,
  y as isWindowsDriveLetterPath,
...
import {
  a as root,
  d as expandHomePrefix,
  l as getFsSafeTestHooks,
...
async function ensureDirectoryWithinRoot(params) {
  ...
  try {
    ...
    for (const segment of relative.split(path.sep).filter(Boolean)) {
      ...
      if (!isPathInside(rootReal, await fs$1.realpath(current)))
        return invalidPath(params.scopeLabel);
    }
    ...
    try {
      ...
      if (allowMissingFallback && err instanceof FsSafeError && err.code === "not-found") {
        ...
        try {
          ...
          if (!isPathInside(rootRealPath, await fs$1.realpath(existingPath)))
            return invalidPath(params.scopeLabel);
        } catch {
          return invalidPath(params.scopeLabel);
        }
...
//#endregion
export {
  resolveAbsolutePathForRead as C,
  findExistingAncestor as S,
  resolveLocalPathFromRootsSync as _,
  walkDirectory as a,
  assertAbsolutePathInput as b,
  withTimeout as c,
  resolveExistingPathsWithinRoot as d,
  resolvePathWithinRoot as f,
  readLocalFileFromRoots as g,
  resolveWritablePathWithinRoot as h,
  writeFileWithinRoot as i,
  movePathToTrash as l,
  resolveStrictExistingPathsWithinRoot as m,
  readFileWithinRoot as n,
  walkDirectorySync as o,
  resolvePathsWithinRoot as p,
  writeExternalFileWithinRoot as r,
  readSecureFile as s,
  ensureAbsoluteDirectory as t,
  pathScope as u,
  pathExists as v,
  resolveAbsolutePathForWrite as w,
  canonicalPathFromExistingAncestor as x,
  pathExistsSync as y,
};
