...
import {
  ...
  i as resolveDefaultPluginGitDir,
...
import { i as relinkOpenClawPeerDependenciesInManagedNpmRoot } from "./plugin-peer-link-CMrc53yA.js";
...
function resolveNpmManagedProjectInstall(params) {
  if (
    ...
    return null;
  const segments = path
    .relative(path.resolve(params.projectsDir), path.resolve(params.installPath))
    .split(path.sep)
    .filter(Boolean);
  ...
  if (removal.cleanup?.kind === "npm" && npmCleanupManifestExists) {
    ...
    try {
      ...
      if (
        ...
        if (cleanup.code !== 0)
          warnings.push(
            `Failed to prune managed peer dependencies after uninstalling ${removal.cleanup.packageName}: ${cleanup.stderr.trim() || cleanup.stdout.trim() || `npm exited with code ${cleanup.code}`}`,
          );
      }
    } catch (error) {
      warnings.push(
...
//#endregion
export {
  planPluginUninstall as a,
  formatUninstallSlotResetPreview as i,
  applyPluginUninstallDirectoryRemoval as n,
  removePluginFromConfig as o,
  formatUninstallActionLabels as r,
  resolveUninstallChannelConfigKeys as s,
