...
} from "../../runtime-entry-DYy8VHuV.js";
...
import { t as MAX_TCP_PORT } from "../../tcp-port-1cXcKObm.js";
...
function parseVoiceCallIntOption(raw, optionName, opts) {
  ...
  if (parsed === void 0 || parsed < min || (opts?.max !== void 0 && parsed > opts.max))
...
function registerVoiceCallCli(params) {
  ...
  root
    ...
    .action(async (options) => {
      ...
      if (options.callId) {
        writeStdoutJson(
          (await rt.manager.getCallFromMemoryOrStore(options.callId)) ?? { found: false },
        );
        return;
      }
      writeStdoutJson({
  ...
  root
    ...
    .action(async (options) => {
      ...
      const tailSqliteHistory = async (initialLimit) => {
        ...
        for (;;) {
          try {
            for (const call of await getCallHistoryFromStore(path.dirname(file), 1e3))
              printCall(call);
          } catch {}
          await sleep(pollMs);
        }
