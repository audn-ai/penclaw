...
function commandDetail(result) {
  ...
  if (!raw) return "";
  const redacted = truncateUtf16Safe(
    redactSensitiveText(raw).replace(/\s+/gu, " "),
...
function commandError(action, result) {
  if (result.termination !== "exit")
    return /* @__PURE__ */ new Error(
      `Crabbox ${action} did not exit normally (${result.termination})`,
    );
  const exitCode = result.code === null ? "unknown" : String(result.code);
  return /* @__PURE__ */ new Error(
    `Crabbox ${action} failed with exit code ${exitCode}${commandDetail(result)}`,
  );
}
...
function parseInspectJson(stdout) {
  ...
  if (value.ready !== void 0 && typeof value.ready !== "boolean")
...
function createCrabboxWorkerProvider(dependencies = {}) {
  ...
  return {
    ...
    async provision(profile, operationId) {
      ...
      if (!allocatedId) throw new Error("Crabbox warmup did not return a lease id");
