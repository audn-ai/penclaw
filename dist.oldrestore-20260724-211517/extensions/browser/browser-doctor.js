...
import { n as listSystemProfiles } from "../../system-profiles-Bhl6WpeJ.js";
...
function detectLegacyClawdBrowserProfileResidue(cfg, deps) {
  ...
  if (resolved.defaultProfile !== "openclaw" || defaultProfile?.driver === "existing-session")
    return null;
  return {
    legacyProfileDir,
    legacyUserDataDir,
...
async function noteChromeMcpBrowserReadiness(cfg, deps) {
  ...
  if (platform === "darwin") {
    ...
    const systemProfileCount = ["chrome", "brave", "edge", "chromium"].reduce(
      (count, browser) =>
        count +
...
async function maybeArchiveLegacyClawdBrowserProfileResidue(cfg, deps) {
  ...
  try {
    ...
    return {
      changes: [],
      warnings: [
        `Legacy clawd browser profile residue could not be archived: ${error instanceof Error ? error.message : String(error)}`,
      ],
    };
  }
}
