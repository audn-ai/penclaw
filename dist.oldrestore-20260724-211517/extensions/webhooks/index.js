...
import "../../api-CXu7MDoz.js";
import "../../runtime-api-D1geySoj.js";
//#region extensions/webhooks/src/config.ts
const secretRefSchema = object({
  source: _enum(["env", "file", "exec"]),
...
const webhooksPluginConfigSchema = object({
  routes: record(string().trim().min(1), webhookRouteConfigSchema).default({}),
}).strict();
...
const webhookActionSchema = discriminatedUnion("action", [
  strictObject({
    ...
    status: _enum(["queued", "running", "waiting", "blocked"]).optional(),
    notifyPolicy: _enum(["done_only", "state_changes", "silent"]).optional(),
...
function describeWebhookOutcome(params) {
  switch (params.action.action) {
    case "create_flow":
      return mapCreateFlowStatus(params.result);
    case "set_waiting":
    case "resume_flow":
    case "finish_flow":
    case "fail_flow":
