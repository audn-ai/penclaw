...
import { r as listEnabledFeishuAccounts } from "../../accounts-upJPqv8I.js";
import {
...
import { n as createFeishuClient } from "../../client-BCjLU970.js";
...
import {
  a as toolExecutionErrorResult,
  f as registerFeishuChatTools,
  g as resolveToolsConfig,
  h as resolveFeishuToolAccount,
  m as resolveAnyEnabledFeishuToolsConfig,
  n as registerFeishuDriveTools,
  o as unknownToolActionResult,
  p as createFeishuToolClient,
...
} from "../../subagent-hooks-DEESAIn6.js";
import {
  n as getFeishuThreadBindingManager,
  r as testing,
  t as createFeishuThreadBindingManager,
} from "../../thread-bindings-DNvmknT8.js";
...
function normalizeConvertedBlockTree(blocks, firstLevelIds) {
  ...
  const uniqueRootIds = uniqueStrings(
    (firstLevelIds && firstLevelIds.length > 0 ? firstLevelIds : inferredTopLevelIds).filter(
...
async function downloadImage(url, maxBytes) {
  return (
    await getFeishuRuntime().channel.media.readRemoteMediaBuffer({
      url,
      maxBytes,
    })
  ).buffer;
        ...
        return {
          ...
          async execute(_toolCallId, params) {
            ...
            try {
              ...
              switch (p.action) {
                case "read":
                  return jsonResult(await readDoc(client, p.doc_token));
                case "write":
                  return jsonResult(
                    await writeDoc(
                      client,
                ...
                case "list_blocks":
                  return jsonResult(await listBlocks(client, p.doc_token));
                case "get_block":
                  return jsonResult(await getBlock(client, p.doc_token, p.block_id));
                case "update_block":
                  return jsonResult(await updateBlock(client, p.doc_token, p.block_id, p.content));
                case "delete_block":
                  return jsonResult(await deleteBlock(client, p.doc_token, p.block_id));
                case "create_table":
                  return jsonResult(
                    await createTable(
                      client,
                ...
                case "color_text":
                  return jsonResult(
                    await updateColorText(client, p.doc_token, p.block_id, p.content),
                  );
                case "insert_table_row":
                  return jsonResult(
                    await insertTableRow(client, p.doc_token, p.block_id, p.row_index),
                  );
...
const BitableFieldValueSchema = Type.Unsafe({
