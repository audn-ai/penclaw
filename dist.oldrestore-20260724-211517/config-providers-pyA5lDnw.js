import {
  c as readProviderHeaders,
  d as resolveHermesProviderApiKeyEnv,
  f as resolveHermesProviderBaseUrlEnv,
  i as readEnvReference,
  l as resolveHermesEndpointApiKeyEnv,
  n as collectProviderModels,
  o as readProviderApiKeyEnv,
  p as resolveProviderApi,
  s as readProviderBaseUrl,
  t as HERMES_TRANSPORTS,
  u as resolveHermesImplicitBaseUrl,
} from "./config-provider-contract-g5C6wKyT.js";
import {
  a as isRecord,
  c as readString,
  f as sanitizeName,
  n as childRecord,
} from "./helpers-DtCn6Wrs.js";
import { s as createMigrationManualItem } from "./migration-nGWjmzKy.js";
import {
  a as resolveHermesConfiguredProviderId,
  n as normalizeHermesCustomProviderId,
} from "./model-CFxX3h6A.js";
...
function collectHermesProviderSecretBindings(config, env = {}) {
  ...
  const selectedEnv =
    (model ? readProviderApiKeyEnv(model) : void 0) ??
...
function providerManualItems(config, env, includeSecrets) {
  ...
  if (isRecord(config.model)) {
    ...
    if (readString(config.model.base_url) ?? readString(config.model.baseUrl))
      entries.push({
        id: provider
...
//#endregion
export {
  providerManualItems as i,
  collectHermesProviderSecretBindings as n,
  collectHermesProviders as r,
  addSelectedModelToProvider as t,
};
