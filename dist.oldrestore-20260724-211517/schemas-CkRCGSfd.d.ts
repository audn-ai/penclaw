...
interface StandardSchemaWithJSONProps<Input = unknown, Output = Input>
...
type $replace<Meta, S extends $ZodType> = Meta extends $output
  ? output<S>
  : Meta extends $input
    ? input<S>
...
type Processor<T extends $ZodType = $ZodType> = (
  schema: T,
 ...
 */
declare const createToJSONSchemaMethod: <T extends $ZodType>(
  schema: T,
  processors?: Record<string, Processor>,
...
>[0];
declare const createStandardJSONSchemaMethod: <T extends $ZodType>(
  schema: T,
  io: "input" | "output",
  processors?: Record<string, Processor>,
...
declare function assignProp<T extends object, K extends PropertyKey>(
  target: T,
  prop: K,
...
declare function promiseAllObject<T extends object>(
  promisesObj: T,
...
declare function clone<T extends $ZodType>(
  inst: T,
  def?: T["_zod"]["def"],
  params?: {
    parent: boolean;
...
type Normalize<T> = T extends undefined
  ...
  : T extends Record<any, any>
    ? Flatten<
        { [k in keyof Omit$1<T, "error" | "message">]: T[k] } & ("error" extends keyof T
          ? {
...
type CleanKey<T extends PropertyKey> = T extends `?${infer K}`
  ? K
  : T extends `${infer K}?`
    ? K
    : T;
...
declare function partial(
  Class: SchemaClass<$ZodOptional> | null,
  schema: $ZodObject,
  mask: object | undefined,
): any;
declare function required(
  Class: SchemaClass<$ZodNonOptional>,
  schema: $ZodObject,
  mask: object | undefined,
): any;
...
declare function unwrapMessage(
  message:
    | string
    | {
        message: string;
...
declare function finalizeIssue(
  iss: $ZodRawIssue,
...
interface _$ZodTypeInternals {
  ...
  processJSONSchema?:
    | ((ctx: ToJSONSchemaContext, json: BaseSchema, params: ProcessParams) => void)
    | undefined;
...
interface _$ZodType<T extends $ZodTypeInternals = $ZodTypeInternals> extends $ZodType<
  T["output"],
  T["input"],
  T
> {}
...
interface $ZodStringFormatDef<Format extends string = string>
  extends $ZodStringDef, $ZodCheckStringFormatDef<Format> {}
interface $ZodStringFormatInternals<Format extends string = string>
  extends $ZodStringInternals<string>, $ZodCheckStringFormatInternals {
...
interface $ZodNumberFormatInternals
  extends $ZodNumberInternals<number>, $ZodCheckNumberFormatInternals {
...
interface $ZodBigIntFormatInternals
  extends $ZodBigIntInternals<bigint>, $ZodCheckBigIntFormatInternals {
...
interface $ZodArray<T extends SomeType = $ZodType> extends $ZodType<
  any,
  any,
  $ZodArrayInternals<T>
> {}
...
> = string extends keyof T
  ? IsAny<T[keyof T]> extends true
    ...
    : Record<string, output<T[keyof T]>>
...
interface $ZodObjectInternals<
  /** @ts-ignore Cast variance */ out Shape extends $ZodShape = $ZodShape,
...
interface $ZodObject<
  /** @ts-ignore Cast variance */ out Shape extends Readonly<$ZodShape> = Readonly<$ZodShape>,
...
interface $ZodXorInternals<
  T extends readonly SomeType[] = readonly $ZodType[],
> extends $ZodUnionInternals<T> {}
interface $ZodXor<T extends readonly SomeType[] = readonly $ZodType[]> extends $ZodType<
  any,
  any,
  $ZodXorInternals<T>
> {
...
type $InferZodRecordOutput<
  ...
  Value extends SomeType = $ZodType,
> = Key extends $partial
  ? Partial<Record<output<Key>, output<Value>>>
...
interface $ZodMapInternals<
  Key extends SomeType = $ZodType,
  Value extends SomeType = $ZodType,
> extends $ZodTypeInternals<Map<output<Key>, output<Value>>, Map<input<Key>, input<Value>>> {
...
interface $ZodSetInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  Set<output<T>>,
  Set<input<T>>
> {
  def: $ZodSetDef<T>;
...
interface $ZodEnumInternals<
  /** @ts-ignore Cast variance */ out T extends EnumLike = EnumLike,
...
interface $ZodOptionalInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  output<T> | undefined,
  input<T> | undefined
> {
...
interface $ZodNullableInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  output<T> | null,
  input<T> | null
> {
...
interface $ZodDefaultInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
...
interface $ZodNonOptionalInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  NoUndefined<output<T>>,
  NoUndefined<input<T>>
> {
...
interface $ZodSuccessInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  boolean,
  input<T>
> {
  def: $ZodSuccessDef<T>;
...
interface $ZodCatchInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  output<T>,
  input<T>
> {
  def: $ZodCatchDef<T>;
...
interface $ZodPipeInternals<
  A extends SomeType = $ZodType,
  B extends SomeType = $ZodType,
> extends $ZodTypeInternals<output<B>, input<A>> {
  def: $ZodPipeDef<A, B>;
  isst: never;
...
interface $ZodPreprocessInternals<B extends SomeType = $ZodType> extends $ZodPipeInternals<
  $ZodTransform,
  B
> {
...
interface $ZodReadonlyInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  MakeReadonly<output<T>>,
...
interface $ZodTemplateLiteralInternals<Template extends string = string> extends $ZodTypeInternals<
  Template,
  Template
> {
  pattern: RegExp;
...
> = Suffix extends LiteralPart
  ...
  : Suffix extends $ZodType
    ? `${Template}${output<Suffix> extends infer T extends LiteralPart ? UndefinedToEmptyString<T> : never}`
    : never;
...
type ConvertPartsToStringTuple<Parts extends $ZodTemplateLiteralPart[]> = {
  [K in keyof Parts]: Parts[K] extends LiteralPart
    ...
    : Parts[K] extends $ZodType
      ? `${output<Parts[K]> extends infer T extends LiteralPart ? UndefinedToEmptyString<T> : never}`
      : never;
};
type ToTemplateLiteral<Parts extends $ZodTemplateLiteralPart[]> = ConcatenateTupleOfStrings<
...
interface $ZodFunctionInternals<
...
> extends $ZodType<any, any, $ZodFunctionInternals<Args, Returns>> {
  ...
  input<const Items extends TupleItems, const Rest extends $ZodFunctionOut = $ZodFunctionOut>(
    args: Items,
    rest?: Rest,
...
interface $ZodPromiseInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  Promise<output<T>>,
  MaybeAsync<input<T>>
> {
  def: $ZodPromiseDef<T>;
...
interface $ZodLazyInternals<T extends SomeType = $ZodType> extends $ZodTypeInternals<
  output<T>,
  input<T>
> {
  def: $ZodLazyDef<T>;
...
interface $ZodCustomInternals<O = unknown, I = unknown>
  extends $ZodTypeInternals<O, I>, $ZodCheckInternals<O> {
  def: $ZodCustomDef;
  issc: $ZodIssue;
  isst: never;
  bag: LoosePartial<{
...
type $ZodStringFormatTypes =
  ...
  | $ZodCustomStringFormat<HashFormat>
...
interface $ZodCheckMaxLengthInternals<
  T extends HasLength = HasLength,
> extends $ZodCheckInternals<T> {
...
interface $ZodCheckMinLengthInternals<
  T extends HasLength = HasLength,
> extends $ZodCheckInternals<T> {
...
interface $ZodCheckLengthEqualsInternals<
  T extends HasLength = HasLength,
> extends $ZodCheckInternals<T> {
...
type RawIssue$1<T extends $ZodIssueBase> = T extends any
  ? Flatten<
      MakePartial<T, "message" | "path"> & {
        /** The input data */ readonly input: unknown /** The schema or check that originated this issue. */;
        readonly inst?:
          | $ZodType
          | $ZodCheck /** If `true`, Zod will continue executing checks/refinements after this issue. */;
...
declare function flattenError<T, U>(
  error: $ZodError<T>,
...
declare function formatError<T, U>(
  error: $ZodError<T>,
  mapper?: (issue: $ZodIssue) => U,
...
declare function treeifyError<T, U>(
  error: $ZodError<T>,
  mapper?: (issue: $ZodIssue) => U,
): $ZodErrorTree<T, U>;
...
declare function $constructor<T extends ZodTrait, D = T["_zod"]["def"]>(
  name: string,
...
type $ParseAsync = <T extends $ZodType>(
  schema: T,
  value: unknown,
  _ctx?: ParseContext<$ZodIssue>,
  _params?: {
...
type $SafeParse = <T extends $ZodType>(
  schema: T,
  value: unknown,
  _ctx?: ParseContext<$ZodIssue>,
...
type $EncodeAsync = <T extends $ZodType>(
  schema: T,
  value: output<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => Promise<input<T>>;
...
type $DecodeAsync = <T extends $ZodType>(
  schema: T,
  value: input<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => Promise<output<T>>;
...
declare function time(args: { precision?: number | null }): RegExp;
...
type Params<
  T extends $ZodType | $ZodCheck,
  IssueTypes extends $ZodIssueBase,
      ...
      Omit<T["_zod"]["def"], OmitKeys> &
        ([IssueTypes] extends [never]
          ...
          : {
              error?:
                | string
                | $ZodErrorMap<IssueTypes>
                | undefined /** @deprecated This parameter is deprecated. Use `error` instead. */;
...
type CheckParams<
  T extends $ZodCheck = $ZodCheck, // & { _issc: never },
...
type CheckTypeParams<
  T extends $ZodType & $ZodCheck = $ZodType & $ZodCheck,
...
declare function _string<T extends $ZodString>(
  Class: SchemaClass<T>,
  params?: string | $ZodStringParams,
): T;
declare function _coercedString<T extends $ZodString>(
  Class: SchemaClass<T>,
  params?: string | $ZodStringParams,
): T;
type $ZodStringFormatParams = CheckTypeParams<
  $ZodStringFormat,
...
declare function _email<T extends $ZodEmail>(
  Class: SchemaClass<T>,
...
declare function _guid<T extends $ZodGUID>(
  Class: SchemaClass<T>,
...
declare function _uuid<T extends $ZodUUID>(
  Class: SchemaClass<T>,
...
declare function _uuidv4<T extends $ZodUUID>(
  Class: SchemaClass<T>,
...
declare function _uuidv6<T extends $ZodUUID>(
  Class: SchemaClass<T>,
...
declare function _uuidv7<T extends $ZodUUID>(
  Class: SchemaClass<T>,
...
declare function _url<T extends $ZodURL>(
  Class: SchemaClass<T>,
...
declare function _emoji<T extends $ZodEmoji>(
  Class: SchemaClass<T>,
...
declare function _nanoid<T extends $ZodNanoID>(
  Class: SchemaClass<T>,
 ...
 * See https://github.com/paralleldrive/cuid.
 */
declare function _cuid<T extends $ZodCUID>(
  Class: SchemaClass<T>,
...
declare function _cuid2<T extends $ZodCUID2>(
  Class: SchemaClass<T>,
...
declare function _ulid<T extends $ZodULID>(
  Class: SchemaClass<T>,
...
declare function _xid<T extends $ZodXID>(
  Class: SchemaClass<T>,
...
declare function _ksuid<T extends $ZodKSUID>(
  Class: SchemaClass<T>,
...
declare function _ipv4<T extends $ZodIPv4>(
  Class: SchemaClass<T>,
...
declare function _ipv6<T extends $ZodIPv6>(
  Class: SchemaClass<T>,
...
declare function _mac<T extends $ZodMAC>(
  Class: SchemaClass<T>,
...
declare function _cidrv4<T extends $ZodCIDRv4>(
  Class: SchemaClass<T>,
...
declare function _cidrv6<T extends $ZodCIDRv6>(
  Class: SchemaClass<T>,
...
declare function _base64<T extends $ZodBase64>(
  Class: SchemaClass<T>,
...
declare function _base64url<T extends $ZodBase64URL>(
  Class: SchemaClass<T>,
...
declare function _e164<T extends $ZodE164>(
  Class: SchemaClass<T>,
...
declare function _jwt<T extends $ZodJWT>(
  Class: SchemaClass<T>,
...
declare function _isoDateTime<T extends $ZodISODateTime>(
  Class: SchemaClass<T>,
...
declare function _isoDate<T extends $ZodISODate>(
  Class: SchemaClass<T>,
...
declare function _isoTime<T extends $ZodISOTime>(
  Class: SchemaClass<T>,
...
declare function _isoDuration<T extends $ZodISODuration>(
  Class: SchemaClass<T>,
...
declare function _number<T extends $ZodNumber>(
  Class: SchemaClass<T>,
  params?: string | $ZodNumberParams,
): T;
declare function _coercedNumber<T extends $ZodNumber>(
  Class: SchemaClass<T>,
  params?: string | $ZodNumberParams,
): T;
declare function _int<T extends $ZodNumberFormat>(
  Class: SchemaClass<T>,
  params?: string | $ZodCheckNumberFormatParams,
): T;
declare function _float32<T extends $ZodNumberFormat>(
  Class: SchemaClass<T>,
  params?: string | $ZodCheckNumberFormatParams,
): T;
declare function _float64<T extends $ZodNumberFormat>(
  Class: SchemaClass<T>,
  params?: string | $ZodCheckNumberFormatParams,
): T;
declare function _int32<T extends $ZodNumberFormat>(
  Class: SchemaClass<T>,
  params?: string | $ZodCheckNumberFormatParams,
): T;
declare function _uint32<T extends $ZodNumberFormat>(
  Class: SchemaClass<T>,
  params?: string | $ZodCheckNumberFormatParams,
): T;
type $ZodBooleanParams = TypeParams<$ZodBoolean<boolean>, "coerce">;
declare function _boolean<T extends $ZodBoolean>(
  Class: SchemaClass<T>,
  params?: string | $ZodBooleanParams,
): T;
declare function _coercedBoolean<T extends $ZodBoolean>(
  Class: SchemaClass<T>,
  params?: string | $ZodBooleanParams,
): T;
...
declare function _bigint<T extends $ZodBigInt>(
  Class: SchemaClass<T>,
  params?: string | $ZodBigIntParams,
): T;
declare function _coercedBigint<T extends $ZodBigInt>(
  Class: SchemaClass<T>,
  params?: string | $ZodBigIntParams,
): T;
declare function _int64<T extends $ZodBigIntFormat>(
  Class: SchemaClass<T>,
  params?: string | $ZodBigIntFormatParams,
): T;
declare function _uint64<T extends $ZodBigIntFormat>(
  Class: SchemaClass<T>,
  params?: string | $ZodBigIntFormatParams,
): T;
type $ZodSymbolParams = TypeParams<$ZodSymbol>;
declare function _symbol<T extends $ZodSymbol>(
  Class: SchemaClass<T>,
  params?: string | $ZodSymbolParams,
): T;
type $ZodUndefinedParams = TypeParams<$ZodUndefined>;
declare function _undefined$1<T extends $ZodUndefined>(
  Class: SchemaClass<T>,
  params?: string | $ZodUndefinedParams,
): T;
type $ZodNullParams = TypeParams<$ZodNull>;
declare function _null$1<T extends $ZodNull>(
  Class: SchemaClass<T>,
...
declare function _never<T extends $ZodNever>(
  Class: SchemaClass<T>,
  params?: string | $ZodNeverParams,
): T;
type $ZodVoidParams = TypeParams<$ZodVoid>;
declare function _void$1<T extends $ZodVoid>(
  Class: SchemaClass<T>,
  params?: string | $ZodVoidParams,
): T;
type $ZodDateParams = TypeParams<$ZodDate, "coerce">;
declare function _date<T extends $ZodDate>(
  Class: SchemaClass<T>,
  params?: string | $ZodDateParams,
): T;
declare function _coercedDate<T extends $ZodDate>(
  Class: SchemaClass<T>,
  params?: string | $ZodDateParams,
): T;
...
declare function _array<T extends $ZodType>(
  Class: SchemaClass<$ZodArray>,
  element: T,
...
declare function _union<const T extends readonly $ZodObject[]>(
  Class: SchemaClass<$ZodUnion>,
  options: T,
  params?: string | $ZodUnionParams,
): $ZodUnion<T>;
type $ZodXorParams = TypeParams<$ZodXor, "options">;
declare function _xor<const T extends readonly $ZodObject[]>(
  Class: SchemaClass<$ZodXor>,
  options: T,
  params?: string | $ZodXorParams,
): $ZodXor<T>;
interface $ZodTypeDiscriminableInternals<Disc extends string = string> extends $ZodTypeInternals<
  unknown,
  { [K in Disc]?: unknown }
> {
  propValues: PropValues;
...
declare function _discriminatedUnion<
  Types extends [$ZodTypeDiscriminable<Disc>, ...$ZodTypeDiscriminable<Disc>[]],
  Disc extends string,
>(
  Class: SchemaClass<$ZodDiscriminatedUnion>,
  discriminator: Disc,
  options: Types,
...
declare function _intersection<T extends $ZodObject, U extends $ZodObject>(
  Class: SchemaClass<$ZodIntersection>,
  left: T,
  right: U,
): $ZodIntersection<T, U>;
type $ZodTupleParams = TypeParams<$ZodTuple, "items" | "rest">;
declare function _tuple<T extends readonly [$ZodType, ...$ZodType[]]>(
  Class: SchemaClass<$ZodTuple>,
  items: T,
  params?: string | $ZodTupleParams,
): $ZodTuple<T, null>;
declare function _tuple<T extends readonly [$ZodType, ...$ZodType[]], Rest extends $ZodType>(
  Class: SchemaClass<$ZodTuple>,
  items: T,
  rest: Rest,
  params?: string | $ZodTupleParams,
...
declare function _record<Key extends $ZodRecordKey, Value extends $ZodObject>(
  Class: SchemaClass<$ZodRecord>,
  keyType: Key,
  valueType: Value,
  params?: string | $ZodRecordParams,
): $ZodRecord<Key, Value>;
type $ZodMapParams = TypeParams<$ZodMap, "keyType" | "valueType">;
declare function _map<Key extends $ZodObject, Value extends $ZodObject>(
  Class: SchemaClass<$ZodMap>,
  keyType: Key,
  valueType: Value,
  params?: string | $ZodMapParams,
): $ZodMap<Key, Value>;
type $ZodSetParams = TypeParams<$ZodSet, "valueType">;
declare function _set<Value extends $ZodObject>(
  Class: SchemaClass<$ZodSet>,
  valueType: Value,
  params?: string | $ZodSetParams,
): $ZodSet<Value>;
type $ZodEnumParams = TypeParams<$ZodEnum, "entries">;
declare function _enum$1<const T extends string[]>(
  Class: SchemaClass<$ZodEnum>,
  values: T,
  params?: string | $ZodEnumParams,
): $ZodEnum<ToEnum<T[number]>>;
declare function _enum$1<T extends EnumLike>(
  Class: SchemaClass<$ZodEnum>,
  entries: T,
  params?: string | $ZodEnumParams,
): $ZodEnum<T>;
 ...
 * ```
 */
declare function _nativeEnum<T extends EnumLike>(
  Class: SchemaClass<$ZodEnum>,
  entries: T,
  params?: string | $ZodEnumParams,
): $ZodEnum<T>;
type $ZodLiteralParams = TypeParams<$ZodLiteral, "values">;
declare function _literal<const T extends Array<Literal>>(
  Class: SchemaClass<$ZodLiteral>,
  value: T,
  params?: string | $ZodLiteralParams,
): $ZodLiteral<T[number]>;
declare function _literal<const T extends Literal>(
  Class: SchemaClass<$ZodLiteral>,
  value: T,
  params?: string | $ZodLiteralParams,
): $ZodLiteral<T>;
...
declare function _transform<I = unknown, O = I>(
  Class: SchemaClass<$ZodTransform>,
  fn: (input: I, ctx?: ParsePayload) => O,
): $ZodTransform<Awaited<O>, I>;
type $ZodOptionalParams = TypeParams<$ZodOptional, "innerType">;
declare function _optional<T extends $ZodObject>(
  Class: SchemaClass<$ZodOptional>,
  innerType: T,
): $ZodOptional<T>;
type $ZodNullableParams = TypeParams<$ZodNullable, "innerType">;
declare function _nullable<T extends $ZodObject>(
  Class: SchemaClass<$ZodNullable>,
  innerType: T,
): $ZodNullable<T>;
type $ZodDefaultParams = TypeParams<$ZodDefault, "innerType" | "defaultValue">;
declare function _default$1<T extends $ZodObject>(
  Class: SchemaClass<$ZodDefault>,
  innerType: T,
  defaultValue: NoUndefined<output<T>> | (() => NoUndefined<output<T>>),
): $ZodDefault<T>;
type $ZodNonOptionalParams = TypeParams<$ZodNonOptional, "innerType">;
declare function _nonoptional<T extends $ZodObject>(
  Class: SchemaClass<$ZodNonOptional>,
  innerType: T,
  params?: string | $ZodNonOptionalParams,
): $ZodNonOptional<T>;
type $ZodSuccessParams = TypeParams<$ZodSuccess, "innerType">;
declare function _success<T extends $ZodObject>(
  Class: SchemaClass<$ZodSuccess>,
  innerType: T,
): $ZodSuccess<T>;
type $ZodCatchParams = TypeParams<$ZodCatch, "innerType" | "catchValue">;
declare function _catch$1<T extends $ZodObject>(
  Class: SchemaClass<$ZodCatch>,
  innerType: T,
  catchValue: output<T> | ((ctx: $ZodCatchCtx) => output<T>),
): $ZodCatch<T>;
type $ZodPipeParams = TypeParams<$ZodPipe, "in" | "out">;
declare function _pipe<
  const A extends $ZodType,
  B extends $ZodType<unknown, output<A>> = $ZodType<unknown, output<A>>,
>(Class: SchemaClass<$ZodPipe>, in_: A, out: B | $ZodType<unknown, output<A>>): $ZodPipe<A, B>;
type $ZodReadonlyParams = TypeParams<$ZodReadonly, "innerType">;
declare function _readonly<T extends $ZodObject>(
  Class: SchemaClass<$ZodReadonly>,
  innerType: T,
): $ZodReadonly<T>;
type $ZodTemplateLiteralParams = TypeParams<$ZodTemplateLiteral, "parts">;
declare function _templateLiteral<const Parts extends $ZodTemplateLiteralPart[]>(
  Class: SchemaClass<$ZodTemplateLiteral>,
  parts: Parts,
...
declare function _lazy<T extends $ZodType>(
  Class: SchemaClass<$ZodLazy>,
  getter: () => T,
): $ZodLazy<T>;
type $ZodPromiseParams = TypeParams<$ZodPromise, "innerType">;
declare function _promise<T extends $ZodObject>(
  Class: SchemaClass<$ZodPromise>,
  innerType: T,
): $ZodPromise<T>;
type $ZodCustomParams = CheckTypeParams<$ZodCustom, "fn">;
declare function _custom<O = unknown, I = O>(
  Class: SchemaClass<$ZodCustom>,
  fn: (data: O) => unknown,
  _params: string | $ZodCustomParams | undefined,
): $ZodCustom<O, I>;
declare function _refine<O = unknown, I = O>(
  Class: SchemaClass<$ZodCustom>,
  fn: (data: O) => unknown,
  _params: string | $ZodCustomParams | undefined,
): $ZodCustom<O, I>;
type $ZodSuperRefineIssue<T extends $ZodIssueBase = $ZodIssue> = T extends any
  ? RawIssue<T>
  : never;
type RawIssue<T extends $ZodIssueBase> = T extends any
  ? Flatten<
      MakePartial<T, "message" | "path"> & {
        /** The schema or check that originated this issue. */ readonly inst?:
          | $ZodType
          | $ZodCheck /** If `true`, Zod will execute subsequent checks/refinements instead of immediately aborting */;
        readonly continue?: boolean | undefined;
      } & Record<string, unknown>
    >
  : never;
...
declare function _stringFormat<Format extends string>(
  Class: typeof $ZodCustomStringFormat,
  format: Format,
  fnOrRegex: ((arg: string) => MaybeAsync<unknown>) | RegExp,
...
declare function toJSONSchema<T extends $ZodType>(
  schema: T,
...
declare namespace index_d_exports {
  export {
    ...
    File,
    GlobalMeta,
    json_schema_d_exports as JSONSchema,
    JSONSchemaGenerator,
    JSONSchemaGeneratorParams,
    JSONSchemaMeta,
    NEVER,
    ...
    ZodStandardSchemaWithJSON$1 as ZodStandardSchemaWithJSON,
    _$ZodType,
    _$ZodTypeInternals,
    _any,
    _array,
    ...
    decode$1 as decode,
    decodeAsync$1 as decodeAsync,
    describe$1 as describe,
    encode$1 as encode,
    encodeAsync$1 as encodeAsync,
    extractDefs,
    finalize,
    flattenError,
    formatError,
    ...
    isValidBase64URL,
    isValidJWT,
    index_d_exports$1 as locales,
    meta$1 as meta,
    output,
    parse$1 as parse,
    parseAsync$1 as parseAsync,
    prettifyError,
    process,
    regexes_d_exports as regexes,
    registry,
    safeDecode$1 as safeDecode,
    safeDecodeAsync$1 as safeDecodeAsync,
    safeEncode$1 as safeEncode,
    safeEncodeAsync$1 as safeEncodeAsync,
    safeParse$1 as safeParse,
    safeParseAsync$1 as safeParseAsync,
    toDotPath,
    toJSONSchema,
    treeifyError,
    util_d_exports as util,
    version,
  };
}
//#endregion
...
declare const parse: <T extends $ZodType>(
  schema: T,
  value: unknown,
  _ctx?: ParseContext<$ZodIssue>,
  _params?: {
...
declare const parseAsync: <T extends $ZodType>(
  schema: T,
  value: unknown,
  _ctx?: ParseContext<$ZodIssue>,
  _params?: {
...
declare const safeParse: <T extends $ZodType>(
  schema: T,
  value: unknown,
  _ctx?: ParseContext<$ZodIssue>,
) => ZodSafeParseResult<output<T>>;
declare const safeParseAsync: <T extends $ZodType>(
  schema: T,
  value: unknown,
  _ctx?: ParseContext<$ZodIssue>,
) => Promise<ZodSafeParseResult<output<T>>>;
declare const encode: <T extends $ZodType>(
  schema: T,
  value: output<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => input<T>;
declare const decode: <T extends $ZodType>(
  schema: T,
  value: input<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => output<T>;
declare const encodeAsync: <T extends $ZodType>(
  schema: T,
  value: output<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => Promise<input<T>>;
declare const decodeAsync: <T extends $ZodType>(
  schema: T,
  value: input<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => Promise<output<T>>;
declare const safeEncode: <T extends $ZodType>(
  schema: T,
  value: output<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => ZodSafeParseResult<input<T>>;
declare const safeDecode: <T extends $ZodType>(
  schema: T,
  value: input<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => ZodSafeParseResult<output<T>>;
declare const safeEncodeAsync: <T extends $ZodType>(
  schema: T,
  value: output<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => Promise<ZodSafeParseResult<input<T>>>;
declare const safeDecodeAsync: <T extends $ZodType>(
  schema: T,
  value: input<T>,
  _ctx?: ParseContext<$ZodIssue>,
) => Promise<ZodSafeParseResult<output<T>>>;
...
> extends $ZodType<Output, Input, Internals> {
  ...
  register<R extends $ZodRegistry>(
    ...
    ...meta: this extends R["_schema"]
      ? undefined extends R["_meta"]
        ? [$replace<R["_meta"], this>?]
        : [$replace<R["_meta"], this>]
      : ["Incompatible schema"]
  ): this;
  brand<T extends PropertyKey = PropertyKey, Dir extends "in" | "out" | "inout" = "out">(
    value?: T,
  ): PropertyKey extends T ? this : $ZodBranded<this, T, Dir>;
  ...
  ): Promise<ZodSafeParseResult<output<this>>>;
  spa: (
    data: unknown,
    params?: ParseContext<$ZodIssue>,
  ) => Promise<ZodSafeParseResult<output<this>>>;
  ...
  safeEncodeAsync(
    data: output<this>,
    params?: ParseContext<$ZodIssue>,
  ): Promise<ZodSafeParseResult<input<this>>>;
  safeDecodeAsync(
    data: input<this>,
    params?: ParseContext<$ZodIssue>,
  ): Promise<ZodSafeParseResult<output<this>>>;
  refine<Ch extends (arg: output<this>) => unknown | Promise<unknown>>(
    check: Ch,
    params?: string | $ZodCustomParams,
  ): Ch extends ((arg: any) => arg is infer R) ? this & ZodType<R, input<this>> : this;
  superRefine(
    refinement: (arg: output<this>, ctx: $RefinementCtx<output<this>>) => void | Promise<void>,
    params?: $ZodSuperRefineParams,
  ): this;
  ...
  transform<NewOut>(
    transform: (arg: output<this>, ctx: $RefinementCtx<output<this>>) => NewOut | Promise<NewOut>,
  ): ZodPipe<this, ZodTransform<Awaited<NewOut>, output<this>>>;
  catch(def: output<this>): ZodCatch<this>;
  catch(def: (ctx: $ZodCatchCtx) => output<this>): ZodCatch<this>;
  pipe<T extends $ZodType<any, output<this>>>(
    target: T | $ZodType<any, output<this>>,
  ): ZodPipe<this, T>;
...
interface _ZodType<out Internals extends $ZodTypeInternals = $ZodTypeInternals> extends ZodType<
  any,
  any,
  Internals
> {}
...
interface ZodStringFormat<Format extends string = string> extends _ZodString<
  $ZodStringFormatInternals<Format>
> {}
declare const ZodStringFormat: $constructor<ZodStringFormat>;
...
interface ZodCustomStringFormat<Format extends string = string>
  extends ZodStringFormat<Format>, $ZodCustomStringFormat<Format> {
  _zod: $ZodCustomStringFormatInternals<Format>;
  "~standard": ZodStandardSchemaWithJSON<this>;
}
declare const ZodCustomStringFormat: $constructor<ZodCustomStringFormat>;
declare function stringFormat<Format extends string>(
  format: Format,
  fnOrRegex: ((arg: string) => MaybeAsync<unknown>) | RegExp,
  _params?: string | $ZodStringFormatParams,
): ZodCustomStringFormat<Format>;
...
declare function hash<Alg extends HashAlgorithm, Enc extends HashEncoding = "hex">(
  alg: Alg,
  params?: {
    enc?: Enc;
  } & $ZodStringFormatParams,
...
interface ZodArray<T extends SomeType = $ZodType>
  extends _ZodType<$ZodArrayInternals<T>>, $ZodArray<T> {
  element: T;
  min(minLength: number, params?: string | $ZodCheckMinLengthParams): this;
  nonempty(params?: string | $ZodCheckMinLengthParams): this;
  max(maxLength: number, params?: string | $ZodCheckMaxLengthParams): this;
...
declare const ZodArray: $constructor<ZodArray>;
declare function array<T extends SomeType>(
  element: T,
  params?: string | $ZodArrayParams,
): ZodArray<T>;
...
interface ZodObject<
  /** @ts-ignore Cast variance */ out Shape extends $ZodShape = $ZodLooseShape,
  out Config extends $ZodObjectConfig = $strip,
>
  extends _ZodType<$ZodObjectInternals<Shape, Config>>, $ZodObject<Shape, Config> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  shape: Shape;
  ...
  ): ZodObject<Extend<Shape, Writeable<U>>, Config>;
  /**
...
declare const ZodObject: $constructor<ZodObject>;
declare function object<T extends $ZodLooseShape = Partial<Record<never, SomeType>>>(
  shape?: T,
  params?: string | $ZodObjectParams,
): ZodObject<Writeable<T>, $strip>;
declare function strictObject<T extends $ZodLooseShape>(
  shape: T,
  params?: string | $ZodObjectParams,
): ZodObject<Writeable<T>, $strict>;
declare function looseObject<T extends $ZodLooseShape>(
  shape: T,
  params?: string | $ZodObjectParams,
): ZodObject<Writeable<T>, $loose>;
interface ZodUnion<T extends readonly SomeType[] = readonly $ZodType[]>
  extends _ZodType<$ZodUnionInternals<T>>, $ZodUnion<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  options: T;
}
declare const ZodUnion: $constructor<ZodUnion>;
declare function union<const T extends readonly SomeType[]>(
  options: T,
  params?: string | $ZodUnionParams,
): ZodUnion<T>;
interface ZodXor<T extends readonly SomeType[] = readonly $ZodType[]>
  extends _ZodType<$ZodXorInternals<T>>, $ZodXor<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  options: T;
}
declare const ZodXor: $constructor<ZodXor>;
...
declare function xor<const T extends readonly SomeType[]>(
  options: T,
  params?: string | $ZodXorParams,
): ZodXor<T>;
interface ZodDiscriminatedUnion<
  ...
  Disc extends string = string,
>
  extends ZodUnion<Options>, $ZodDiscriminatedUnion<Options, Disc> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  _zod: $ZodDiscriminatedUnionInternals<Options, Disc>;
  def: $ZodDiscriminatedUnionDef<Options, Disc>;
}
...
interface ZodIntersection<A extends SomeType = $ZodType, B extends SomeType = $ZodType>
  extends _ZodType<$ZodIntersectionInternals<A, B>>, $ZodIntersection<A, B> {
  "~standard": ZodStandardSchemaWithJSON<this>;
}
declare const ZodIntersection: $constructor<ZodIntersection>;
declare function intersection<T extends SomeType, U extends SomeType>(
  left: T,
  right: U,
): ZodIntersection<T, U>;
interface ZodTuple<
  ...
  Rest extends SomeType | null = $ZodType | null,
>
  extends _ZodType<$ZodTupleInternals<T, Rest>>, $ZodTuple<T, Rest> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  rest<Rest extends SomeType = $ZodType>(rest: Rest): ZodTuple<T, Rest>;
}
declare const ZodTuple: $constructor<ZodTuple>;
declare function tuple<T extends readonly [SomeType, ...SomeType[]]>(
  items: T,
  params?: string | $ZodTupleParams,
): ZodTuple<T, null>;
declare function tuple<T extends readonly [SomeType, ...SomeType[]], Rest extends SomeType>(
  items: T,
  rest: Rest,
  params?: string | $ZodTupleParams,
): ZodTuple<T, Rest>;
declare function tuple(items: [], params?: string | $ZodTupleParams): ZodTuple<[], null>;
interface ZodRecord<Key extends $ZodRecordKey = $ZodRecordKey, Value extends SomeType = $ZodType>
  extends _ZodType<$ZodRecordInternals<Key, Value>>, $ZodRecord<Key, Value> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  keyType: Key;
  valueType: Value;
}
declare const ZodRecord: $constructor<ZodRecord>;
declare function record<Key extends $ZodRecordKey, Value extends SomeType>(
  keyType: Key,
  valueType: Value,
  params?: string | $ZodRecordParams,
): ZodRecord<Key, Value>;
declare function partialRecord<Key extends $ZodRecordKey, Value extends SomeType>(
  keyType: Key,
  valueType: Value,
  params?: string | $ZodRecordParams,
): ZodRecord<Key & $partial, Value>;
declare function looseRecord<Key extends $ZodRecordKey, Value extends SomeType>(
  keyType: Key,
  valueType: Value,
  params?: string | $ZodRecordParams,
): ZodRecord<Key, Value>;
interface ZodMap<Key extends SomeType = $ZodType, Value extends SomeType = $ZodType>
  extends _ZodType<$ZodMapInternals<Key, Value>>, $ZodMap<Key, Value> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  keyType: Key;
  valueType: Value;
...
declare const ZodMap: $constructor<ZodMap>;
declare function map<Key extends SomeType, Value extends SomeType>(
  keyType: Key,
  valueType: Value,
  params?: string | $ZodMapParams,
): ZodMap<Key, Value>;
...
declare const ZodSet: $constructor<ZodSet>;
declare function set<Value extends SomeType>(
  valueType: Value,
  params?: string | $ZodSetParams,
): ZodSet<Value>;
interface ZodEnum</** @ts-ignore Cast variance */ out T extends EnumLike = EnumLike>
  extends _ZodType<$ZodEnumInternals<T>>, $ZodEnum<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  enum: T;
  ...
  extract<const U extends readonly (keyof T)[]>(
    values: U,
    params?: string | $ZodEnumParams,
  ): ZodEnum<Flatten<Pick<T, U[number]>>>;
  exclude<const U extends readonly (keyof T)[]>(
    values: U,
    params?: string | $ZodEnumParams,
  ): ZodEnum<Flatten<Omit<T, U[number]>>>;
}
declare const ZodEnum: $constructor<ZodEnum>;
...
declare function _enum<const T extends EnumLike>(
  entries: T,
  params?: string | $ZodEnumParams,
): ZodEnum<T>;
/** @deprecated This API has been merged into `z.enum()`. Use `z.enum()` instead.
 ...
 * ```
 */
declare function nativeEnum<T extends EnumLike>(
  entries: T,
  params?: string | $ZodEnumParams,
): ZodEnum<T>;
interface ZodLiteral<T extends Literal = Literal>
  extends _ZodType<$ZodLiteralInternals<T>>, $ZodLiteral<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  values: Set<T>;
...
declare const ZodLiteral: $constructor<ZodLiteral>;
declare function literal<const T extends ReadonlyArray<Literal>>(
  value: T,
  params?: string | $ZodLiteralParams,
): ZodLiteral<T[number]>;
declare function literal<const T extends Literal>(
  value: T,
  params?: string | $ZodLiteralParams,
): ZodLiteral<T>;
...
interface ZodTransform<O = unknown, I = unknown>
  extends _ZodType<$ZodTransformInternals<O, I>>, $ZodTransform<O, I> {
  "~standard": ZodStandardSchemaWithJSON<this>;
}
declare const ZodTransform: $constructor<ZodTransform>;
declare function transform<I = unknown, O = I>(
  fn: (input: I, ctx: $RefinementCtx) => O,
): ZodTransform<Awaited<O>, I>;
interface ZodOptional<T extends SomeType = $ZodType>
  extends _ZodType<$ZodOptionalInternals<T>>, $ZodOptional<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodOptional: $constructor<ZodOptional>;
declare function optional<T extends SomeType>(innerType: T): ZodOptional<T>;
interface ZodExactOptional<T extends SomeType = $ZodType>
  extends _ZodType<$ZodExactOptionalInternals<T>>, $ZodExactOptional<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodExactOptional: $constructor<ZodExactOptional>;
...
interface ZodNullable<T extends SomeType = $ZodType>
  extends _ZodType<$ZodNullableInternals<T>>, $ZodNullable<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodNullable: $constructor<ZodNullable>;
...
interface ZodDefault<T extends SomeType = $ZodType>
  extends _ZodType<$ZodDefaultInternals<T>>, $ZodDefault<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
...
declare const ZodDefault: $constructor<ZodDefault>;
declare function _default<T extends SomeType>(
  innerType: T,
  defaultValue: NoUndefined<output<T>> | (() => NoUndefined<output<T>>),
): ZodDefault<T>;
interface ZodPrefault<T extends SomeType = $ZodType>
  extends _ZodType<$ZodPrefaultInternals<T>>, $ZodPrefault<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodPrefault: $constructor<ZodPrefault>;
declare function prefault<T extends SomeType>(
  innerType: T,
  defaultValue: input<T> | (() => input<T>),
): ZodPrefault<T>;
interface ZodNonOptional<T extends SomeType = $ZodType>
  extends _ZodType<$ZodNonOptionalInternals<T>>, $ZodNonOptional<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodNonOptional: $constructor<ZodNonOptional>;
declare function nonoptional<T extends SomeType>(
  innerType: T,
  params?: string | $ZodNonOptionalParams,
): ZodNonOptional<T>;
interface ZodSuccess<T extends SomeType = $ZodType>
  extends _ZodType<$ZodSuccessInternals<T>>, $ZodSuccess<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodSuccess: $constructor<ZodSuccess>;
...
interface ZodCatch<T extends SomeType = $ZodType>
  extends _ZodType<$ZodCatchInternals<T>>, $ZodCatch<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
  /** @deprecated Use `.unwrap()` instead. */
  removeCatch(): T;
...
interface ZodPipe<A extends SomeType = $ZodType, B extends SomeType = $ZodType>
  extends _ZodType<$ZodPipeInternals<A, B>>, $ZodPipe<A, B> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  in: A;
  out: B;
}
...
declare function pipe<
  ...
  B extends $ZodType<unknown, output<A>> = $ZodType<unknown, output<A>>,
>(in_: A, out: B | $ZodType<unknown, output<A>>): ZodPipe<A, B>;
interface ZodCodec<A extends SomeType = $ZodType, B extends SomeType = $ZodType>
  extends ZodPipe<A, B>, $ZodCodec<A, B> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  _zod: $ZodCodecInternals<A, B>;
  def: $ZodCodecDef<A, B>;
}
declare const ZodCodec: $constructor<ZodCodec>;
declare function codec<const A extends SomeType, B extends SomeType = $ZodType>(
  in_: A,
  out: B,
  params: {
...
declare function invertCodec<A extends SomeType, B extends SomeType>(
  codec: ZodCodec<A, B>,
): ZodCodec<B, A>;
interface ZodPreprocess<B extends SomeType = $ZodType>
  extends ZodPipe<$ZodTransform, B>, $ZodPreprocess<B> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  _zod: $ZodPreprocessInternals<B>;
  def: $ZodPreprocessDef<B>;
}
...
interface ZodReadonly<T extends SomeType = $ZodType>
  extends _ZodType<$ZodReadonlyInternals<T>>, $ZodReadonly<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodReadonly: $constructor<ZodReadonly>;
...
interface ZodTemplateLiteral<Template extends string = string>
  extends _ZodType<$ZodTemplateLiteralInternals<Template>>, $ZodTemplateLiteral<Template> {
  "~standard": ZodStandardSchemaWithJSON<this>;
}
declare const ZodTemplateLiteral: $constructor<ZodTemplateLiteral>;
declare function templateLiteral<const Parts extends $ZodTemplateLiteralPart[]>(
  parts: Parts,
  params?: string | $ZodTemplateLiteralParams,
): ZodTemplateLiteral<$PartsToTemplateLiteral<Parts>>;
interface ZodLazy<T extends SomeType = $ZodType>
  extends _ZodType<$ZodLazyInternals<T>>, $ZodLazy<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodLazy: $constructor<ZodLazy>;
declare function lazy<T extends SomeType>(getter: () => T): ZodLazy<T>;
interface ZodPromise<T extends SomeType = $ZodType>
  extends _ZodType<$ZodPromiseInternals<T>>, $ZodPromise<T> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  unwrap(): T;
}
declare const ZodPromise: $constructor<ZodPromise>;
...
interface ZodFunction<
  ...
  Returns extends $ZodFunctionOut = $ZodFunctionOut,
>
  extends _ZodType<$ZodFunctionInternals<Args, Returns>>, $ZodFunction<Args, Returns> {
  "~standard": ZodStandardSchemaWithJSON<this>;
  _def: $ZodFunctionDef<Args, Returns>;
  _input: $InferInnerFunctionType<Args, Returns>;
  _output: $InferOuterFunctionType<Args, Returns>;
...
declare function _function<
  const In extends ReadonlyArray<$ZodType>,
...
interface ZodCustom<O = unknown, I = unknown>
  extends _ZodType<$ZodCustomInternals<O, I>>, $ZodCustom<O, I> {
  "~standard": ZodStandardSchemaWithJSON<this>;
}
declare const ZodCustom: $constructor<ZodCustom>;
declare function check<O = unknown>(fn: CheckFn<O>): $ZodCheck<O>;
declare function custom<O>(
  fn?: (data: unknown) => unknown,
  _params?: string | $ZodCustomParams | undefined,
): ZodCustom<O, O>;
declare function refine<T>(
  fn: (arg: NoInfer<T>) => MaybeAsync<unknown>,
  _params?: string | $ZodCustomParams,
): $ZodCheck<T>;
declare function superRefine<T>(
  fn: (arg: T, payload: $RefinementCtx<T>) => void | Promise<void>,
  params?: $ZodSuperRefineParams,
): $ZodCheck<T>;
declare const describe: typeof describe$1;
declare const meta: typeof meta$1;
type ZodInstanceOfParams = Params<
  ZodCustom,
  $ZodIssueCustom,
...
declare function preprocess<A, U extends SomeType, B = unknown>(
...
export {
  ...
  config as Ai,
  meta as An,
  safeEncodeAsync as Ar,
  _ZodType as At,
  ZodLiteral as B,
  treeifyError as Bi,
  optional as Bn,
  $ZodBigIntParams as Br,
  array as Bt,
  ZodEnum as C,
  _trim as Ci,
  ksuid as Cn,
  ...
  _ZodDate as Dt,
  ZodFloat32 as E,
  regexes_d_exports as Ei,
  looseObject as En,
  parseAsync as Er,
  _ZodBoolean as Et,
  ZodJSONSchema as F,
  $ZodFormattedError as Fi,
  nonoptional as Fn,
  ZodIssue as Fr,
  _instanceof as Ft,
  ZodNever as G,
  $ZodISODateTimeInternals as Gi,
  promise as Gn,
  $ZodISODurationParams as Gr,
  check as Gt,
  ZodMap as H,
  $ZodBooleanInternals as Hi,
  pipe as Hn,
  $ZodDateParams as Hr,
  base64url as Ht,
  ZodJSONSchemaInternals as I,
  $ZodIssue as Ii,
  nullable as In,
  ZodRealError as Ir,
  _null as It,
  ZodNullable as J,
  $ZodNumberInternals as Ji,
  refine as Jn,
  $ZodStringParams as Jr,
  codec as Jt,
  ZodNonOptional as K,
  $ZodISODurationInternals as Ki,
  readonly as Kn,
  $ZodISOTimeParams as Kr,
  cidrv4 as Kt,
  ZodJWT as L,
  flattenError as Li,
  nullish as Ln,
  index_d_exports as Lr,
  _undefined as Lt,
  ZodInt as M,
  output as Mi,
  nanoid as Mn,
  safeParseAsync as Mr,
  _default as Mt,
  ZodInt32 as N,
  $ZodErrorMap as Ni,
  nativeEnum as Nn,
  ...
  $constructor as Oi,
  mac as On,
  safeDecodeAsync as Or,
  _ZodNumber as Ot,
  ZodIntersection as P,
  $ZodFlattenedError as Pi,
  never as Pn,
  ZodError as Pr,
  _function as Pt,
  ZodOptional as Q,
  $ZodTypeInternals as Qi,
  stringFormat as Qn,
  _gte as Qr,
  date as Qt,
  ZodKSUID as R,
  ...
  ZodVoid as St,
  ZodFile as T,
  index_d_exports$1 as Ti,
  literal as Tn,
  parse as Tr,
  _ZodBigInt as Tt,
  ZodNaN as U,
  $ZodDateInternals as Ui,
  prefault as Un,
  $ZodISODateParams as Ur,
  bigint as Ut,
  ZodMAC as V,
  $ZodBigIntInternals as Vi,
  partialRecord as Vn,
  $ZodBooleanParams as Vr,
  base64 as Vt,
  ZodNanoID as W,
  $ZodISODateInternals as Wi,
  preprocess as Wn,
  $ZodISODateTimeParams as Wr,
  boolean as Wt,
  ZodNumberFormat as X,
  $ZodStringInternals as Xi,
  strictObject as Xn,
  _endsWith as Xr,
  cuid2 as Xt,
  ZodNumber as Y,
  ...
  guid as cn,
  ulid as cr,
  ZodStringFormat as ct,
  ZodCUID as d,
  globalRegistry as da,
  _nonnegative as di,
  hostname as dn,
  url as dr,
  ZodTemplateLiteral as dt,
  $catchall as ea,
  _length as ei,
  discriminatedUnion as en,
  success as er,
  ZodPrefault as et,
  ZodCUID2 as f,
  registry as fa,
  ...
  ZodPromise as nt,
  ZodBigInt as o,
  util_d_exports as oa,
  _mime as oi,
  float32 as on,
  uint32 as or,
  ZodStandardSchemaWithJSON as ot,
  ZodCatch as p,
  JSONSchema as pa,
  _normalize as pi,
  int as pn,
  ...
  ZodTuple as pt,
  ZodNull as q,
  $ZodISOTimeInternals as qi,
  record as qn,
  $ZodNumberParams as qr,
  cidrv6 as qt,
  ZodArray as r,
  $strip as ra,
  _lte as ri,
  emoji as rn,
  templateLiteral as rr,
  ZodReadonly as rt,
  ZodBigIntFormat as s,
  $ZodRegistry as sa,
  _minLength as si,
  float64 as sn,
  uint64 as sr,
  ZodString as st,
  SafeExtendShape as t,
  $loose as ta,
  _lowercase as ti,
  e164 as tn,
  superRefine as tr,
  ...
  _size as vi,
  ipv4 as vn,
  ZodSafeParseError as vr,
  ZodUUID as vt,
  ZodExactOptional as w,
  _uppercase as wi,
  lazy as wn,
  encodeAsync as wr,
  ZodXor as wt,
