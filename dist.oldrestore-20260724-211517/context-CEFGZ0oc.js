...
import {
  A as buildPinnedWritePlan,
  C as parseSandboxStatSize,
  D as buildPinnedMkdirpPlan,
  M as runDockerSandboxShellCommand,
  O as buildPinnedRemovePlan,
  S as parseSandboxStatMtimeMs,
  i as startBrowserBridgeServer,
  k as buildPinnedRenamePlan,
  l as requireSandboxBackendFactory,
  n as stopCachedBrowserBridge,
...
import {
  ...
  u as resolveProfile,
} from "./browser-profiles-CkQaBz9y.js";
...
import {
  ...
  w as SANDBOX_DOCKER_CREATE_ARGS_EPOCH,
  x as SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
  y as SANDBOX_BROWSER_IMAGE_CONTRACT_EPOCH,
} from "./constants-Bs8JiPHi.js";
import {
  A as computeSandboxBrowserConfigHash,
  C as slugifySessionKey,
  E as formatReadOnlyWorkspaceSkillMountHashState,
  S as resolveSandboxWorkspaceLayoutPaths,
  T as appendWorkspaceMountArgs,
  a as execDocker,
  b as updateRegistry,
  ...
  k as resolveReadOnlyWorkspaceSkillMounts,
...
import {
  n as normalizeContainerPath,
  r as relativePathEscapesContainerRoot,
  t as isPathInsideContainerRoot,
} from "./path-utils-Dj-M9hwE.js";
...
import { a as resolveSandboxPath, i as resolveSandboxInputPath } from "./sandbox-paths-DxDLkOQF.js";
import { f as isSameSsrFPolicy } from "./ssrf-keFRGQLq.js";
import {
...
import { n as validateNetworkMode } from "./validate-sandbox-security-Cdz9Nkl5.js";
import {
...
async function ensureSandboxBrowserImage(image) {
  ...
  if (result.code === 0) {
    ...
    throw new Error(
      `Sandbox browser image ${image} is stale or incompatible (contract=${contract && contract !== "<no value>" ? contract : "missing"}, expected=${SANDBOX_BROWSER_IMAGE_CONTRACT_EPOCH}). Rebuild it with scripts/sandbox-browser-setup.sh.`,
    );
  }
  ...
  if (hasContainer) {
    if (noVncEnabled)
      noVncPassword =
        ...
        void 0;
    cdpAuthToken =
      (await readDockerContainerEnvVar(containerName, CDP_AUTH_TOKEN_ENV_KEY)) ?? void 0;
    if (!cdpAuthToken) {
      defaultRuntime.log(
...
function parseSandboxBindMount(spec) {
  ...
  const writable = !(optionsToken ? normalizeStringEntries(optionsToken.split(",")) : []).includes(
    "ro",
  );
  return {
...
function hasSandboxBindContainerPathAliases(binds) {
  for (const parsed of parseSandboxBindMounts(binds))
    if (parsed.hostRoot !== parsed.containerRoot) return true;
  return false;
}
...
async function resolveSandboxContext(params) {
  ...
  if (cfg.prune.idleHours !== 0 || cfg.prune.maxAgeDays !== 0)
    await (await import("./prune-BQLWsGL6.js")).maybePruneSandboxes(cfg);
  const {
    agentWorkspaceDir,
    scopeKey,
    skillsEligibility,
  ...
  const backend = await requireSandboxBackendFactory(resolvedCfg.backend)({
    ...
    skillsWorkspaceDir,
    cfg: resolvedCfg,
    ...(params.requireCurrentConfig !== void 0
...
//#endregion
export {
  hasSandboxBindReadonlyHostShadows as a,
  hasSandboxBindContainerPathAliases as i,
  resolveSandboxContext as n,
  resolveWritableSandboxBindHostRoots as o,
  buildSandboxFsMounts as r,
  ensureSandboxWorkspaceForSession as t,
};
