...
import { i as resolveEffectiveDebugProxyUrl } from "./env-CAvwY92q.js";
...
import { t as captureHttpExchange } from "./runtime-eYTs6VkC.js";
...
function shouldBypassEnvProxyForTelegramApi(env = process.env) {
  ...
  for (const entry of noProxyEntries) {
    ...
    if (parsed && (parsedHostname === void 0 || parsedPort === void 0)) continue;
    const entryHostname = normalizeLowercaseStringOrEmpty(
      (parsedHostname ?? entry).replace(/^\*?\./, ""),
    );
    const entryPort = parsedPort === void 0 ? 0 : Number.parseInt(parsedPort, 10);
...
function shouldUseTelegramTransportFallback(err) {
  ...
  return (
    FALLBACK_RETRY_ERROR_CODES.some((code) => ctx.codes.has(code)) ||
    (hasFetchFailedEnvelope && ctx.codes.size === 0)
  );
}
...
function resolveTelegramTransport(proxyFetch, options) {
  ...
  const allowStickyFallback =
    !(
      (dnsDecision.source === "config" ||
        dnsDecision.source === `env:OPENCLAW_TELEGRAM_DNS_RESULT_ORDER`) &&
  ...
  return {
    ...
    forceFallback: (reason, err) =>
      promoteStickyAttempt(
        stickyAttemptIndex + 1,
        err ?? /* @__PURE__ */ new Error("forced fallback"),
        reason,
      ),
    close,
  };
