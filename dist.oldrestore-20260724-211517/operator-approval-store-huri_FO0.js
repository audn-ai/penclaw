...
import {
  n as isApprovalResolutionRef,
  t as buildApprovalResolutionRef,
} from "./approval-resolution-ref-BMBlVd2b.js";
import {
...
import { v as validateApprovalPresentation } from "./src-CIJf1lT0.js";
...
const OPERATOR_APPROVAL_DECISIONS = /* @__PURE__ */ new Set(["allow-once", "allow-always", "deny"]);
const OPERATOR_APPROVAL_KINDS = /* @__PURE__ */ new Set(["exec", "plugin"]);
  ...
  return (
    executeSqliteQueryTakeFirstSync(
      ...
      stateDb
        ...
        .where((eb) =>
          eb.or([
            eb("approval_id", "=", params.resolutionRef),
            eb("resolution_ref", "=", params.id),
          ]),
        )
...
function matchesExpectedApprovalOwner(params) {
  return (
    (params.expectedKind === void 0 || params.row.kind === params.expectedKind) &&
    (params.runtimeEpoch === void 0 || params.row.runtime_epoch === params.runtimeEpoch)
  );
}
...
function insertOperatorApproval(params) {
  ...
  if (audienceSessionKeys.length > 64)
    throw new Error(`operator approval audience exceeds 64 sessions`);
...
function listPendingOperatorApprovals(params = {}) {
  ...
  return runOpenClawStateWriteTransaction((database) => {
    ...
    const resultLimit = Math.max(
      1,
      Math.min(params.limit ?? 1e3, OPERATOR_APPROVAL_MAX_LIST_LIMIT),
    );
    const audienceSessionKey =
      params.audienceSessionKey === void 0
        ? void 0
...
function resolveOperatorApproval(params) {
  ...
  return runOpenClawStateWriteTransaction((database) => {
    ...
    let resolveQuery = getNodeSqliteKysely(database.db)
      .updateTable("operator_approvals")
      .set({
...
function forceDenyOperatorApproval(params) {
  ...
  return runOpenClawStateWriteTransaction((database) => {
    ...
    const auditTimestampMs = clampAuditTimestamp(nowMs, record.createdAtMs);
    let denyQuery = getNodeSqliteKysely(database.db)
      .updateTable("operator_approvals")
      .set({
...
function consumeOperatorApprovalAllowOnce(params) {
  ...
  const runtimeEpoch =
    params.runtimeEpoch === void 0
      ? void 0
      : requireString(params.runtimeEpoch, "operator approval runtime epoch");
  if (params.redemptionWindowMs !== void 0 && !isValidTimestamp(params.redemptionWindowMs))
    throw new Error("operator approval redemption window must be a non-negative safe integer");
  return runOpenClawStateWriteTransaction((database) => {
    const nowMs = params.nowMs ?? Date.now();
    const redemptionThresholdMs =
      params.redemptionWindowMs === void 0 ? void 0 : nowMs - params.redemptionWindowMs;
    let row = selectOperatorApprovalRow(database, id);
    if (!row) return { outcome: "not-found" };
    if (
      !matchesExpectedApprovalOwner({
    ...
    if (redemptionThresholdMs !== void 0 && record.resolvedAtMs <= redemptionThresholdMs)
      return {
    ...
    let consumeQuery = getNodeSqliteKysely(database.db)
      .updateTable("operator_approvals")
      .set({
    ...
    if (redemptionThresholdMs !== void 0)
...
//#endregion
export {
  getOperatorApprovalDetailed as a,
  listPendingOperatorApprovals as c,
  forceDenyOperatorApproval as i,
  pruneTerminalOperatorApprovals as l,
  consumeOperatorApprovalAllowOnce as n,
  getOperatorApprovalDetailedByLocator as o,
  expireDueOperatorApprovals as r,
  insertOperatorApproval as s,
  closeOrphanedOperatorApprovals as t,
  resolveOperatorApproval as u,
};
