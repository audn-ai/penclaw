...
async function resolveCandidates(params) {
  ...
  const [
    ...
    import("./manifest-contract-runtime-CfKMS6Ve.js"),
  ...
  for (const provider of providers) {
    ...
    if (
      !resolveManifestContractRuntimePluginResolution({
        ...
        value: provider.id,
      }).pluginIds.some((pluginId) => installedIds.has(pluginId))
    )
      continue;
    try {
...
async function offerPostInstallMigrations(params) {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      params.runtime.log(
        `${candidate.provider.label} migration failed: ${formatErrorMessage(error)}. Re-run with ${formatCliCommand(`openclaw migrate ${candidate.provider.id} --dry-run`)} to inspect.`,
      );
    } finally {
