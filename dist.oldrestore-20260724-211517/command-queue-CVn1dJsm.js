...
function getQueueState() {
  ...
  for (const lane of state.lanes.values())
    for (const [index, entry] of lane.queue.entries()) {
  ...
  const readLastProgressAtMs = () => {
    ...
    try {
      ...
      diagnosticLogger.warn(
        `lane task timeout progress callback failed: lane=${lane} error="${String(err)}"`,
      );
    }
    ...
    try {
      while (state.activeTaskIds.size < state.maxConcurrent && state.queue.length > 0) {
        ...
        if (waitedMs >= entry.warnAfterMs) {
          ...
          diagnosticLogger.warn(
            `lane wait exceeded: lane=${lane} waitedMs=${waitedMs} queueAhead=${entry.queuedAheadAtEnqueue} activeAhead=${entry.activeAheadAtEnqueue} activeNow=${state.activeTaskIds.size} queueBehind=${state.queue.length}`,
          );
        }
...
//#endregion
export {
  getCommandLaneActiveTaskIds as a,
  getQueueSize as c,
  isGatewayDraining as d,
  markGatewayDraining as f,
  waitForActiveTasks as g,
  setCommandLaneConcurrency as h,
  getActiveTaskCount as i,
  getTotalQueueSize as l,
  resetCommandLane as m,
  clearCommandLane as n,
  getCommandLaneSnapshot as o,
  resetAllLanes as p,
  enqueueCommandInLane as r,
  getCommandLaneSnapshots as s,
  CommandLaneClearedError as t,
  isCommandLaneTaskTimeoutError as u,
};
