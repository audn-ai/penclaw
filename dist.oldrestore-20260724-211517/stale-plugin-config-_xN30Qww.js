...
function scanStalePluginConfigWithState(cfg, registryState, env) {
  ...
  if (entries)
    for (const rawPluginId of Object.keys(entries)) {
      ...
      if (!pluginId || knownIds.has(pluginId) || registryState.knownChannelIds.has(pluginId))
        continue;
      if (pluginId === "codex" && shouldSuppressMissingCodexPluginDiagnostics(cfg, env)) continue;
      hits.push({
        pluginId: rawPluginId,
