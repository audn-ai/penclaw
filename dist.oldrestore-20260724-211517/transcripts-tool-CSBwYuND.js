...
} from "./provider-registry-L4xiTdMt.js";
...
function resolveTranscriptsConfig(raw) {
  ...
  const maxUtterances =
    typeof config.maxUtterances === "number" && Number.isFinite(config.maxUtterances)
      ? Math.max(1, Math.min(1e4, Math.floor(config.maxUtterances)))
      : 2e3;
  return {
...
function firstSentences(utterances, limit) {
  return normalizeStringEntries(
    (
      normalizeStringEntries(utterances.map((utterance) => utterance.text))
        .join(" ")
        .match(/[^.!?]+[.!?]?/g) ?? []
    ).slice(0, limit),
  ).join(" ");
...
var TranscriptsStore = class {
  ...
  async findSessionDir(selector) {
    ...
    for (const entry of datedEntries) {
      ...
      if ((await readJsonFile(path.join(candidate, "metadata.json")))?.sessionId === selector)
...
async function stopTranscripts(params) {
  ...
  const activeCandidate =
    resolvedSession !== void 0 ? activeSessions.get(resolvedSession.sessionId) : void 0;
  const activeMatchesResolved =
    activeCandidate !== void 0 &&
    resolvedSession !== void 0 &&
  ...
  const provider = resolveSourceProvider(
    selectedActive?.providerId ?? session.source.providerId,
    params.ctx,
  );
...
async function statusTranscripts(ctx) {
  const uniqueProviders = uniqueStrings([
  ...
  return {
    ...
    async execute(_toolCallId, rawParams, signal) {
      ...
      switch (action) {
        ...
        case "stop":
          return await stopTranscripts({
            ctx,
            store,
            rawParams: params,
          });
        case "import":
          return await importTranscripts({
            ctx,
            store,
            rawParams: params,
          });
...
function createTranscriptsAutoStartService(ctx) {
    ...
    const startTask = startTranscripts({
      ...
      .catch((err) => {
        ...
        if (attempt >= AUTO_START_RETRY_ATTEMPTS) {
          ctx.logger.warn(
            `transcripts autoStart failed provider=${entry.providerId}: ${err instanceof Error ? err.message : String(err)} (check the transcripts.autoStart entry in your config)`,
          );
          return;
        }
  ...
  return {
    ...
    async stop() {
      ...
      if (!(await waitForPendingAutoStartsToSettle(pendingStarts)))
        ctx.logger.warn(
      ...
      for (const sessionId of startedSessionIds)
        await stopTranscripts({
          ...
          ctx.logger.warn(
            `transcripts autoStart stop failed session=${sessionId}: ${err instanceof Error ? err.message : String(err)}`,
          ),
        );
...
//#endregion
export {
  createTranscriptsTool as n,
  resolveTranscriptsConfig as r,
  createTranscriptsAutoStartService as t,
};
