...
} from "./diagnostic-support-bundle-CjmCKt4p.js";
import { n as redactSupportString } from "./diagnostic-support-redaction-DW6kKzSO.js";
...
import {
  o as safeTrajectorySessionFileName,
...
import {
  n as resolveTrajectoryRuntimeFile,
  t as isRegularNonSymlinkFile,
} from "./runtime-file-NaFGaXpk.js";
import { r as loadSqliteTrajectoryRuntimeEvents } from "./runtime-store.sqlite-BEvK2xDc.js";
...
async function readSessionEntries(params) {
  ...
  if (!marker) {
    const { entries, warnings, rowByEntry } = parseSessionFileEntriesWithWarnings(
      await fs.readFile(params.sessionFile, "utf8"),
    );
    return {
      entries,
...
function redactTrajectoryExportValue(value, redaction) {
  return redactTrajectoryExportObjectKeys(
    sanitizeTrajectoryExportValue(redactLocalPathValues(value, redaction)),
    redaction,
  );
}
  ...
  if (bundleRuntimeContext.systemPrompt)
    files.push(
      textSupportBundleFile(
        "system-prompt.txt",
        redactTrajectoryExportValue(bundleRuntimeContext.systemPrompt, redaction),
      ),
    );
