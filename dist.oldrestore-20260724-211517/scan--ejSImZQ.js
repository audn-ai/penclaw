...
import { i as truncate, r as pad } from "./list.format-TWH7EIb7.js";
...
import { d as updateConfig, i as formatTokenK, r as formatMs } from "./shared-DFGOIlxJ.js";
...
function normalizeCreatedAtMs(value) {
  ...
  return (
    asDateTimestampMs(value > 0xe8d4a51000 ? Math.round(value) : Math.round(value * 1e3)) ?? null
  );
}
...
async function readOpenRouterModelsJson(response, timeoutMs) {
  const buffer = await readResponseWithLimit(response, OPENROUTER_MODELS_BODY_MAX_BYTES, {
    ...
    onOverflow: ({ size, maxBytes }) =>
      /* @__PURE__ */ new Error(
        `OpenRouter /models response too large: ${size} bytes (limit ${maxBytes} bytes)`,
      ),
    onIdleTimeout: ({ chunkTimeoutMs }) =>
      /* @__PURE__ */ new Error(`OpenRouter /models response stalled after ${chunkTimeoutMs}ms`),
  });
  try {
