...
import "./model-pricing-cache-DKphTFlG.js";
    ...
    for (const model of providerConfig?.models ?? []) {
      const normalized = normalizeModelRef(normalizedProvider, model.id, {
        allowManifestNormalization:
          ...
          (options?.allowPluginNormalization === false ? false : void 0),
...
function getProviderCostIndex(providers, options) {
  ...
  const isRawLookup =
    ...
    (options.allowManifestNormalization === false || options.allowManifestNormalization === void 0);
  const isDefaultNormalizedLookup =
    options?.allowPluginNormalization !== false && options?.allowManifestNormalization === void 0;
  ...
  if (isRawLookup) {
    ...
    if (cache.rawEntries.structureFingerprint !== buildProviderCostStructureFingerprint(providers))
      cache.rawEntries = buildProviderCostIndexBundle(providers, rawOptions);
...
//#endregion
export {
  resolveModelCostConfigFingerprint as a,
  resolveModelCostConfig as i,
  formatUsd as n,
  resetUsageFormatCachesForTest as r,
  estimateUsageCost as t,
};
