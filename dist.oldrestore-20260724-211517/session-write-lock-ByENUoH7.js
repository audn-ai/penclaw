...
import { r as readGatewayProcessArgsSync } from "./gateway-processes-IA9RuZ0v.js";
import { n as MAX_TIMER_TIMEOUT_MS } from "./number-coercion-CJQ8TR--.js";
import { n as getProcessStartTime, r as isPidAlive } from "./pid-alive-FK23rkys.js";
import {
  n as SessionWriteLockTimeoutError,
  t as SessionWriteLockStaleError,
} from "./session-write-lock-error-CYOzPsPk.js";
...
function inspectLockPayload(payload, staleMs, nowMs, opts = {}) {
  ...
  const holderMaxHoldMs =
    isValidLockNumber(payload?.maxHoldMs) && payload.maxHoldMs > 0 ? payload.maxHoldMs : void 0;
  if (
...
async function shouldReportContendedLockStale(params) {
  ...
  if (lockInspectionNeedsMtimeStaleFallback(params.details))
    try {
      ...
      return (
        Math.max(0, params.nowMs - stat.mtimeMs) >
        Math.min(params.staleMs, params.orphanPayloadGraceMs)
      );
    } catch (error) {
...
//#endregion
export {
  resolveSessionLockMaxHoldFromTimeout as a,
  resolveSessionWriteLockStaleMs as c,
  resetSessionWriteLockStateForTest as i,
  testing as l,
  cleanStaleLockFiles as n,
  resolveSessionWriteLockAcquireTimeoutMs as o,
  drainSessionWriteLockStateForTest as r,
  resolveSessionWriteLockOptions as s,
