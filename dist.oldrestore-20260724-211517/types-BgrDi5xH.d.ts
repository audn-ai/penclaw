...
type ReplyThreadingPolicy = {
  /** Override implicit reply-to-current behavior for the current turn. */ implicitCurrentMessage?:
    | "default"
    | "allow"
    | "deny";
};
...
type QueuedReplyLifecycle = {
  ...
  onEnqueued?: () =>
    | boolean
    | void /** Retires this source's cancellation ownership while retaining its live identity. */;
...
type GetReplyOptions = {
  ...
  ) => void /** If false, send only the initial typing signal without periodic keepalive refreshes. */;
  ...
  bootstrapContextMode?:
    | "full"
    | "lightweight" /** If true, suppress tool error warning payloads for this run. */;
  ...
  shouldSuppressToolErrorWarnings?: () =>
    | boolean
    | undefined /** If true, run the model without OpenClaw tools for this turn. */;
  ...
  onVerboseProgressVisibility?: (
    isActive: () => boolean,
  ) => void /** Preserve source-event callback start order for stateful channel progress renderers. */;
  ...
  ) => Promise<void> | void /** Called when a tool phase starts/updates, before summary payloads are emitted. */;
  onToolStart?: (payload: {
    itemId?: string;
    toolCallId?: string;
    name?: string;
  ...
  onCommandOutput?: (payload: {
    ...
    | ProgressCallbackResult /** Called when a patch completes with a file summary. */;
...
//#endregion
export {
  SourceReplyDeliveryMode as a,
  PromptImageOrderEntry as c,
  ReplyThreadingPolicy as i,
  GetReplyOptions as n,
  TaskSuggestionDeliveryMode as o,
  PartialReplyPayload as r,
  TypingController as s,
  BlockReplyContext as t,
};
