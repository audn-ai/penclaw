...
import { t as addGatewayRunCommand } from "./run-command-DBfefxUa.js";
...
const gatewayStatusModuleLoader = createLazyImportLoader(
  () => import("./gateway-status-BzbKNH3x.js"),
);
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-D283zUOU.js"));
const bonjourDiscoveryModuleLoader = createLazyImportLoader(
  () => import("./bonjour-discovery-BBv38KQ1.js"),
);
const wideAreaDnsModuleLoader = createLazyImportLoader(() => import("./widearea-dns-CwbNL6d9.js"));
const healthStyleModuleLoader = createLazyImportLoader(
  () => import("./terminal-core/health-style.js"),
);
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-CLzxdY-E.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(
  () => import("./diagnostic-stability-bundle-B9O_oIVi.js"),
);
const supportExportModuleLoader = createLazyImportLoader(
  () => import("./diagnostic-support-export-d1CsDSGj.js"),
);
const daemonStatusGatherModuleLoader = createLazyImportLoader(
  () => import("./status.gather-CnDEbV58.js"),
);
...
async function callGatewayCli(method, opts, params) {
  return (await import("./call-BIo62rcf.js")).callGatewayCli(method, opts, params);
}
async function loadSettledCostUsageSummary(rpcOpts, params) {
  ...
  for (;;) {
    ...
    const summary = await callGatewayCli(
      "usage.cost",
      {
        ...rpcOpts,
...
function renderStabilityBundleSummary(params) {
  ...
  if (memoryPressure) {
    lines.push(
      `${colorize(rich, theme.muted, "Memory pressure:")} ${memoryPressure.level}/${memoryPressure.reason} rss=${formatBytes(memoryPressure.memory.rssBytes)} heap=${formatBytes(memoryPressure.memory.heapUsedBytes)} threshold=${formatBytes(memoryPressure.thresholdBytes)}`,
    );
    if (memoryPressure.heapStatistics)
      lines.push(
        `${colorize(rich, theme.muted, "V8 heap:")} used=${formatBytes(memoryPressure.heapStatistics.usedHeapSizeBytes)} limit=${formatBytes(memoryPressure.heapStatistics.heapSizeLimitBytes)} available=${formatBytes(memoryPressure.heapStatistics.totalAvailableSizeBytes)}`,
      );
    if (memoryPressure.activeResources) {
      const resources = Object.entries(memoryPressure.activeResources.byType)
        ...
        .join(", ");
      lines.push(
        `${colorize(rich, theme.muted, "Active resources:")} total=${memoryPressure.activeResources.total}${resources ? ` · ${resources}` : ""}`,
      );
    }
        ...
        await runGatewayCommand(
          async () => {
            const { normalizeDiagnosticStabilityQuery, selectDiagnosticStabilitySnapshot } =
              await import("./diagnostic-stability-2au80tx-.js");
            const rpcOpts = resolveGatewayRpcOptions(opts, command);
