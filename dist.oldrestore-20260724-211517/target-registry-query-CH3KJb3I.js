...
} from "./channel-contract-api-9nE4dbUf.js";
...
import { a as parseDotPath } from "./shared-CrrsbqCi.js";
...
function resolvePlanTargetAgainstRegistry(candidate) {
  ...
  if (explicitChannelId) {
    ...
    const channelTypeEntries = buildTargetTypeIndex(
...
function discoverConfigSecretTargetsByIds(config, targetIds) {
  ...
  const configuredEntries =
    ...
    Array.from(allowedTargetIds).every((targetId) => coreState.knownTargetIds.has(targetId))
      ...
      : allowedTargetIds !== null && !configHasPluginEntries(config)
        ? [
            ...coreState.openClawCompiledSecretTargets,
            ...getConfiguredChannelOpenClawTargets(config),
          ]
        : null;
  ...
  return discoverSecretTargetsFromEntries(
    ...
    resolveDiscoveryEntries({
      ...
      entriesById:
        configuredEntriesById ?? registryState?.openClawTargetsById ?? /* @__PURE__ */ new Map(),
    }),
  );
}
/**
...
//#endregion
export {
  isKnownSecretTargetId as a,
  resolveConfigSecretTargetByPath as c,
  isKnownCoreSecretTargetId as i,
  ...
  resolveSecretPlanTargetByPath as u,
};
