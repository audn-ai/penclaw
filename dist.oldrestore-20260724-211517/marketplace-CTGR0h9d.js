...
import { n as isImmutableGitCommitRef } from "./git-install-JvNiWiSz.js";
import { a as resolveOsHomeRelativePath, c as resolveUserPath } from "./home-dir-DxrrpDft.js";
import { o as installPluginFromPath } from "./install-CjCRVnj8.js";
...
function marketplaceEntryGitRef(source) {
  switch (source.kind) {
    ...
    case "git-subdir":
      return source.ref;
    case "url":
      return resolveArchiveKind(source.url) ? void 0 : normalizeGitCloneSource(source.url)?.ref;
    case "path":
      return;
  }
  throw new Error("Unsupported marketplace entry source");
...
async function cloneMarketplaceRepo(params) {
  ...
  if (refIsCommit) {
    const checkout = await runCommandWithTimeout(
      ["git", "switch", "--detach", "--", normalized.ref],
      {
        cwd: repoDir,
        timeoutMs: params.timeoutMs ?? DEFAULT_GIT_TIMEOUT_MS,
      },
...
async function loadMarketplace(params) {
  const loadMarketplaceFromManifestFile = async (paramsLocal) => {
    const parsed = parseMarketplaceManifest(
      await fs.readFile(paramsLocal.manifestPath, "utf-8"),
      paramsLocal.manifestPath,
    );
    if (!parsed.ok) {
