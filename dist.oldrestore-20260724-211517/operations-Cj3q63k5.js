...
import {
  n as resolveCrestodianAuditPath,
  t as appendCrestodianAuditEntry,
} from "./audit-UlWJ43I9.js";
...
const loadOverviewModule = async () => await import("./overview-nHPDbAb1.js");
...
function isReservedCrestodianAgentId(agentId) {
...
async function runGatewayLifecycle(operation) {
  const lifecycle = await import("./lifecycle-DBE6UamH.js");
  if (operation === "start") {
...
async function resolveChannelSetupState(deps) {
  const listPlugins =
    ...
    (await import("./setup-registry-DbvXfkoR.js")).listChannelSetupPlugins;
  const resolveEntries =
    deps?.resolveChannelSetupEntries ??
    (await import("./discovery-B2x4f8mn.js")).resolveChannelSetupEntries;
  const isConfigured =
    deps?.isChannelConfigured ??
    (await import("./channel-configured-shared-BFnxGO9C.js")).isStaticallyChannelConfigured;
  const { shouldShowChannelInSetup } = await import("./discovery-B2x4f8mn.js");
...
async function runConfigSetOperation(params) {
  ...
  const runConfigSet =
    ...
    (async (setOpts) => {
      const { runConfigSet: importedRunConfigSet } = await import("./config-cli-Dsa4PXNC.js");
...
function isInferenceRouteConfigPath(path) {
  const [root, scope, ownerOrField, field] = path
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
  const { parseConfigSetPath } = await import("./config-cli-Dsa4PXNC.js");
...
async function verifyCurrentSetupInference(runtime, deps) {
  ...
  const verification = await (
    ...
    (await import("./crestodian/setup-inference.js")).verifySetupInferenceConfig
  )({
    config: beforeConfig,
    runtime,
  });
...
async function executeSetup(operation, runtime, opts) {
  ...
  return await applyPersistentOperation({
    ...
    run: async (ctx) => {
      const applySetup =
        ctx.deps?.applySetup ?? (await import("./setup-apply-4uYmEVdf.js")).applyCrestodianSetup;
  ...
  return await applyPersistentOperation({
    ...
    run: async (ctx) => {
      ...
      const { applyCrestodianModelSelection, createCrestodianModelSelectionUpdater } =
        await import("./setup-apply-4uYmEVdf.js");
      ...
      const selectModel = await createCrestodianModelSelectionUpdater({ model: operation.model });
      const result = await mutateConfigFile({
        ...
        mutate: async (cfg) => {
          if (!sameDefaultInferenceRoute(await projectDefaultInferenceRoute(cfg), beforeRoute))
            throw new Error(
    ...
    run: async (ctx) => {
      const runPluginInstall =
        ...
        (async (spec, pluginRuntime) => {
          const { runPluginInstallCommand } = await import("./plugins-install-command-B6oG1wH0.js");
  ...
  switch (operation.kind) {
    ...
    case "plugin-list":
      await (
        ...
        (async (pluginRuntime) => {
          const { runPluginsListCommand } = await import("./plugins-list-command-nvGcnKNj.js");
    ...
    case "plugin-search":
      await (
        ...
        (async (query, pluginRuntime) => {
          const { runPluginsSearchCommand } = await import("./plugins-search-command-D56fqFbE.js");
    ...
    case "config-get": {
      ...
      const lookup = readConfigValueAtPath(
        (snapshot.valid ? (snapshot.sourceConfig ?? snapshot.config) : snapshot.sourceConfig) ?? {},
        operation.path,
      );
      if (!lookup.found) {
        runtime.log(
    ...
    case "config-schema": {
      const { buildConfigSchema, lookupConfigSchema } = await import("./schema-DWDeoke3.js");
      ...
      return await applyPersistentOperation({
        ...
        run: async (ctx) => {
          const runAgentsAdd =
            ...
            (await import("./agents.commands.add-CTUMgm0l.js")).agentsAddCommand;
          await ctx.commit(async () => {
            await runAgentsAdd(
              {
    ...
    case "doctor":
      await (opts.deps?.runDoctor ?? (await import("./doctor-RCwy0WEl.js")).doctorCommand)(
        runtime,
        { nonInteractive: true },
      );
      return { applied: false };
    case "doctor-fix":
      ...
      return { applied: false };
    case "status": {
      const { statusCommand } = await import("./status.command-YH9YR_xQ.js");
      await statusCommand({ timeoutMs: 1e4 }, runtime);
      return { applied: false };
    }
    case "health": {
      const { healthCommand } = await import("./health-D283zUOU.js");
    ...
    case "gateway-restart":
      return await applyPersistentOperation({
        ...
        run: async (ctx) => {
          ...
          if ((await ctx.commit(runGatewayRestart)) === false)
    ...
    case "open-tui": {
      ...
      const result = await (opts.deps?.runTui ?? (await import("./tui-BHinJXPT.js")).runTui)({
        local: true,
        session,
        deliver: false,
        historyLimit: 200,
