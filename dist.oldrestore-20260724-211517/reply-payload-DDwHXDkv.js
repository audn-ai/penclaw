...
function normalizeOutboundLocation(value, label = "location") {
  ...
  for (const unsupportedField of ["source", "isLive", "caption"])
    if (raw[unsupportedField] !== void 0)
...
async function sendPayloadWithChunkedTextAndMedia(params) {
  ...
  const [firstChunk, ...remainingChunks] =
    limit && params.chunker ? params.chunker(text, limit) : [text];
...
export {
  ...
  normalizeOutboundLocation as D,
  formatLocationText as E,
  toLocationContext as O,
  buildMediaPayload as S,
  normalizeOutboundReplyPayload$1 as T,
