...
import { n as shouldIncludeHook } from "./config-CzBjCjbr.js";
    ...
    for (const entry of eligible)
      try {
        ...
        if (unknownEvents.length > 0)
          log.warn(
            `Hook '${safeLogValue(entry.hook.name)}' subscribes to event${unknownEvents.length === 1 ? "" : "s"} ${unknownEvents.map((event) => safeLogValue(event)).join(", ")} not emitted by OpenClaw core — likely a typo; unless a plugin emits it, the hook never fires. Known events: https://docs.openclaw.ai/automation/hooks`,
          );
  ...
  for (const handlerConfig of handlers)
    try {
      ...
      if (!isKnownInternalHookEventKey(handlerConfig.event))
        log.warn(
          `Legacy hook handler ${safeLogValue(rawModule)} subscribes to event ${safeLogValue(handlerConfig.event)} not emitted by OpenClaw core — likely a typo; unless a plugin emits it, the hook never fires. Known events: https://docs.openclaw.ai/automation/hooks`,
        );
