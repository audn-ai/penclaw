...
import {
  ...
  l as clearMemoryCoreWorkspaceNamespace,
...
import { n as extractErrorCode } from "./errors-BoHeli7m.js";
...
async function repairDreamingArtifacts(params) {
  ...
  if (sessionCorpusDestination || sessionIngestionDestination)
    try {
      ...
      warnings.push(
        `Failed clearing dreaming session-ingestion SQLite state: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
