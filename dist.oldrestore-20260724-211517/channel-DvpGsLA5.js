...
import { n as NostrProfileSchema, t as NostrConfigSchema } from "./config-schema-BJmy1b1O.js";
...
import { $ as randomBytes, Z as hexToBytes, i as cbc } from "./hkdf-CTfM9HGH.js";
...
import { i as DEFAULT_RELAYS } from "./setup-adapter-BeDn7fu4.js";
import {
  ...
  u as getPublicKey,
} from "./setup-surface-Bcb_g_F_.js";
...
async function startNostrBus(options) {
  ...
  function scheduleStatePersist(eventCreatedAt, eventId) {
    ...
    if (recentEventIds.length > MAX_PERSISTED_EVENT_IDS)
      recentEventIds = recentEventIds.slice(-5e3);
  ...
  return {
    close: () => {
      ...
      Promise.resolve(sub.close("closed by caller"))
        .catch((err) => onError?.(err, "close subscription"))
        .finally(() => {
          pool.close(relays);
        });
      seen.stop();
  ...
  const authorizeSender = async (input) => {
    ...
    if (resolved.senderAccess.decision === "pairing") {
      await pairing.issueChallenge({
        ...
        onReplyError: (err) => {
          ctx.log?.warn?.(
            `[${account.accountId}] nostr pairing reply failed for ${input.senderId}: ${String(err)}`,
          );
        },
      });
      return "pairing";
      ...
      const bus = await startNostrBus({
        ...
        onMessage: async (senderPubkey, text, reply, meta) => {
          ...
          const { dispatchInboundDirectDmWithRuntime } =
            await import("./inbound-direct-dm-runtime-Faqft783.js");
