...
async function requireStoredSession(selector) {
  ...
  const matches = (await listStoredSessions()).filter(
    (entry) => entry.session.sessionId === selector,
  );
  if (matches.length === 1 && matches[0]) return assertRequestedSession(matches[0], selector);
  if (matches.length > 1)
    throw new Error(
      `multiple transcripts sessions match ${selector}; use one of: ${matches.map(formatSelector).join(", ")}`,
    );
...
async function listStoredSessions() {
  ...
  return (await Promise.all(dirs.map((dir) => readStoredSession(dir, { ignoreInvalid: true }))))
