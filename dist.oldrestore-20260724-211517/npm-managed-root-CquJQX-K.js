...
import { a as readJsonIfExists, n as readJson } from "./json-files-CTWRDHag.js";
...
function applyManagedNpmRootOverrides(params) {
  ...
  return {
    overrides,
    managedOverrideKeys: Object.keys(params.managedOverrides)
      ...
      if (isHostPeerResolutionFailure(result)) {
        if (
          ...
          return collectNpmLockPeerDependencyPins({
            lockfile: await readManagedNpmRootManifest(tempLockPath),
          });
      }
...
async function syncManagedNpmRootPeerDependencies(params) {
  ...
  for (const packageName of previousManagedPeerDependencies)
    if (!Object.hasOwn(peerPins, packageName)) delete nextDependencies[packageName];
  for (const [packageName, dependencySpec] of Object.entries(peerPins))
    if (managedPeerDependencyNames.has(packageName)) nextDependencies[packageName] = dependencySpec;
  const { overrides, managedOverrideKeys } = applyManagedNpmRootOverrides({
    manifest,
    managedOverrides: params.omitUnsupportedManagedOverrides
      ? filterUnsupportedManagedNpmRootOverrides(params.managedOverrides)
      : readOverrideRecord(params.managedOverrides),
    dependencies: nextDependencies,
...
//#endregion
export {
  removeManagedNpmRootDependency as a,
  restoreManagedNpmRootPeerDependencySnapshot as c,
  readOpenClawManagedNpmRootOverrides as i,
  syncManagedNpmRootPeerDependencies as l,
  readManagedNpmRootInstalledDependency as n,
  repairManagedNpmRootOpenClawPeer as o,
  readManagedNpmRootPeerDependencySnapshot as r,
  resolveManagedNpmRootDependencySpec as s,
  listMissingRequiredPlatformPackages as t,
  upsertManagedNpmRootDependency as u,
};
