...
function normalizeDeepJsonPathSegments(segments, file, input) {
  ...
  if (pathSegments.length > 256)
    fail(
      `JSON oc:// path exceeds 256 nested segments: ${printable(input)}`,
      input,
      "OC_PATH_TOO_DEEP",
    );
...
function formatOcPath(path) {
  ...
  if (path.field !== void 0 && path.item === void 0)
...
function parseMd(raw) {
  ...
  const { preamble, blocks } = walkBlocks(
    md.parse(bodyLines.join("\n"), {}),
    bodyLines,
    bodyFileLine,
  );
  return {
...
function unquote(value) {
  if (value.length >= 2) {
    ...
    if (f === value.charCodeAt(value.length - 1) && (f === 34 || f === 39))
...
function toDiagnostic(error, lineMap, tree) {
  ...
  const isTrailingInput =
    ...
    (tree !== void 0 && errorCode === JSONC_PARSE_INVALID_SYMBOL && error.offset >= treeEnd);
  return {
...
function emitMd(ast, opts = {}) {
  ...
  if (mode === "roundtrip") {
    if (!acceptPreExisting && ast.raw.includes("__OPENCLAW_REDACTED__"))
      guardSentinel$1("__OPENCLAW_REDACTED__", `${guardPath}/[raw]`);
    return ast.raw;
  }
  const parts = [];
  ...
  for (const block of ast.blocks) {
    ...
    if (block.bodyText.length > 0) {
      ...
      for (const item of block.items)
        if (item.kv)
          guardSentinel$1(item.kv.value, `${guardPath}/${block.slug}/${item.slug}/${item.kv.key}`);
...
function setMdInsertion(ast, info, value) {
  ...
  if (p.item === void 0 && p.field === void 0) {
    ...
    const kvKey =
      kvMatch === null ? void 0 : expectDefined(kvMatch[1], "Markdown item key capture");
    const kvValue =
      kvMatch === null ? void 0 : expectDefined(kvMatch[2], "Markdown item value capture");
    const newItem = {
      text: value,
...
const jsonlOps = {
  ...
  *predicate(ast, pred) {
    for (const l of ast.lines) {
      ...
      if (evaluatePredicate(topLevelLeafText(l.value, pred.key), pred))
        yield {
          keySub: `L${l.line}`,
          child: lineHolder(ast, l),
        };
...
const yamlOps = {
  *enumerate(node) {
  ...
  lookup(node, key) {
    if (isMap(node)) {
      ...
      return pair?.value === void 0 || pair.value === null
        ? null
        : {
            keySub: key,
            child: pair.value,
  ...
  *predicate(node, pred) {
    if (isMap(node))
      for (const p of node.items) {
        const k = isScalar(p.key) ? p.key.value : p.key;
        if (p.value !== null && yamlChildMatchesPredicate(p.value, pred))
          yield {
            keySub: quoteSeg(String(k)),
            child: p.value,
          };
      ...
      for (let idx = 0; idx < node.items.length; idx++) {
        const child = node.items[idx];
        if (child !== null && yamlChildMatchesPredicate(child, pred))
          yield {
            keySub: String(idx),
            child,
          };
