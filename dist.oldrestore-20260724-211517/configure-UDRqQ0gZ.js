...
import { t as runSecretsApply } from "./apply-BzVasP8P.js";
...
import {
  o as listSecretProviderIntegrationPresets,
...
import { o as createSecretsConfigIO } from "./storage-scan-DmQrHGJt.js";
import {
...
function collectConfigureProviderChanges(params) {
  ...
  for (const providerAlias of Object.keys(originalProviders))
    if (!Object.hasOwn(nextProviders, providerAlias)) deletes.push(providerAlias);
  return {
    upserts,
...
async function promptProviderConfig(source, current) {
  ...
  return await promptExecProvider(
    current?.source === "exec" && "command" in current ? current : void 0,
  );
}
