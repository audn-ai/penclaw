...
const daemonStatusModuleLoader = createLazyImportLoader(
  () => import("./status.runtime-BCE5cp41.js"),
);
...
function addGatewayServiceCommands(parent, opts) {
  ...
  parent
    ...
    .option(
      "--safe",
      "Request an OpenClaw-aware restart after active work drains (bounded wait; may force after gateway.reload.deferralTimeoutMs expires; set deferralTimeoutMs=0 for indefinite wait)",
      false,
    )
    .option("--skip-deferral", "Bypass the safe-restart deferral gate; requires --safe", false)
    .option(
      "--wait <duration>",
      "Wait duration before restart (ms, 10s, 5m; 0 waits indefinitely). For non-safe restarts (plain restart); not compatible with --force or --safe",
    )
    .option("--json", "Output JSON", false)
    .action(async (cmdOpts, command) => {
