...
import { a as mergeDiscordAccountConfig, s as resolveDiscordAccount } from "./accounts-BNPpAWWI.js";
import {
  Ct as getGuildMember,
  Kt as PermissionFlagsBits$1,
  Lt as ChannelType$1,
  St as getGuild,
  T as Embed,
  Ut as MessageFlags$1,
  X as getCurrentUser,
  Y as createUserDmChannel,
  Yt as require_v10,
  et as createChannelMessage,
  f as RequestClient,
  m as RateLimitError,
  ot as getChannel,
  w as serializePayload,
...
import { t as danger } from "./globals-CNUzHTcU.js";
...
import { h as resolveTextChunksWithFallback } from "./reply-payload-DDwHXDkv.js";
import {
  i as chunkDiscordTextWithMode,
  n as resolveDiscordReplyMessageId,
} from "./reply-reference-CeynpHZu.js";
...
import { a as resolveRetryConfig } from "./src-Dqmh2693.js";
...
import { t as parseAndResolveDiscordTarget } from "./target-resolver-DaaelveM.js";
...
function isRetryableDiscordGatewayTransportError(err) {
  ...
  return !collectErrorGraphCandidates(err, (current) => [current.cause, current.error]).some(
    (candidate) => readDiscordErrorStatus(candidate) !== void 0,
  );
}
 ...
 */
async function hasAnyChannelPermissionDiscord(
  guildId,
  channelId,
  userId,
  requiredPermissions,
  ...
  const rest = resolveDiscordRest(opts);
  try {
    const permissionChannel = await resolveChannelPermissionSubject(
      rest,
      await getChannel(rest, channelId),
    );
    if (("guild_id" in permissionChannel ? permissionChannel.guild_id : void 0) !== guildId)
      return false;
    const [guild, member] = await Promise.all([
      getGuild(rest, guildId),
      getGuildMember(rest, guildId, userId),
...
async function canManageGuildMemberRoleDiscord(
  guildId,
  senderUserId,
  targetUserId,
  roleId,
...
function buildDiscordMessageRequest(params) {
  ...
  const nonce =
    params.endpoint === "create-message" ? (params.nonce ?? createDiscordMessageNonce()) : void 0;
...
async function sendDiscordText(params) {
  ...
  if (chunks.length === 1) {
    const { result, replyToId } = await sendChunk(
      expectDefined(chunks.at(0), "single Discord text chunk"),
      true,
    );
...
//#endregion
export {
  hasAllGuildPermissionsDiscord as A,
  DISCORD_REST_TIMEOUT_MS as B,
  DISCORD_MAX_STICKER_BYTES as C,
  canViewDiscordGuildChannel as D,
  canManageGuildRoleDiscord as E,
  createDiscordClient as F,
  registerGateway as G,
  withValidatedDiscordProxy as H,
  createDiscordRestClient as I,
  unregisterGateway as K,
  createDiscordRuntimeAccountContext as L,
  hasAnyGuildPermissionDiscord as M,
  isThreadChannelType as N,
  fetchChannelPermissionsDiscord as O,
  parseAndResolveChannelRecipient as P,
  resolveDiscordClientAccountContext as R,
  DISCORD_MAX_EVENT_COVER_BYTES as S,
  canManageGuildMemberRoleDiscord as T,
  clearGateways as U,
  validateDiscordProxyUrl as V,
  getGateway as W,
  resolveDiscordMessageFlags as _,
  normalizeDiscordPollInput as a,
  stripUndefinedFields as b,
  normalizeStickerIds as c,
  resolveDiscordTargetChannelId as d,
  sendDiscordMedia as f,
  createDiscordMessageNonce as g,
  buildDiscordMessageRequest as h,
  formatReactionEmoji as i,
  hasAnyChannelPermissionDiscord as j,
  fetchMemberGuildPermissionsDiscord as k,
  resolveChannelId as l,
  SUPPRESS_NOTIFICATIONS_FLAG as m,
  ...
  resolveDiscordChannel as u,
  resolveDiscordSendComponents as v,
  DiscordSendError as w,
  DISCORD_MAX_EMOJI_BYTES as x,
  resolveDiscordSendEmbeds as y,
  resolveDiscordRest as z,
};
